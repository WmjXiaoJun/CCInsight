      ...
      {activeTab === 'files' && (
        <>
          ...
          <div className="scrollbar-thin flex-1 overflow-y-auto py-2">
            {fileTree.length === 0 ? (
              <div className="px-3 py-4 text-center text-xs text-text-muted">暂无加载文件</div>
            ) : (
              fileTree.map((node) => (
                <TreeItem
                  key={node.id}
        ...
        <p className="mb-3 text-[11px] text-text-muted">
          ...
          <div className="mt-6 border-t border-border-subtle pt-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              <p className="mt-2 text-[10px] text-amber-400">选择一个节点以应用深度过滤</p>
            )}
          </div>
      ...
      {graph && (
        <div className="border-t border-border-subtle bg-elevated/50 px-3 py-2">
          <div className="flex items-center justify-between text-[10px] text-text-muted">
            ...
            <span>{graph.relationships.length} 条边</span>
          </div>
        </div>
      )}
    </div>
