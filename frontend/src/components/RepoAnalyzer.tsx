...
// Check if path is absolute: starts with / (Unix), C:\ or other drive letter (Windows), or \\ (UNC)
function isAbsolutePath(p: string): boolean {
  if (!p || p.trim().length < 2) return false;
  // Unix absolute path
...
function AnalyzeButton({
  ...
  return (
    ...
    >
      ...
      <span>{isLoading ? '正在启动分析...' : '分析仓库'}</span>
...
function DoneState({ repoName }: { repoName: string }) {
  return (
    ...
    >
      ...
      <p className="text-xs text-text-secondary">正在加载图谱...</p>
    </div>
  );
}
...
export const RepoAnalyzer = ({ variant, onComplete, onCancel }: RepoAnalyzerProps) => {
  ...
  const nodeStreamControllerRef = useRef<AbortController | null>(null);
  const completeTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    return () => {
      sseControllerRef.current?.abort();
      nodeStreamControllerRef.current?.abort();
  ...
  const handleAnalyze = async () => {
    if (mode === 'github' && !isValidGithubUrl(githubUrl)) {
      setValidationError('请输入有效的 GitHub 仓库地址。');
      return;
    }
    if (mode === 'local' && localPath.trim().length < 2) {
      setValidationError('请输入文件夹路径。');
      return;
    }
    if (mode === 'local' && !isAbsolutePath(localPath)) {
      setValidationError('请输入绝对路径（以 / 或 C:\\ 开头的完整路径）。');
      return;
    }
    ...
    try {
      ...
      // Subscribe to node batches for real-time stats
      nodeStreamControllerRef.current = streamAnalysisNodes(
        jobId,
        (batch: GraphNodeBatch) => {
          setStreamStats((prev) => ({
            nodes: prev.nodes + batch.nodes.length,
            edges: prev.edges + batch.relationships.length,
          }));
        },
        () => {
          // Done streaming
      ...
      const controller = streamAnalyzeProgress(
        ...
        (errMsg) => {
          setValidationError(errMsg || '分析失败，请检查服务器日志。');
          setPhase('error');
        },
      );
      sseControllerRef.current = controller;
    } catch (err) {
      setValidationError(err instanceof Error ? err.message : '启动分析失败');
      setPhase('error');
    }
  };
  ...
  const handleCancel = async () => {
    ...
    sseControllerRef.current = null;
    nodeStreamControllerRef.current?.abort();
    nodeStreamControllerRef.current = null;
    if (jobIdRef.current) {
      try {
    ...
    <div className="space-y-4">
      ...
      {phase === 'analyzing' && (
        <div className="animate-slide-up">
          <AnalyzeProgress progress={progress} onCancel={handleCancel} streamStats={streamStats} />
        </div>
      )}
