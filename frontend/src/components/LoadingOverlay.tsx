import type { PipelineProgress } from 'ccinsight-shared';
      ...
      {progress.stats && (
        <div className="mt-8 flex items-center gap-6 text-xs text-text-muted">
          <div className="flex items-center gap-2">
            ...
            <span>
              {progress.stats.filesProcessed} / {progress.stats.totalFiles} 个文件
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-node-function" />
            <span>{progress.stats.nodesCreated} 个节点</span>
          </div>
        </div>
      )}
