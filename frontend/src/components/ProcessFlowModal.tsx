...
// Initialize mermaid with cyan/purple theme matching ccinsight
// Initialize mermaid with cyan/purple theme matching ccinsight
mermaid.initialize({
  startOnLoad: false,
        ...
        diagramRef.current!.innerHTML = `
          <div class="text-center p-8">
            <div class="text-red-400 text-sm font-medium mb-2">
              ${isSizeError ? '📊 图谱过大' : '⚠️ 渲染错误'}
            </div>
            <div class="text-slate-400 text-xs max-w-md">
              ${
                isSizeError
                  ? `此图有 ${process.steps?.length || 0} 个步骤，过于复杂无法渲染。建议改为查看单个执行流程而不是"全部执行流程"。`
                  : `无法渲染图谱。步骤数：${process.steps?.length || 0}`
              }
            </div>
          </div>
        `;
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 border-b border-white/10 px-6 py-5">
          <h2 className="text-lg font-semibold text-white">执行流程：{process.label}</h2>
        </div>
