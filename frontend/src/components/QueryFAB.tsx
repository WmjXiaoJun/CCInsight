...
export const QueryFAB = () => {
  ...
  const handleRunQuery = useCallback(async () => {
    ...
    if (!graph) {
      setError('未加载项目。请先加载一个项目。');
      return;
    }

    const ready = await isDatabaseReady();
    if (!ready) {
      setError('数据库未就绪。请等待加载完成。');
      return;
    }
    ...
    >
      <div className="flex items-center justify-between border-b border-border-subtle px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Cypher 查询</span>
        </div>
        <button
          onClick={handleClose}
          ...
          <div className="flex items-center justify-between bg-cyan-500/5 px-4 py-2.5">
            <div className="flex items-center gap-3 text-xs">
              <span className="text-text-secondary">
                <span className="font-semibold text-cyan-400">{queryResult.rows.length}</span> 条记录
              </span>
              {queryResult.nodeIds.length > 0 && (
                <span className="text-text-secondary">
                  <span className="font-semibold text-cyan-400">{queryResult.nodeIds.length}</span> 个高亮
                </span>
              )}
            ...
            <div className="scrollbar-thin max-h-48 overflow-auto border-t border-border-subtle">
              ...
              {queryResult.rows.length > 50 && (
                <div className="border-t border-border-subtle bg-surface px-3 py-2 text-xs text-text-muted">
                  显示前 50 条，共 {queryResult.rows.length} 条
                </div>
              )}
            </div>
          )}
