  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-[11px] text-text-muted">检测到服务器后自动连接</p>
      </div>
    </div>
  );
}
...
export const OnboardingGuide = ({ isPolling }: OnboardingGuideProps) => {
  const primary = isDev ? 'cd ccinsight && npm run serve' : 'npx ccinsight@latest serve';
  const termLabel = isDev ? '启动后端' : '终端';
    ...
    <div className="relative animate-fade-in overflow-hidden rounded-3xl border border-border-default bg-surface p-7">
      ...
      <div className="relative space-y-5">
        ...
        <StepRow
          ...
          number={1}
          title="复制命令"
          description={isPolling ? undefined : '点击终端内的图标复制命令。'}
        >
          ...
          {!isDev && (
            <>
              ...
              <TerminalWindow
                command="npm install -g ccinsight && ccinsight serve"
                label="全局安装"
                isActive={false}
              />
            </>
        ...
        <StepRow
          state={step2State}
          number={2}
          title={isPolling ? '等待服务器启动' : '粘贴到终端并运行'}
          description={isPolling ? undefined : '打开新的终端窗口，粘贴命令并按回车。'}
        >
