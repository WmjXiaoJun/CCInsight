...
export const EDGE_INFO: Record<EdgeType, { color: string; label: string }> = {
  CONTAINS: { color: '#2d5a3d', label: '包含' },
  DEFINES: { color: '#0e7490', label: '定义' },
  IMPORTS: { color: '#1d4ed8', label: '导入' },
  CALLS: { color: '#7c3aed', label: '调用' },
  EXTENDS: { color: '#c2410c', label: '继承' },
  IMPLEMENTS: { color: '#be185d', label: '实现' },
};
