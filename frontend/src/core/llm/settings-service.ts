...
const STORAGE_KEY = 'cinsight-llm-settings';
              ...
              : T extends 'openrouter'
                ...
                : T extends 'minimax'
                  ...
                  : T extends 'glm'
                    ...
                    : T extends 'generic'
                      ? Partial<Omit<GenericConfig, 'provider'>>
                      : never
  >,
): LLMSettings => {
  ...
  switch (provider) {
    ...
    case 'generic': {
      const updated: LLMSettings = {
        ...
        generic: {
          ...(current.generic ?? {}),
          ...(updates as Partial<Omit<GenericConfig, 'provider'>>),
        },
      };
      saveSettings(updated);
      return updated;
...
const providerBuilders: Record<LLMProvider, ProviderBuilder> = {
  ...
  generic: (settings) => {
    if (!settings.generic?.apiKey || !settings.generic?.baseUrl) return null;
    return {
      provider: 'generic',
      apiKey: settings.generic.apiKey,
      model: settings.generic.model || '',
      modelVariant: settings.generic.modelVariant,
      baseUrl: settings.generic.baseUrl,
      temperature: settings.generic.temperature,
      maxTokens: settings.generic.maxTokens,
    } as GenericConfig;
  },
};
