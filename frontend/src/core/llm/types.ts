...
export interface ModelDefinition {
  ...
  /** Available variants for this model (not all models have all variants) */
  variants?: Partial<Record<ModelVariant, Record<string, unknown>>>;
}
 ...
 */
export interface GenericConfig extends BaseProviderConfig {
  provider: 'generic';
  apiKey: string;
  model: string; // e.g., 'gpt-4o', 'claude-3-sonnet', etc.
  modelVariant?: ModelVariant; // Selected variant (low/medium/high/xhigh)
  baseUrl: string; // e.g., 'https://new.fastaicode.top/v1'
  apiVersion?: string; // optional, for APIs that require it
...
export interface LLMSettings {
  ...
  generic?: Partial<Omit<GenericConfig, 'provider'>> & {
    /** Available models defined by the provider config */
