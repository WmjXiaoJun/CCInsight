...
import { ProxyChatModel } from './proxy-chat-model';
import type {
  ProviderConfig,
  OpenAIConfig,
  AzureOpenAIConfig,
 ...
 */
export const BASE_SYSTEM_PROMPT = `You are CInsight, a Code Analysis Agent with access to a Knowledge Graph. Your responses MUST be grounded.
...
export const createChatModel = (config: ProviderConfig): BaseChatModel => {
  switch (config.provider) {
    ...
    case 'generic': {
      const genericConfig = config as GenericConfig;

      if (!genericConfig.baseUrl || genericConfig.baseUrl.trim() === '') {
        throw new Error('Base URL is required for generic API provider');
      }

      if (!genericConfig.model || genericConfig.model.trim() === '') {
        throw new Error('Model name is required for generic API provider');
      }

      // Use proxy to avoid CORS issues
      return new ProxyChatModel({
        baseUrl: genericConfig.baseUrl,
        apiKey: genericConfig.apiKey,
        model: genericConfig.model,
        temperature: genericConfig.temperature ?? 0.1,
        maxTokens: genericConfig.maxTokens,
        streaming: true,
