...
/**
 * E2E tests for the ccinsight web UI ?exploring view features.
 *
 * Requires:
 *   - ccinsight serve running on localhost:4747 with at least one indexed repo
 *   - ccinsight-web dev server running on localhost:5173
 *
...
const FRONTEND_URL = process.env.FRONTEND_URL ?? 'http://localhost:5288';
...
test.beforeAll(async () => {
  ...
  try {
    ...
    if (
      ...
      test.skip(true, 'ccinsight serve not available on :4747');
      return;
    }
    if (
      ...
      test.skip(true, 'Vite dev server not available on :5288');
      return;
    }
    ...
    if (backendRes.status === 'fulfilled') {
      ...
      if (!repos.length) {
        test.skip(true, 'No indexed repos ?run ccinsight analyze first');
        return;
      }
    }
  } catch {
...
test.describe('CInsight AI', () => {
  test('panel opens and agent initializes without error', async ({ page }, testInfo) => {
    await waitForGraphLoaded(page, testInfo);

    await page.getByRole('button', { name: 'CInsight AI' }).click();
