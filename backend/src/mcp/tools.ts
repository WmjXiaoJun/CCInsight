 ...
 *
 * Defines the tools that ccinsight exposes to external AI agents.
...
export const ccinsight_TOOLS: ToolDefinition[] = [
  {
    name: 'list_repos',
    description: `List all indexed repositories available to ccinsight.

Returns each repo's name, path, indexed date, last commit, and stats.

WHEN TO USE: First step when multiple repos are indexed, or to discover available repos.
AFTER THIS: READ ccinsight://repo/{name}/context for the repo you want to work with.
...
WHEN TO USE: Complex structural queries that search/explore can't answer. READ ccinsight://repo/{name}/schema first for the full schema.
...
OUTPUT: Returns { markdown, row_count } â€?results formatted as a Markdown table for easy reading.

TIPS:
- All relationships use single CodeRelation table â€?filter with {type: 'CALLS'} etc.
...
AFTER THIS: Use impact() if planning changes, or READ ccinsight://repo/{name}/process/{processName} for full execution trace.
...
AFTER THIS: Review affected processes. Use context() on high-risk symbols. READ ccinsight://repo/{name}/process/{name} for full traces.
...
Risk levels: LOW (0-3 consumers), MEDIUM (4-9 or any mismatches), HIGH (10+ consumers or mismatches with 4+ consumers). Mismatches with confidence "low" indicate the consumer file fetches multiple routes â€?property attribution is approximate.
