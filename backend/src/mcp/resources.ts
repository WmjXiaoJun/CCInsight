 ...
 * All resources use repo-scoped URIs: ccinsight://repo/{name}/context
 */
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'ccinsight://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'ccinsight://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'ccinsight://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'ccinsight://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'ccinsight://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'ccinsight://repo/{name}/process/{processName}',
      name: 'Process Trace',
...
function parseUri(uri: string): { repoName?: string; resourceType: string; param?: string } {
  if (uri === 'ccinsight://repos') return { resourceType: 'repos' };
  if (uri === 'ccinsight://setup') return { resourceType: 'setup' };

  // Repo-scoped: ccinsight://repo/{name}/context
  const repoMatch = uri.match(/^ccinsight:\/\/repo\/([^/]+)\/(.+)$/);
  if (repoMatch) {
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: ccinsight analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# ccinsight_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: ccinsight analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `npx ccinsight analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - ccinsight://repos: All indexed repositories');
  lines.push(`  - ccinsight://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - ccinsight://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - ccinsight://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - ccinsight://repo/${context.projectName}/process/{name}: Process trace`);
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: ccinsight analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use ccinsight_query for deeper search.`,
      );
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: ccinsight analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use ccinsight_query for deeper search.`,
      );
    }
 ...
 * Useful for `ccinsight setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  const repos = await backend.listRepos();

  if (repos.length === 0) {
    return '# ccinsight\n\nNo repositories indexed. Run: `npx ccinsight analyze` in a repository.';
  }
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# ccinsight MCP �?${repo.name}`,
      '',
      `This project is indexed by ccinsight as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`ccinsight://repo/${repo.name}/context\` �?Stats, staleness check`,
      `- \`ccinsight://repo/${repo.name}/clusters\` �?All functional areas`,
      `- \`ccinsight://repo/${repo.name}/processes\` �?All execution flows`,
      `- \`ccinsight://repo/${repo.name}/schema\` �?Graph schema for Cypher`,
    ];
