 ...
 *
 * Provides tool implementations using local .ccinsight/ indexes.
...
// AI context generation is CLI-only (ccinsight analyze)
...
function logQueryError(context: string, err: unknown): void {
  ...
  console.error(`ccinsight [${context}]: ${msg}`);
}
...
export class LocalBackend {
  ...
  private async refreshRepos(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (kuzu.found && kuzu.needsReindex) {
        console.error(
          `ccinsight: "${entry.name}" has a stale KuzuDB index. Run: ccinsight analyze ${entry.path}`,
        );
      }
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: ccinsight analyze');
    }
    if (repoParam) {
  ...
  private async query(
    ...
    return {
      ...
      ...(!ftsUsed && {
        warning:
          'FTS extension unavailable - keyword search degraded. Run: ccinsight analyze --force to rebuild indexes.',
      }),
    };
  }
  ...
  private async bm25Search(
    ...
    try {
      ...
      console.error('ccinsight: BM25/FTS search failed (FTS indexes may not exist) -', err.message);
  ...
  private async context(
    ...
    // Fix #480: Class/Interface nodes have no direct CALLS/IMPORTS edges �?    // those point to Constructor and File nodes respectively. Fetch those
  ...
  private async impact(
    ...
    try {
      ...
      return {
        ...
        impactedCount: 0,
        risk: 'UNKNOWN',
        suggestion: 'The graph query failed �?try ccinsight context <symbol> as a fallback',
      };
    }
  }
