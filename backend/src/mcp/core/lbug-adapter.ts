 ...
 *
 * LadybugDB Connections are NOT thread-safe â€?a single Connection
 ...
 *
 * @see https://docs.ladybugdb.com/concurrency â€?multiple Connections
...
interface PoolEntry {
  ...
  lastUsed: number;
  dbPath: string;
  /** Set to true when the pool entry is closed â€?checkin will close orphaned connections */
  closed: boolean;
}
...
interface SharedDB {
  ...
  refCount: number;
  ftsLoaded: boolean;
  /** When true, closeOne skips db.close() â€?the Database is owned externally. */
  external?: boolean;
}
...
/** Saved real stdout.write â€?used to silence LadybugDB native output without race conditions */
export const realStdoutWrite = process.stdout.write.bind(process.stdout);
let stdoutSilenceCount = 0;
/** True while pre-warming connections â€?prevents watchdog from prematurely restoring stdout */
...
function closeOne(repoId: string): void {
  ...
  // Close available connections â€?fire-and-forget with .catch() to prevent
  ...
  // Checked-out connections can't be closed here â€?they're in-flight.
  ...
  // External databases (injected via initLbugWithDb) are never closed here â€?  // the core adapter owns them and handles their lifecycle.
  const shared = dbCache.get(entry.dbPath);
  if (shared) {
    shared.refCount--;
    if (shared.refCount === 0) {
      if (shared.external) {
        // External databases are owned by the core adapter â€?don't close
...
// Exempts active queries and pre-warm â€?these legitimately hold silence for
 ...
 * Retries on lock errors (e.g., when `ccinsight analyze` is running).
 *
 * Concurrent calls for the same repoId are deduplicated â€?the second caller
...
export const initLbug = async (repoId: string, dbPath: string): Promise<void> => {
  ...
  // Deduplicate concurrent init calls for the same repoId â€?  // prevents double-init race when multiple parallel tool calls
...
/**
 * Internal init â€?creates DB, pre-warms connections, loads FTS, then registers pool.
 * Pool entry is registered LAST so concurrent executeQuery calls see either
 * "not initialized" (and throw) or a fully ready pool â€?never a half-built one.
 */
async function doInitLbug(repoId: string, dbPath: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`LadybugDB not found at ${dbPath}. Run: ccinsight analyze`);
  }
  ...
  if (!shared) {
    // Open in read-only mode â€?MCP server never writes to the database.
    // This allows multiple MCP server instances to read concurrently, and
    // avoids lock conflicts when `ccinsight analyze` is writing.
  ...
  if (!shared.ftsLoaded) {
    try {
      ...
      // Extension may not be installed â€?FTS queries will fail gracefully
    }
  }

  // Register pool entry only after all connections are pre-warmed and FTS is
  // loaded.  Concurrent executeQuery calls see either "not initialized"
  // (and throw cleanly) or a fully ready pool â€?never a half-built one.
  pool.set(repoId, {
    db,
    available,
    checkedOut: 0,
 ...
 *
 * Used in tests to avoid the writableâ†’closeâ†’read-only cycle that crashes
...
function checkout(entry: PoolEntry): Promise<lbug.Connection> {
  ...
  // with fewer total connections, something leaked â€?surface the bug rather
  ...
  // At capacity â€?queue the caller with a timeout.
...
function checkin(entry: PoolEntry, conn: lbug.Connection): void {
  if (entry.closed) {
    // Pool entry was deleted during checkout â€?close the orphaned connection
    conn.close().catch(() => {});
    return;
  }
  if (entry.waiters.length > 0) {
    // Hand directly to the next waiter â€?no intermediate available state
 ...
 * Note: CALL is NOT blocked â€?it's used for read-only FTS (CALL QUERY_FTS_INDEX)
