...
program.name('ccinsight').description('ccinsight local CLI and MCP server').version(pkg.version);
...
program
  ...
  .option('--skip-agents-md', 'Skip updating the ccinsight section in AGENTS.md and CLAUDE.md')
  .option('--skip-git', 'Index a folder without requiring a .git directory')
  .option('-v, --verbose', 'Enable verbose ingestion warnings (default: false)')
  .addHelpText(
    'after',
    '\nEnvironment variables:\n  ccinsight_NO_GITIGNORE=1  Skip .gitignore parsing (still reads .ccinsightignore)',
  )
...
program
  .command('index [path...]')
  .description(
    'Register an existing .ccinsight/ folder into the global registry (no re-analysis needed)',
  )
...
program
  .command('clean')
  .description('Delete ccinsight index for current repo')
...
program
  ...
  .option('--api-key <key>', 'LLM API key or Azure api-key (saved to ~/.ccinsight/config.json)')
  .option(
    '--api-version <version>',
