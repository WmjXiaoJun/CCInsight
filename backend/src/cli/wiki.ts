 ...
 * Usage: ccinsight wiki [path] [options]
 */
...
export const wikiCommand = async (inputPath?: string, options?: WikiCommandOptions) => {
  ...
  if (options?.verbose) {
    process.env.ccinsight_VERBOSE = '1';
  }

  console.log('\n  ccinsight Wiki Generator\n');
  ...
  if (!meta) {
    console.log('  Error: No ccinsight index found.');
    console.log('  Run `ccinsight analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (
    ...
    console.log('  Config saved to ~/.ccinsight/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      ...
      if (!llmConfig.apiKey && llmConfig.provider !== 'cursor') {
        ...
        console.log('  Set OPENAI_API_KEY or ccinsight_API_KEY environment variable,');
      ...
      if (choice === '5' && hasCursor) {
        ...
        // API key
        const envKey = process.env.ccinsight_API_KEY || process.env.OPENAI_API_KEY || '';
        let azureKey: string;
        if (envKey) {
  ...
  try {
    ...
    if (options?.review && result.moduleTree) {
      ...
      if (choice === 'n' || choice === 'no') {
        console.log('\n  Generation cancelled. Run `ccinsight wiki` later to generate.\n');
        return;
      }
      ...
      if (choice === 'edit' || choice === 'e') {
        ...
        try {
          ...
          console.log('  Then run `ccinsight wiki` to continue.\n');
          return;
        }
      }
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `ccinsight wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
      if (process.env.ccinsight_VERBOSE) {
        console.error(err);
      }
    }
    process.exitCode = 1;
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync(
      'gh',
      ['gist', 'create', htmlPath, '--desc', 'Repository Wiki �?generated by ccinsight', '--public'],
