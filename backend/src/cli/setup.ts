 ...
 * so the ccinsight MCP server is available in all projects.
 */
...
function getMcpEntry() {
  if (process.platform === 'win32') {
    return {
      command: 'cmd',
      args: ['/c', 'npx', '-y', 'ccinsight@latest', 'mcp'],
    };
  }
  return {
    command: 'npx',
    args: ['-y', 'ccinsight@latest', 'mcp'],
  };
}

/**
 * Merge ccinsight entry into an existing MCP config JSON object.
...
function mergeMcpConfig(existing: any): any {
  ...
  existing.mcpServers.ccinsight = getMcpEntry();
  return existing;
}
...
/**
 * Install ccinsight skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install ccinsight hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the ccinsight package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/ccinsight/
  const destHooksDir = path.join(claudeDir, 'hooks', 'ccinsight');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'ccinsight-hook.cjs');
    const dest = path.join(destHooksDir, 'ccinsight-hook.cjs');
    try {
    ...
    const hookPath = path.join(destHooksDir, 'ccinsight-hook.cjs').replace(/\\/g, '/');
    ...
    // Helper: add a hook entry if one with 'ccinsight-hook' isn't already registered
    interface HookEntry {
    ...
    function ensureHookEntry(
      ...
      const hasHook = existing.hooks[eventName].some((h: HookEntry) =>
        h.hooks?.some((hh) => hh.command?.includes('ccinsight-hook')),
      );
      if (!hasHook) {
    ...
    ensureHookEntry('PreToolUse', 'Grep|Glob|Bash', 10, 'Enriching with ccinsight graph context...');
    ensureHookEntry('PostToolUse', 'Bash', 10, 'Checking ccinsight index freshness...');
...
async function setupOpenCode(result: SetupResult): Promise<void> {
  ...
  try {
    ...
    config.mcp.ccinsight = getMcpEntry();
...
function getCodexMcpTomlSection(): string {
  ...
  return `[mcp_servers.ccinsight]\ncommand = ${command}\nargs = ${args}\n`;
}

/**
 * Append ccinsight MCP server config to Codex's config.toml if missing.
 */
async function upsertCodexConfigToml(configPath: string): Promise<void> {
  ...
  if (existing.includes('[mcp_servers.ccinsight]')) {
    return;
  }
...
async function setupCodex(result: SetupResult): Promise<void> {
  ...
  try {
    const entry = getMcpEntry();
    await execFileAsync('codex', ['mcp', 'add', 'ccinsight', '--', entry.command, ...entry.args], {
...
/**
 * Install ccinsight skills to a target directory.
 * Each skill is installed as {targetDir}/ccinsight-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/ccinsight/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/ccinsight/
 */
...
/**
 * Install global Codex skills to ~/.agents/skills/ccinsight/
 */
...
export const setupCommand = async () => {
  ...
  console.log('    2. Run: ccinsight analyze');
