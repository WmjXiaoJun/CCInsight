 ...
 *
 * Exposes ccinsight tools (query, context, impact, cypher) as direct CLI commands.
 * Bypasses MCP entirely â€?invokes LocalBackend directly for minimal overhead.
 *
 * Usage:
 *   ccinsight query "authentication flow"
 *   ccinsight context --name "validateUser"
 *   ccinsight impact --target "AuthService" --direction upstream
 *   ccinsight cypher "MATCH (n:Function) RETURN n.name LIMIT 10"
 *
...
async function getBackend(): Promise<LocalBackend> {
  ...
  if (!ok) {
    console.error('ccinsight: No indexed repositories found. Run: ccinsight analyze');
    process.exit(1);
  }
  return _backend;
}
...
function output(data: any): void {
  ...
  try {
    ...
    if (err?.code === 'EPIPE') {
      // Consumer closed the pipe (e.g., `ccinsight cypher ... | head -1`)
...
export async function queryCommand(
  ...
  if (!queryText?.trim()) {
    console.error('Usage: ccinsight query <search_query>');
    process.exit(1);
  }
...
export async function contextCommand(
  ...
  if (!name?.trim() && !options?.uid) {
    console.error('Usage: ccinsight context <symbol_name> [--uid <uid>] [--file <path>]');
    process.exit(1);
  }
...
export async function impactCommand(
  ...
  if (!target?.trim()) {
    console.error('Usage: ccinsight impact <symbol_name> [--direction upstream|downstream]');
    process.exit(1);
  }
  ...
  try {
    ...
    output({
      ...
      target: { name: target },
      direction: options?.direction || 'upstream',
      suggestion: 'Try reducing --depth or using ccinsight context <symbol> as a fallback',
    });
    process.exit(1);
  }
}
...
export async function cypherCommand(
  ...
  if (!query?.trim()) {
    console.error('Usage: ccinsight cypher <cypher_query>');
    process.exit(1);
  }
