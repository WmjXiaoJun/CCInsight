...
export const statusCommand = async () => {
  ...
  if (!repo) {
    ...
    if (await hasKuzuIndex(storagePath)) {
      ...
      console.log('Run: ccinsight analyze   (rebuilds the index with LadybugDB)');
    } else {
      console.log('Repository not indexed.');
      console.log('Run: ccinsight analyze');
    }
    return;
  }
  ...
  console.log(`Status: ${isUpToDate ? '�?up-to-date' : '⚠️ stale (re-run ccinsight analyze)'}`);
};
