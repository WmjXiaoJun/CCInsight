 ...
 * - Graceful failure: any error â†?return empty string
 */
...
async function findRepoForCwd(cwd: string): Promise<{
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // (e.g. /projects/ccinsightv2 should NOT match /projects/ccinsight)
      let matched = false;
...
export async function augment(pattern: string, cwd?: string): Promise<string> {
  ...
  try {
    ...
    const lines: string[] = [`[ccinsight] ${enriched.length} related symbols found:`, ''];
    ...
    // Graceful failure â€?never break the original tool
    return '';
  }
}
