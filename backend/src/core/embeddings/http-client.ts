 ...
 * Returns null when ccinsight_EMBEDDING_URL + ccinsight_EMBEDDING_MODEL are unset.
 * Not cached â€?env vars are read fresh so late configuration takes effect.
 */
const readConfig = (): HttpConfig | null => {
  const baseUrl = process.env.ccinsight_EMBEDDING_URL;
  const model = process.env.ccinsight_EMBEDDING_MODEL;
  if (!baseUrl || !model) return null;

  const rawDims = process.env.ccinsight_EMBEDDING_DIMS;
  let dimensions: number | undefined;
  if (rawDims !== undefined) {
    const parsed = parseInt(rawDims, 10);
    if (Number.isNaN(parsed) || parsed <= 0) {
      throw new Error(`ccinsight_EMBEDDING_DIMS must be a positive integer, got "${rawDims}"`);
    }
    dimensions = parsed;
  }
  ...
  return {
    ...
    model,
    apiKey: process.env.ccinsight_EMBEDDING_API_KEY ?? 'unused',
    dimensions,
  };
};
...
const httpEmbedBatch = async (
  ...
  try {
    ...
    // Timeouts should not be retried â€?the server is unresponsive.
    ...
    // DNS, connection errors â€?retry with backoff
    ...
    for (const item of items) {
      ...
      if (vec.length !== expected) {
        const hint = config.dimensions
          ? 'Update ccinsight_EMBEDDING_DIMS to match your model output.'
          : `Set ccinsight_EMBEDDING_DIMS=${vec.length} to match your model output.`;
        throw new Error(
...
export const httpEmbedQuery = async (text: string): Promise<number[]> => {
  ...
  // Same dimension checks as httpEmbed â€?catch mismatches before they
  // reach the Kuzu FLOAT[N] cast in search queries.
  const expected = config.dimensions ?? DEFAULT_DIMS;
  if (embedding.length !== expected) {
    const hint = config.dimensions
      ? 'Update ccinsight_EMBEDDING_DIMS to match your model output.'
      : `Set ccinsight_EMBEDDING_DIMS=${embedding.length} to match your model output.`;
    throw new Error(
