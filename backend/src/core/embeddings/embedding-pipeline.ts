...
const createVectorIndex = async (
  ...
  if (!vectorExtensionLoaded) {
    try {
      ...
      // Extension may already be loaded â€?CREATE_VECTOR_INDEX will fail clearly if not
      vectorExtensionLoaded = true;
    }
  }
...
export const runEmbeddingPipeline = async (
  ...
  try {
    ...
    if (isDev) {
      console.log('ðŸ” Querying embeddable nodes...');
    }
    ...
    if (skipNodeIds && skipNodeIds.size > 0) {
      ...
      if (isDev) {
        console.log(
          `ðŸ“¦ Incremental embeddings: ${beforeCount} total, ${skipNodeIds.size} cached, ${nodes.length} to embed`,
        );
      }
    }
    ...
    if (isDev) {
      console.log(`ðŸ“Š Found ${totalNodes} embeddable nodes`);
    }
    ...
    if (isDev) {
      console.log('âœ?Embedding pipeline complete!');
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';

    if (isDev) {
      console.error('â?Embedding pipeline error:', error);
    }
