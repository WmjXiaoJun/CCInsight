 ...
 * npm may install two copies â€?a top-level 1.24.x (our dep) and a nested
 ...
 *
 * Both conditions must be true â€?system CUDA libs alone are not enough
...
function isCudaAvailable(): boolean {
  ...
  // Primary: query the dynamic linker cache â€?covers all architectures,
...
export const initEmbedder = async (
  ...
  // Probe for CUDA first â€?ONNX Runtime crashes (uncatchable native error)
 ...
 * In HTTP mode, uses ccinsight_EMBEDDING_DIMS if set, otherwise the default.
 */
