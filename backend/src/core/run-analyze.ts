...
import type { GraphNode, GraphRelationship } from 'ccinsight-shared';

export interface AnalyzeCallbacks {
  onProgress: (phase: string, percent: number, message: string) => void;
  onLog?: (message: string) => void;
  /** Called periodically with batches of graph nodes/relationships for real-time display */
  onNodes?: (nodes: GraphNode[], relationships: GraphRelationship[]) => void;
}
...
export interface AnalyzeOptions {
  ...
  embeddings?: boolean;
  skipGit?: boolean;
  /** Skip AGENTS.md and CLAUDE.md ccinsight block updates. */
...
export interface AnalyzeResult {
  ...
  /** The raw pipeline result ù?only populated when needed by callers (e.g. skill generation). */
  pipelineResult?: any;
}
...
/**
 * Run the full ccinsight analysis pipeline.
 *
 ...
 * the {@link AnalyzeCallbacks} interface ù?it never writes to stdout/stderr
...
export async function runFullAnalysis(
  ...
  if (kuzuResult.found && kuzuResult.needsReindex) {
    log('Migrating from KuzuDB to LadybugDB ù?rebuilding index...');
  }
  ...
  if (existingMeta && !options.force && existingMeta.lastCommit === currentCommit) {
    // Non-git folders have currentCommit = '' ù?always rebuild since we can't detect changes
    if (currentCommit !== '') {
      return {
  ...
  // Stream graph nodes to frontend for real-time display
  if (callbacks.onNodes) {
    const BATCH_SIZE = 500;
    const nodeArr = pipelineResult.graph.nodes;
    const relArr = pipelineResult.graph.relationships;

    for (let i = 0; i < nodeArr.length; i += BATCH_SIZE) {
      const batch = nodeArr.slice(i, i + BATCH_SIZE);
      callbacks.onNodes(batch, []);
    }
    // Also stream relationships in batches
    for (let i = 0; i < relArr.length; i += BATCH_SIZE) {
      const batch = relArr.slice(i, i + BATCH_SIZE);
      callbacks.onNodes([], batch);
    }
  }

  // ?? Phase 2: LadybugDB (60ù?5%) ??????????????????????????????????
  ...
  try {
    ...
    // is called even if an error occurs ù?the module-level singleton DB handle
    ...
    try {
      ...
      // Non-fatal ù?FTS is best-effort
    }
    ...
    if (cachedEmbeddings.length > 0) {
      ...
      if (cachedDims !== EMBEDDING_DIMS) {
        // Dimensions changed (e.g. switched embedding model) ù?discard cache and re-embed all
        log(
    ...
    try {
      ...
      // Best-effort ù?don't fail the entire analysis for context file issues
    }
