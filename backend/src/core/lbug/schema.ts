...
// Import from shared package (single source of truth) â€?used in DDL templates below
import { NODE_TABLES, REL_TABLE_NAME, REL_TYPES, EMBEDDING_TABLE_NAME } from 'ccinsight-shared';
...
export type { NodeTableName, RelType } from 'ccinsight-shared';
...
const _rawDims = parseInt(process.env.ccinsight_EMBEDDING_DIMS ?? '384', 10);
if (Number.isNaN(_rawDims) || _rawDims <= 0) {
  throw new Error(
    `ccinsight_EMBEDDING_DIMS must be a positive integer, got "${process.env.ccinsight_EMBEDDING_DIMS}"`,
  );
}
