 ...
 * an exclusive lock on the LadybugDB file (e.g. `ccinsight analyze` or
 * `ccinsight serve` running at the same time).
 */
 ...
 * database is busy (e.g. `ccinsight analyze` holds the write lock).
...
export const withLbugDb = async <T>(dbPath: string, operation: () => Promise<T>): Promise<T> => {
  ...
  for (let attempt = 1; attempt <= DB_LOCK_RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      // Sleep outside the lock â€?no need to block others while waiting
      await new Promise((resolve) => setTimeout(resolve, DB_LOCK_RETRY_DELAY_MS * attempt));
    }
  }
  // This line is unreachable â€?the loop either returns or throws inside,
...
const doInitLbug = async (dbPath: string) => {
  // Different database requested â€?close the old one first
  if (conn || db) {
    try {
      if (conn) await conn.close();
    } catch {}
  ...
  try {
    ...
    if (stat.isSymbolicLink()) {
      // Never follow symlinks â€?just remove the link itself
  ...
  for (const schemaQuery of SCHEMA_QUERIES) {
    try {
      ...
      if (!msg.includes('already exists')) {
        console.warn(`âš ï¸ Schema creation warning: ${msg.slice(0, 120)}`);
      }
    }
  }
...
export const loadGraphToLbug = async (
  ...
  // Bulk COPY all node CSVs (sequential â€?LadybugDB allows only one write txn at a time)
  ...
  // Bulk COPY relationships â€?split by FROMâ†’TO label pair (LadybugDB requires it)
...
export const insertNodeToLbug = async (
  ...
  try {
    ...
    if (label === 'File') {
      ...
      // Multi-language tables (Struct, Impl, Trait, Macro, etc.) â€?no isExported
      const descPart = properties.description
        ? `, description: ${escapeValue(properties.description)}`
        : '';
 ...
 * Safe to call multiple times â€?tracks loaded state via module-level ftsLoaded.
 */
export const loadFTSExtension = async (): Promise<void> => {
  ...
  try {
    ...
    if (
      ...
      console.error('ccinsight: FTS extension load failed:', msg);
    }
  }
};
