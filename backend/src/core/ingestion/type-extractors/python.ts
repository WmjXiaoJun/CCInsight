...
const extractDeclaration: TypeBindingExtractor = (
  ...
  // stripNullable can process it at lookup time (e.g., "User | None" â†?"User").
  const inner = typeNode.type === 'type' ? (typeNode.firstNamedChild ?? typeNode) : typeNode;
  const typeName = extractSimpleTypeName(inner) ?? inner.text;
  if (varName && typeName) env.set(varName, typeName);
};
...
/** Python: user = User("alice") â€?infer type from call when callee is a known class.
...
const extractInitializer: InitializerExtractor = (
  ...
  if (node.type === 'named_expression') {
    ...
    // Skip if already has type annotation â€?extractDeclaration handled it
  ...
  // tree-sitter-python: direct â†?identifier, qualified â†?attribute
...
/** Python: user = User("alice") â€?scan assignment/walrus for constructor-like calls.
 ...
 * Handles:
 *   subscript "List[User]"  â†? extractElementTypeFromString("List[User]") â†?"User"
 *   generic_type            â†? extractGenericTypeArgs â†?first arg
...
const extractPyElementTypeFromAnnotation = (
  ...
  // Python subscript: List[User], Sequence[User] â€?use raw text
  if (inner.type === 'subscript') {
    return extractElementTypeFromString(inner.text, pos);
  }
  // generic_type: dict[str, User] â€?tree-sitter-python uses type_parameter child
 ...
 * `typed_parameter` may not expose the name as a `name` field â€?falls back to
 ...
 *   1. declarationTypeNodes â€?raw type annotation AST node (covers stored container types)
 *   2. scopeEnv string â€?extractElementTypeFromString on the stored type
 *   3. AST walk â€?walks up to the enclosing function's parameters to read List[User] directly
 *
 * Also handles `enumerate(iterable)` â€?unwraps the outer call and skips the integer
...
const extractForLoopBinding: ForLoopExtractor = (
  ...
  // Extract iterable info from the `right` field â€?may be identifier, attribute, or call.
  if (rightNode?.type === 'identifier') {
    ...
    if (fn?.type === 'identifier' && fn.text === 'enumerate') {
      // enumerate(iterable) or enumerate(d.items()) â€?unwrap to inner iterable.
      isEnumerate = true;
      ...
      // data.items() â†?call > function: attribute > identifier('data') + identifier('items')
      const extracted = extractMethodCall(rightNode);
      if (extracted) ({ iterableName, methodName } = extracted);
    } else if (fn?.type === 'identifier') {
      // Direct function call: for user in get_users() (Phase 7.3 â€?return-type path)
  ...
  // The loop variable is the `left` field â€?identifier or pattern_list.
  const leftNode = node.childForFieldName('left');
  if (!leftNode) return;

  if (leftNode.type === 'pattern_list' || leftNode.type === 'tuple_pattern') {
    // Tuple unpacking: `key, value` or `i, (k, v)` or `(k, v)` â€?bind the last identifier to element type.
...
/** Python: alias = u â†?assignment with left/right fields.
 *  Also handles walrus operator: alias := u â†?named_expression with name/value fields. */
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  ...
  // attribute RHS â†?fieldAccess (a.field)
  ...
  if (right.type === 'call') {
    ...
    // method call with receiver: call â†?function: attribute
 ...
 *   as_pattern
 *     alias: as_pattern_target   â†?the bound variable name (e.g. "u")
 *     children[0]: case_pattern  â†?wraps class_pattern (or is class_pattern directly)
 *       class_pattern
 *         dotted_name            â†?the class name (e.g. "User")
 *
...
const extractPatternBinding: PatternBindingExtractor = (node, scopeEnv) => {
  ...
  // as_pattern: `case User() as u:` â€?binds matched value to a name.
  ...
  // Find the class_pattern â€?may be direct or wrapped in case_pattern.
