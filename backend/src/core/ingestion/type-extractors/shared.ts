 ...
 *  - 'first': key type (e.g., K from Map<K,V>) â€?used for .keys(), .keySet()
 *  - 'last':  value type (e.g., V from Map<K,V>) â€?used for .values(), .items(), .iter() */
export type TypeArgPosition = 'first' | 'last';

// ---------------------------------------------------------------------------
// Container type descriptors â€?maps container base names to type parameter
 ...
 *
 *  Resolution order:
 *  1. If container is known and method is in keyMethods â†?'first'
 *  2. If container is known with arity 1 â†?'last' (same as 'first' for single-arg)
 *  3. If container is unknown â†?fall back to method name heuristic
 ...
 *
 * Strategy 1: declarationTypeNodes â€?raw AST type annotation node (handles container types
 *             where extractSimpleTypeName returned undefined, e.g., User[], List[User])
 * Strategy 2: scopeEnv string â€?extractElementTypeFromString on the stored type string
 * Strategy 3: AST walk â€?language-specific upward walk to enclosing function parameters
 *
...
export function resolveIterableElementType(
  ...
  // Strategy 2: scopeEnv string â†?extractElementTypeFromString
 ...
 *  for receiver resolution. Optional<User> â†?"User", Option<User> â†?"User".
 *  Only nullable wrappers â€?NOT containers (List, Vec) or async wrappers (Promise, Future).
 ...
 * Handles generic types (e.g., List<User> â†?List), qualified names
 * (e.g., models.User â†?User), and nullable types (e.g., User? â†?User).
...
export const extractSimpleTypeName = (typeNode: SyntaxNode, depth = 0): string | undefined => {
  ...
  // Qualified/scoped names: take the last segment (e.g., models.User â†?User, Models::User â†?User)
  if (
  ...
  // Generic types: extract the base type (e.g., List<User> â†?List)
  ...
  if (
    ...
    // Unwrap known nullable wrappers: Optional<User> â†?User, Option<User> â†?User
  ...
  if (typeNode.type === 'pointer_type' || typeNode.type === 'reference_type') {
    // Skip mutable_specifier for Rust &mut references â€?firstNamedChild would be
...
export const extractVarName = (node: SyntaxNode): string | undefined => {
  ...
  // Rust: let mut x = ... â€?mut_pattern wraps an identifier
...
export const TYPED_PARAMETER_TYPES = new Set([
  ...
  'typed_parameter', // Python: def f(x: Foo) â€?distinct from 'parameter' in tree-sitter-python
  ...
  'closure_parameter', // Rust: |user: User| â€?typed closure parameters
]);

/**
 * Extract type arguments from a generic type node.
 * e.g., List<User, String> â†?['User', 'String'], Vec<User> â†?['User']
 *
 * Used by extractSimpleTypeName to unwrap nullable wrappers (Optional<User> â†?User).
 *
 ...
 *
 * Note: Go slices/maps use slice_type/map_type, not generic_type â€?those are
 * NOT handled here. Use language-specific extractors for Go container types.
 *
 * @param typeNode A generic_type or parameterized_type AST node (or any node â€? *   returns [] for non-generic types).
...
export const extractRubyConstructorAssignment = (
  ...
  if (receiver.type === 'constant') {
    ...
    // Models::User â†?extract last segment "User"
 ...
 *
 *   "User | null"           â†?"User"
 *   "User | undefined"      â†?"User"
 *   "User | null | undefined" â†?"User"
 *   "User?"                 â†?"User"
 *   "User | Repo"           â†?undefined  (genuine union â€?refuse)
 *   "null"                  â†?undefined
 */
export const stripNullable = (typeName: string): string | undefined => {
  ...
  // Strip nullable suffix: User? â†?User
  ...
  if (text.includes('|')) {
    ...
    return undefined; // genuine union or all-nullable â€?refuse
  }
 ...
 *
 * Handles:
 * - Array<User>    â†?User  (generic angle brackets)
 * - User[]         â†?User  (array suffix)
 * - []User         â†?User  (Go slice prefix)
 * - List[User]     â†?User  (Python subscript)
 * - [User]         â†?User  (Swift array sugar)
 * - vector<User>   â†?User  (C++ container)
 * - Vec<User>      â†?User  (Rust container)
 *
 * For multi-argument generics (Map<K, V>), returns the first or last type arg
 * based on `pos` ('first' for keys, 'last' for values â€?default 'last').
...
export function extractElementTypeFromString(
  ...
  for (let i = start; i < typeStr.length; i++) {
    ...
    if (ch === '<' || ch === '[') {
      ...
      if (depth === 0) {
        // At depth 0 â€?only match if it is our selected close bracket.
...
// â”€â”€ Return type text helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 *   "User"                â†?"User"
 *   "Promise<User>"       â†?"User"   (unwrap wrapper generics)
 *   "Option<User>"        â†?"User"
 *   "Result<User, Error>" â†?"User"   (first type arg)
 *   "User | null"         â†?"User"   (strip nullable union)
 *   "User?"               â†?"User"   (strip nullable suffix)
 *   "*User"               â†?"User"   (Go pointer)
 *   "&User"               â†?"User"   (Rust reference)
...
const WRAPPER_GENERICS = new Set([
  ...
  'Cow', // copy-on-write
  // Containers (List, Array, Vec, Set, etc.) are intentionally excluded â€?  // methods are called on the container, not the element type.
 ...
 *   "Result<User, Error>"  â†?"Result<User, Error>"  (no top-level comma)
 *   "User, Error"          â†?"User"
 *   "Map<K, V>, string"    â†?"Map<K, V>"
 */
 ...
 *   "'_, User"       â†?"User"
 *   "'a, User"       â†?"User"
 *   "User, Error"    â†?"User"  (no lifetime â€?delegates to extractFirstGenericArg)
 */
function extractFirstTypeArg(args: string): string {
  ...
  while (remaining) {
    ...
    if (commaIdx < 0) return first; // only lifetimes â€?fall through
...
export const extractReturnTypeName = (raw: string, depth = 0): string | undefined => {
  ...
  // Handle union types: "User | null" â†?"User"
  if (text.includes('|')) {
    const parts = text
      .split('|')
      .map((p) => p.trim())
    ...
    else return undefined; // genuine union â€?too complex
  }

  // Handle generics: Promise<User> â†?unwrap if wrapper, else take base
  ...
  if (genericMatch) {
    ...
    // Non-wrapper generic: return the base type (e.g., Map<K,V> â†?Map)
  ...
  // should not produce a binding â€?these are meaningless without a type parameter
  if (WRAPPER_GENERICS.has(text)) return undefined;

  // Handle qualified names: models.User â†?User, Models::User â†?User, \App\Models\User â†?User
...
// extractPropertyDeclaredType removed â€?all 14 languages register a FieldExtractor
