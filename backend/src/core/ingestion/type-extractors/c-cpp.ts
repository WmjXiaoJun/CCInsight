...
const extractInitializer: InitializerExtractor = (
  ...
  // Only handle auto/placeholder â€?typed declarations are handled by extractDeclaration
  ...
  // auto x = new User() â€?new_expression
  ...
  // auto x = User() â€?call_expression where function is a type name
  ...
  if (value.type === 'call_expression') {
    ...
    if (func.type === 'type_identifier') {
      ...
      // auto x = std::make_shared<Dog>() â€?smart pointer factory via template_function.
  ...
  // auto x = User{} â€?compound_literal_expression (brace initialization)
...
/** C/C++: parameter_declaration â†?type declarator */
...
/** C++: auto alias = user â†?declaration with auto type + init_declarator where value is identifier */
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  if (node.type !== 'declaration') return undefined;
  const typeNode = node.childForFieldName('type');
  if (!typeNode) return undefined;
  // Only handle auto â€?typed declarations already resolved by extractDeclaration
  const typeText = typeNode.text;
  if (
    typeText !== 'auto' &&
    typeText !== 'decltype(auto)' &&
  ...
  if (value.type === 'call_expression') {
    ...
    // method call with receiver: call_expression â†?function: field_expression
...
const extractCppElementTypeFromTypeNode = (
  ...
  // template_type: vector<User>, map<string, User> â€?extract type arg based on position
  ...
  // reference/pointer types: unwrap and recurse (vector<User>& â†?vector<User>)
  if (
  ...
  // qualified/scoped types: std::vector<User> â†?unwrap to template_type child
    ...
    if (current.type === 'function_definition') {
      ...
      if (paramsNode) {
        for (let i = 0; i < paramsNode.namedChildCount; i++) {
          ...
          // Unwrap reference/pointer declarators: vector<User>& users â†?&users
...
/** C++: for (auto& user : users) â€?extract loop variable binding.
...
const extractForLoopBinding: ForLoopExtractor = (
  ...
  // Check if the type is auto/placeholder â€?if not, use the explicit type directly
  const isAuto =
    typeNode.type === 'placeholder_type_specifier' ||
    typeNode.text === 'auto' ||
  ...
  if (!isAuto) {
    // Explicit type: for (User& user : users) â€?extract directly
    const typeName = extractSimpleTypeName(typeNode);
    if (typeName) scopeEnv.set(varName, typeName);
    return;
  }

  // auto/const auto/auto& â€?resolve from the iterable's container type
  ...
  if (rightNode.type === 'identifier') {
    ...
    // Dereference: for (auto& user : *ptr) â†?pointer_expression > identifier
 ...
 *  E.g., shared_ptr<Animal> â†?Animal. Returns the original name if not a smart pointer. */
