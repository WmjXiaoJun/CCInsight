 ...
 * and extract variable â†?type bindings from them.
 */
const extractCapturedPatternBindings = (
  ...
  if (pattern.type === 'captured_pattern') {
    ...
    // Find the struct_pattern child â€?that gives us the type
...
const extractDeclaration: TypeBindingExtractor = (
  ...
  if (node.type === 'let_condition') {
    ...
    //
    // Supported patterns:
    // - captured_pattern: `if let user @ User { .. } = expr` â†?user: User
    // - tuple_struct_pattern with nested captured_pattern:
    //   `if let Some(user @ User { .. }) = expr` â†?user: User
    //
    // NOT supported (requires generic unwrapping â€?Phase 3):
    // - `if let Some(x) = opt` where opt: Option<T> â†?x: T
    //
    // struct_pattern without capture (`if let User { name } = expr`)
    // destructures fields â€?individual field types are unknown without
...
const extractInitializer: InitializerExtractor = (
  ...
  // Skip if there's an explicit type annotation â€?Tier 0 already handled it
  ...
  // Only match ::new() and ::default() â€?the two idiomatic Rust constructors.
...
/** Rust: let user = get_user("alice") â€?let_declaration with call_expression value, no type annotation.
 * Skips `let user: User = ...` (explicit type annotation â€?handled by extractDeclaration).
 * Skips `let user = User::new()` (scoped_identifier callee named "new" â€?handled by extractInitializer).
...
const scanConstructorBinding: ConstructorBindingScanner = (node) => {
  ...
  // Unwrap `.await`: `let user = get_user().await` â†?await_expression wraps call_expression
...
/** Rust: let alias = u; â†?let_declaration with pattern + value fields.
 *  Also handles struct destructuring: `let Point { x, y } = p` â†?N fieldAccess items. */
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  ...
  if (pattern.type === 'struct_pattern' && value.type === 'identifier') {
    ...
    for (let j = 0; j < pattern.namedChildCount; j++) {
      ...
      if (field.type === 'field_pattern') {
        // `Point { x: local_x }` â†?field_pattern with name + pattern children
        ...
        if (nameNode && patNode) {
          ...
          // Shorthand: `Point { x }` â†?field_pattern with only name (varName = fieldName)
  ...
  // call_expression RHS â†?callResult (simple calls only)
  ...
  // method_call_expression RHS â†?methodCallResult (receiver.method())
 ...
 *
 * Supported patterns:
 * - `if let Some(x) = opt`  â†?x: T  (opt: Option<T>, T already in scopeEnv via NULLABLE_WRAPPER_TYPES)
 * - `if let Ok(x) = res`    â†?x: T  (res: Result<T, E>, T extracted from declarationTypeNodes)
 *
...
const extractPatternBinding: PatternBindingExtractor = (
  ...
  if (node.type === 'let_condition') {
    ...
    // match_arm â†?pattern field is match_pattern wrapping the actual pattern
    ...
    const matchExpr = node.parent?.parent; // match_arm â†?match_block â†?match_expression
  ...
  // `Some(x)` â†?the first named child after the type field is the identifier.
  ...
  // For `Ok(x)`: Result<T, E> stores "Result" in scopeEnv â€?must use declarationTypeNodes.
  if (wrapperName === 'Some') {
  ...
  // Ok(x) â†?extract T (typeArgs[0]), Err(e) â†?extract E (typeArgs[1]).
 ...
 *  slice_type ([User]). For call-graph purposes, strips references (&User â†?User). */
const extractRustElementTypeFromTypeNode = (
  ...
  // generic_type: Vec<User>, HashMap<K, V> â€?extract type arg based on position
  if (typeNode.type === 'generic_type') {
    const args = extractGenericTypeArgs(typeNode);
    if (args.length >= 1) return pos === 'first' ? args[0] : args[args.length - 1];
  }
  // reference_type: &[User] or &Vec<User> â€?unwrap the reference and recurse
...
const extractForLoopBinding: ForLoopExtractor = (
  ...
  // Extract iterable name + method â€?may be &users, users, or users.iter()/keys()/values()
  ...
  if (valueNode.type === 'reference_expression') {
    ...
    if (funcExpr?.type === 'field_expression') {
      // users.iter() â†?field_expression > identifier + field_identifier
