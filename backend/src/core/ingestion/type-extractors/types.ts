 ...
 *  Narrower than ReadonlySet â€?only `.has()` is used by extractors. */
 ...
 *  `receiverClassName` â€?optional hint for method calls on known receivers
 ...
 *  with parameterTypes â€?~1-3% of call sites. */
 ...
 *  E.g., C++ shared_ptr<Animal> â†?Animal. Returns undefined if no unwrapping applies. */
export type DeclaredTypeUnwrapper = (
  declaredType: string,
  typeNode: SyntaxNode,
) => string | undefined;

/** Narrow lookup interface for resolving a callee name â†?return type name.
...
export interface ForLoopExtractorContext {
  /** Mutable type-env for the current scope â€?extractor writes bindings here */
 ...
 *  - `copy`             â€?`const b = a` (identifier alias, propagate a's type to b)
 *  - `callResult`       â€?`const b = foo()` (bind b to foo's declared return type)
 *  - `fieldAccess`      â€?`const b = a.field` (bind b to field's declaredType on a's type)
 *  - `methodCallResult` â€?`const b = a.method()` (bind b to method's returnType on a's type) */
...
export interface LanguageTypeConfig {
  ...
  /** Extract a (varName â†?typeName) binding from a declaration node */
  extractDeclaration: TypeBindingExtractor;
  /** Extract a (varName â†?typeName) binding from a parameter node */
  extractParameter: ParameterExtractor;
  /** Extract a (varName â†?typeName) binding from a constructor-call initializer.
   ...
   *  Receives classNames â€?the set of class/struct names visible in the current file's AST. */
   ...
   *  The callee binding is unverified â€?the caller must confirm against the SymbolTable. */
  ...
  /** Extract loop variable â†?type binding from a for-each AST node. */
