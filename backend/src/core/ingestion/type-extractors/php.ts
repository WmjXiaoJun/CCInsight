 ...
 * - self/static â†?enclosing class name
 * - parent â†?superclass from base_clause
 */
...
const normalizePhpType = (raw: string): string | undefined => {
  // Strip nullable prefix: ?User â†?User
  let type = raw.startsWith('?') ? raw.slice(1) : raw;
  // Strip array suffix: User[] â†?User
  type = type.replace(/\[\]$/, '');
  // Strip union with null/false/void: User|null â†?User
  const parts = type
    .split('|')
    .filter((p) => p !== 'null' && p !== 'false' && p !== 'void' && p !== 'mixed');
  if (parts.length !== 1) return undefined;
  type = parts[0];
  // Strip namespace: \App\Models\User â†?User
  ...
  if (
    ...
    return undefined;
  // Extract element type from generic: Collection<User> â†?User
  ...
  // not the container name. This mirrors how User[] â†?User is handled by the [] strip above.
 ...
 *
 * Returns the normalized element type (e.g. User[] â†?User, Collection<User> â†?User).
...
const extractClassPropertyElementType = (propDecl: SyntaxNode): string | undefined => {
  ...
  // Strategy 2: PHP 7.4+ native type field â€?skip generic 'array' since element type is unknown
 ...
 * Returns a map of paramName â†?typeName (without $ prefix).
 */
...
const extractDeclaration: TypeBindingExtractor = (
  ...
  // PHPDoc @param on methods/functions â€?pre-populate env with param types
...
/** PHP: $x = new User() â€?infer type from object_creation_expression */
...
/** PHP: simple_parameter â†?type $name */
const extractParameter: ParameterExtractor = (node: SyntaxNode, env: Map<string, string>): void => {
  ...
  if (!varName) return;
  // Don't overwrite PHPDoc-derived types (e.g. @param User[] $users â†?User)
...
/** PHP: $x = SomeFactory() or $x = $this->getUser() â€?bind variable to call return type */
const scanConstructorBinding: ConstructorBindingScanner = (node) => {
  ...
  // Skip object_creation_expression (new User()) â€?handled by extractInitializer
 ...
 * Unlike normalizePhpType (which strips User[] â†?User for scopeEnv), this preserves
 * array notation so lookupRawReturnType can extract element types for for-loop resolution.
 *   \App\Models\User[] â†?User[]
 *   ?User â†?User
 *   Collection<User> â†?Collection<User>  (preserved for extractElementTypeFromString)
 */
...
/** PHP: $alias = $user â†?assignment_expression with variable_name left/right.
 *  PHP TypeEnv stores variables WITH $ prefix ($user â†?User), so we keep $ in lhs/rhs. */
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  ...
  // member_access_expression RHS â†?fieldAccess ($a->field)
  ...
  // function_call_expression RHS â†?callResult (bare function calls only)
  if (right.type === 'function_call_expression') {
  ...
  // member_call_expression RHS â†?methodCallResult ($a->method())
  if (right.type === 'member_call_expression') {
    const obj = right.childForFieldName('object');
    const name = right.childForFieldName('name');
    if (obj?.type === 'variable_name' && name) {
 ...
 *  PHP has limited AST-level container types â€?`array` is a primitive_type with no generic args.
 ...
 *  PHP parameter type hints are limited (array, ClassName) â€?this extracts element type when possible. */
...
/**
 * PHP: foreach ($users as $user) â€?extract loop variable binding.
 *
 * AST structure (from tree-sitter-php grammar):
 *   foreach_statement â€?no named fields for iterable/value (only 'body')
 ...
 *
 * PHP's PHPDoc @param normalizes `User[]` â†?`User` in the env, so the iterable's
...
const extractForLoopBinding: ForLoopExtractor = (
  ...
  if (valueOrPair.type === 'pair') {
    // $key => $value â€?the value is the last named child of the pair
  ...
  if (iterableNode.type === 'variable_name') {
    ...
    // foreach (getUsers() as $user) â€?resolve via return type lookup
    ...
    // foreach ($this->getUsers() as $user) â€?resolve via return type lookup
  ...
  // Strategy B: direct scopeEnv lookup â€?PHP normalizePhpType strips User[] â†?User,
  ...
  // Strategy C: $this->property â€?scan the enclosing class body for the property
