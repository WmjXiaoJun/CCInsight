...
const normalizeJsDocType = (raw: string): string | undefined => {
  let type = raw.trim();
  // Strip JSDoc nullable/non-nullable prefixes: ?User â†?User, !User â†?User
  if (type.startsWith('?') || type.startsWith('!')) type = type.slice(1);
  // Strip union with null/undefined/void: User|null â†?User
  const parts = type
    .split('|')
    .map((p) => p.trim())
  ...
  type = parts[0];
  // Strip module: prefix â€?module:models.User â†?models.User
  if (type.startsWith('module:')) type = type.slice(7);
  // Take last segment of dotted path: models.User â†?User
  const segments = type.split('.');
  type = segments[segments.length - 1];
  // Strip generic wrapper: Promise<User> â†?Promise (base type, not inner)
 ...
 * Returns a map of paramName â†?typeName.
 */
...
const extractDeclaration: TypeBindingExtractor = (
  ...
  // JSDoc @param on functions/methods â€?pre-populate env with param types
  ...
  // Class field: `private users: User[]` â€?public_field_definition has name + type fields directly.
...
/** TypeScript: required_parameter / optional_parameter â†?name: type */
...
/** TypeScript: const x = new User() â€?infer type from new_expression */
const extractInitializer: InitializerExtractor = (
  ...
  for (let i = 0; i < node.namedChildCount; i++) {
    const declarator = node.namedChild(i);
    if (declarator?.type !== 'variable_declarator') continue;
    // Only activate when there is no explicit type annotation â€?extractDeclaration already
...
/**
 * TypeScript/JavaScript: const user = getUser() â€?variable_declarator with call_expression value.
 * Only matches unannotated declarators; annotated ones are handled by extractDeclaration.
 * await is unwrapped: const user = await fetchUser() â†?callee = 'fetchUser'.
 */
...
/**
 * Minimal sanitization for JSDoc return types â€?preserves generic wrappers
 ...
 * Handles:
 *   type_annotation ": User[]"  â†? array_type â†?type_identifier "User"
 *   type_annotation ": Array<User>"  â†? generic_type â†?extractGenericTypeArgs â†?"User"
...
const extractTsElementTypeFromAnnotation = (
  ...
  // readonly User[] â€?readonly_type wraps array_type: unwrap and recurse
 ...
 *   1. declarationTypeNodes â€?raw type annotation AST node (covers Array<User> from declarations)
 *   2. scopeEnv string â€?extractElementTypeFromString on the stored type (covers locally annotated vars)
 *   3. AST walk â€?walks up to the enclosing function's parameters to read User[] annotations directly
...
const extractForLoopBinding: ForLoopExtractor = (
  ...
  // The iterable is the `right` field â€?may be identifier, member_expression, or call_expression.
  ...
  if (rightNode?.type === 'identifier') {
    ...
    // entries.values() â†?call_expression > function: member_expression > object + property
    // this.repos.values() â†?nested member_expression: extract property from inner member
    // getUsers() â†?call_expression > function: identifier (Phase 7.3 â€?return-type path)
    ...
    if (fn?.type === 'member_expression') {
      ...
      if (obj?.type === 'identifier') {
        ...
        // this.repos.values() â†?obj = this.repos â†?extract 'repos'
  ...
  // `const user` parses as: left â†?variable_declarator containing an identifier named `user`
...
/** TS/JS: const alias = u â†?variable_declarator with name/value fields.
 *  Also handles destructuring: `const { a, b } = obj` â†?N fieldAccess items. */
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    // Emits N fieldAccess items â€?one per destructured binding.
    ...
    // member_expression RHS â†?fieldAccess (a.field, this.field)
    ...
    // Simple call â†?callResult: getUser()
    if (funcNode.type === 'identifier') {
      return { kind: 'callResult', lhs, callee: funcNode.text };
    }
    // Method call with receiver â†?methodCallResult: a.getC()
...
const findIfConsequenceBlock = (binaryExpr: SyntaxNode): SyntaxNode | undefined => {
  ...
  while (current) {
    ...
    // Stop climbing at function/block boundaries â€?don't cross scope
    if (
...
/** TS instanceof narrowing: `x instanceof User` â†?bind x to User.
