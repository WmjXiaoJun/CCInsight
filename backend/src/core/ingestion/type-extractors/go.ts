...
/** Go: x := Foo{...} â€?infer type from composite literal (handles multi-assignment) */
const extractGoShortVarDeclaration = (node: SyntaxNode, env: Map<string, string>): void => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Unwrap &User{} â€?unary_expression (address-of) wrapping composite_literal
    if (
    ...
    // Go built-in new(User) â€?call_expression with 'new' callee and type argument
    // Go built-in make([]User, 0) / make(map[string]User) â€?extract element/value type
    ...
    // Go type assertion: user := iface.(User) â€?type_assertion_expression with 'type' field
...
/** Go: user := NewUser(...) â€?infer type from single-assignment call expression */
const scanConstructorBinding: ConstructorBindingScanner = (node) => {
  ...
  // Multi-return: user, err := NewUser() â€?bind first var when second is err/ok/_
  ...
  const func = rightExprs[0].childForFieldName('function');
  if (!func) return undefined;
  // Skip new() and make() â€?already handled by extractDeclaration
  if (func.text === 'new' || func.text === 'make') return undefined;
  const calleeName = extractSimpleTypeName(func);
  if (!calleeName) return undefined;
  return { varName: leftIds[0].text, calleeName };
 ...
 * Handles:
 *   slice_type "[]User"  â†? element field â†?type_identifier "User"
 *   array_type "[10]User" â†? element field â†?type_identifier "User"
...
const extractGoElementTypeFromTypeNode = (
  ...
  // slice_type: []User â€?element field is the element type
  ...
  // map_type: map[string]User â€?value field is the element type (for range, second var gets value)
  ...
  // channel_type: chan User â€?the type argument is the element type
  ...
  // Fallback: text-based extraction ([]User â†?User, User[] â†?User)
    ...
    if (GO_FUNCTION_NODE_TYPES.has(current.type)) {
      ...
      if (paramsNode) {
        for (let i = 0; i < paramsNode.namedChildCount; i++) {
          ...
          // parameter_declaration: name type â€?name field is the identifier
 ...
 *   1. declarationTypeNodes â€?raw type annotation AST node
 *   2. scopeEnv string â€?extractElementTypeFromString on the stored type
 *   3. AST walk â€?walks up to the enclosing function's parameters to read []User directly
...
const extractForLoopBinding: ForLoopExtractor = (
  ...
  // Find the range_clause child â€?this distinguishes range loops from other for forms.
  ...
  // Go range semantics:
  //   Slice/Array/String: single-var â†?INDEX (int); two-var â†?(index, element)
  //   Map:                single-var â†?KEY; two-var â†?(key, value)
  //   Channel:            single-var â†?ELEMENT (channels have no index)
  ...
  if (leftNode.type === 'expression_list') {
    if (leftNode.namedChildCount >= 2) {
      // Two-var form: `_, user` or `i, user` â€?second variable gets element/value type
      loopVarNode = leftNode.namedChild(1);
    } else {
      // Single-var in expression_list â€?yields INDEX for slices/maps, ELEMENT for channels.
      ...
      if (iterableName && isChannelType(iterableName, scopeEnv, declarationTypeNodes, scope)) {
        ...
        return; // index-only range on slice/map â€?skip
      }
    }
  } else {
...
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  if (node.type === 'short_var_declaration') {
    ...
    // selector_expression RHS â†?fieldAccess (a.field)
    ...
    if (rhsNode.type === 'call_expression') {
      ...
      // method call with receiver: call_expression â†?function: selector_expression
  ...
  if (node.type === 'var_spec' || node.type === 'var_declaration') {
    ...
    for (const spec of specs) {
      ...
      if (rhsNode?.type === 'identifier') return { kind: 'copy', lhs, rhs: rhsNode.text };
      // selector_expression RHS â†?fieldAccess
      if (rhsNode?.type === 'selector_expression') {
        const operand = rhsNode.childForFieldName('operand');
        const field = rhsNode.childForFieldName('field');
        if (operand?.type === 'identifier' && field) {
