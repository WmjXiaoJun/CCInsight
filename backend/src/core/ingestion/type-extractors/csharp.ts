...
const extractDeclaration: TypeBindingExtractor = (
  ...
  // Handle 'var x = new Foo()' â€?infer from object_creation_expression
...
/** C#: var x = SomeFactory(...) â†?bind x to SomeFactory (constructor-like call) */
const scanConstructorBinding: ConstructorBindingScanner = (node) => {
  ...
  // Only handle implicit_type (var) â€?explicit types handled by extractDeclaration
  ...
  // Unwrap await: `var user = await svc.GetUserAsync()` â†?await_expression wraps invocation_expression
...
const extractCSharpElementTypeFromTypeNode = (
  ...
  // nullable_type: unwrap and recurse (List<User>? â†?List<User> â†?User)
...
/** C#: foreach (User user in users) â€?extract loop variable binding.
...
const extractForLoopBinding: ForLoopExtractor = (
  ...
  // Tier 1c: implicit type (var) â€?resolve from iterable's container type
  ...
  if (rightNode?.type === 'identifier') {
    ...
    // C# property access: data.Keys, data.Values â†?member_access_expression
    // Also handles bare member access: this.users, repo.users â†?use property as iterableName
    ...
    if (propText && KNOWN_CONTAINER_PROPS.has(propText)) {
      if (obj?.type === 'identifier') {
        ...
        // Nested member access: this.data.Values â†?obj is "this.data", extract "data"
        const innerProp = obj.childForFieldName('name');
        if (innerProp) iterableName = innerProp.text;
      }
      methodName = propText;
    } else if (propText) {
      // Bare member access: this.users â†?use property name for scopeEnv lookup
      iterableName = propText;
    }
  } else if (rightNode?.type === 'invocation_expression') {
    // C# method call: data.Select(...) â†?invocation_expression > member_access_expression
    // Direct function call: GetUsers() â†?invocation_expression > identifier
 ...
 * No scopeEnv lookup is needed â€?the pattern explicitly declares the new variable's type.
 */
/**
...
const extractPatternBinding: PatternBindingExtractor = (
  ...
  // is_pattern_expression: `obj is User user` â€?has a declaration_pattern child
  ...
  if (node.type === 'is_pattern_expression') {
    ...
    // Null-check: `x is not null` â€?negated_pattern > constant_pattern > null_literal
  ...
  // Null-check: `x != null` â€?binary_expression with != operator
...
/** C#: var alias = u â†?variable_declarator with name + equals_value_clause.
 *  Only local_declaration_statement and variable_declaration contain variable_declarator children;
 *  is_pattern_expression and field_declaration never do â€?skip them early. */
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  ...
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    // await_expression â†?unwrap and check inner
