...
/** Swift: let user = User(name: "alice") â€?infer type from call when callee is a known class.
...
const extractInitializer: InitializerExtractor = (
  ...
  // Skip if has type annotation â€?extractDeclaration handled it
  ...
  // Explicit init: User.init(name: "alice") â€?navigation_expression with .init suffix
...
/** Swift: let user = User(name: "alice") â€?scan property_declaration for constructor binding */
const scanConstructorBinding: ConstructorBindingScanner = (node) => {
  ...
  if (callee.type === 'navigation_expression') {
    ...
    // General qualified call: service.getUser() â†?extract method name.
...
function extractIfGuardBinding(
  ...
  // call_expression â†?callResult or methodCallResult
 ...
 * Handles:
 *   let user = getUser()           â†?callResult
 *   let result = user.save()       â†?methodCallResult
 *   let name = user.name           â†?fieldAccess
 *   let copy = user                â†?copy
 *   let user = await getUser()     â†?callResult (unwrapped)
 *   let user = try getUser()       â†?callResult (unwrapped)
 *   if let user = getUser()        â†?callResult (optional binding)
 *   guard let user = getUser()     â†?callResult (optional binding)
 */
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  ...
  if (node.type !== 'property_declaration') return undefined;
  // Skip if type annotation exists â€?extractDeclaration handles it
  if (hasTypeAnnotation(node)) return undefined;
  ...
  for (let i = 0; i < node.namedChildCount; i++) {
    ...
    if (child.type === 'pattern' || child.type === 'simple_identifier') {
      if (!loopVarName) {
        ...
        if (!varName) return; // Non-simple pattern â€?bail out
        loopVarName = varName;
        continue;
      }
    }
...
function extractSwiftElementTypeFromTypeNode(typeNode: SyntaxNode): string | undefined {
  // Swift array sugar: [User] â€?parsed as array_type > user_type > type_identifier
