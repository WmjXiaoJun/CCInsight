...
/**
 * Ruby type extractor â€?YARD annotation parsing.
 *
 ...
 * Handles:
 * - Simple types: "String" â†?"String"
 * - Qualified types: "Models::User" â†?"User"
 * - Generic types: "Array<User>" â†?"Array"
 * - Nullable types: "String, nil" â†?"String"
 * - Union types: "String, Integer" â†?undefined (ambiguous)
 */
const extractYardTypeName = (yardType: string): string | undefined => {
  ...
  // Handle qualified: "Models::User" â†?"User"
  const segments = typePart.split('::');
  const last = segments[segments.length - 1];

  // Handle generic: "Array<User>" â†?"Array"
...
const collectYardParams = (methodNode: SyntaxNode): Map<string, string> => {
  ...
  //   class
  //     constant = "ClassName"
  //     comment = "# @param ..."     â†?sibling of body_statement
  //     comment = "# @param ..."     â†?sibling of body_statement
  //     body_statement
  //       method                     â†?method is here, no preceding siblings
  //
 ...
 * so this is a no-op â€?the bindings are already in the env.
 *
 ...
 *
 * Example: `@param users [Array<User>]` â†?extracts "User" from "Array<User>".
 */
 ...
 *   1. scopeEnv string â€?extractElementTypeFromString on the stored type
 *   2. AST walk â€?walks up to the enclosing method's YARD @param to read Array<User> directly
 *
...
/**
 * Ruby: alias_user = user â†?assignment with left/right identifier fields.
...
const extractPendingAssignment: PendingAssignmentExtractor = (node, scopeEnv) => {
  ...
  // call/method_call RHS â€?Ruby uses method calls for both field access and method calls
