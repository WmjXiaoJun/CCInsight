 ...
 * 1. Same file (lookupExactFull â€?authoritative)
 ...
 * 3. Global (all candidates â€?consumers must check candidate count)
 */
...
/** Maps callerFile â†?(moduleAlias â†?sourceFilePath) for Python namespace imports.
 *  e.g. `import models` in app.py â†?moduleAliasMap.get('app.py')?.get('models') === 'models.py' */
...
export interface ResolutionContext {
   ...
   *
   * Tier 3 ('global') returns ALL candidates regardless of count â€?   * consumers must check candidates.length and refuse ambiguous matches.
   */
  resolve(name: string, fromFile: string): TieredCandidates | null;

  // --- Data access (for pipeline wiring, not resolution) ---
  /** Symbol table â€?used by parsing-processor to populate symbols. */
  readonly symbols: SymbolTable;
  /** Raw maps â€?used by import-processor to populate import data. */
  readonly importMap: ImportMap;
  readonly packageMap: PackageMap;
  readonly namedImportMap: NamedImportMap;
  /** Module-alias map for Python namespace imports: callerFile â†?(alias â†?sourceFile). */
...
export const createResolutionContext = (): ResolutionContext => {
  ...
  const resolveUncached = (name: string, fromFile: string): TieredCandidates | null => {
    // Tier 1: Same file â€?authoritative match (returns all overloads)
    ...
    // Tier 2a: Import-scoped â€?definition in a file imported by fromFile
    ...
    // Tier 2b: Package-scoped â€?definition in a package dir imported by fromFile
    ...
    // Tier 3: Global â€?pass all candidates through.
  ...
  const clearCache = (): void => {
    cacheFile = null;
    // Reuse the Map instance â€?just clear entries to reduce GC pressure at scale.
    cache?.clear();
  };
