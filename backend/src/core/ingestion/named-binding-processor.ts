 ...
 * This function follows the chain: Aâ†’Bâ†’C until a definition is found.
 *
 ...
 *
 * @param allDefs Pre-computed `symbolTable.lookupFuzzy(name)` result â€?must be the
...
export function walkBindingChain(
  ...
  for (let depth = 0; depth < 5; depth++) {
    ...
    // No definition in source file â†?follow re-export chain
