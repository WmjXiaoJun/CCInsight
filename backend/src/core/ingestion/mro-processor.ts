 ...
 * - Rust:      no auto-resolution â€?requires qualified syntax, resolvedTo = null
 * - Default:   single inheritance â€?first definition wins
 *
 * OVERRIDES edge direction: Class â†?Method (not Method â†?Method).
...
function buildAdjacency(graph: KnowledgeGraph) {
  // parentMap: childId â†?parentIds[] (in insertion / declaration order)
  const parentMap = new Map<string, string[]>();
  // methodMap: classId â†?methodIds[]
...
/** Resolve by MRO order â€?first ancestor in linearized order wins. */
...
export function computeMRO(graph: KnowledgeGraph): MROResult {
  ...
  for (const [classId, directParents] of parentMap) {
    ...
    for (const [methodName, defs] of methodsByName) {
      ...
      // Own method shadows inherited â€?no ambiguity
