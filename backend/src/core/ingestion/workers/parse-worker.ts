...
// tree-sitter-swift is an optionalDependency â€?may not be installed
...
// tree-sitter-dart is an optionalDependency â€?may not be installed
let Dart: TreeSitterLanguage | null = null;
try {
  Dart = _require('tree-sitter-dart');
} catch {}

// tree-sitter-kotlin is an optionalDependency â€?may not be installed
...
export interface ExtractedImport {
  ...
  language: SupportedLanguages;
  /** Named bindings from the import (e.g., import {User as U} â†?[{local:'U', exported:'User'}]) */
...
export interface ExtractedCall {
   ...
   *   `svc.getUser().save()`        â†?chain=[{kind:'call',name:'getUser'}], receiverName='svc'
   *   `user.address.save()`         â†?chain=[{kind:'field',name:'address'}], receiverName='user'
   *   `svc.getUser().address.save()` â†?chain=[{kind:'call',name:'getUser'},{kind:'field',name:'address'}]
...
/** File-scope type bindings from TypeEnv fixpoint â€?used for cross-file ExportedTypeMap. */
...
// ============================================================================
// Per-file O(1) memoization â€?avoids repeated parent-chain walks per symbol.
...
// ============================================================================
// FieldExtractor cache â€?extract field metadata once per class, reuse for each property.
 ...
 * Field extraction only uses symbolTable.lookupExactAll for optional type resolution â€? * returning [] causes the extractor to use the raw type string, which is fine for us.
 */
 ...
 * Returns a nameâ†’FieldInfo map, or undefined if the provider has no field extractor
...
// ============================================================================
// MethodExtractor cache â€?extract method metadata once per class, reuse for each method.
// Keyed by class node startIndex (unique per AST node within a file).
// ============================================================================
 ...
 * Returns a "name:line" â†?MethodInfo map, or undefined if the provider has no method extractor
...
// ============================================================================
// Enclosing function detection (for call extraction) â€?cached
// ============================================================================
...
const findEnclosingFunctionId = (
  ...
  while (current) {
    if (FUNCTION_NODE_TYPES.has(current.type)) {
      ...
      if (funcName) {
        // Apply labelOverride so label matches definition phase (e.g., Kotlin Functionâ†’Method).
        // null means "skip as definition" â€?keep original label for scope identification.
...
/** Cached wrapper for findEnclosingClassId â€?avoids repeated parent walks. */
...
/** Cached wrapper for export checking â€?avoids repeated parent walks per symbol. */
    ...
    if (regularFiles.length > 0) {
      if (isLanguageAvailable(language, regularFiles[0].path)) {
        try {
          ...
          // parser unavailable â€?skip this language group
        }
      } else {
...
// Methods like get/post/put/delete/patch overlap with Express â€?those are captured by
...
/** Extract middleware from arguments â€?handles string or array */
...
function extractLaravelRoutes(tree: Parser.Tree, filePath: string): ExtractedRoute[] {
  ...
  function walk(node: SyntaxNode, groupStack: RouteGroupContext[]) {
    ...
    // Case 2: Fluent chain â€?Route::middleware(...)->group(...) or Route::middleware(...)->get(...)
...
const processFileGroup = (
  ...
  for (const file of files) {
    ...
    // Per-file map: decorator end-line â†?decorator info, for associating with definitions
    ...
    for (const match of matches) {
      ...
      if (captureMap['decorator'] && captureMap['decorator.name']) {
        ...
        // Store by the decorator's end line â€?the definition follows immediately after
      ...
      if (captureMap['call']) {
        ...
        if (callNameNode) {
          ...
          if (routed) {
            ...
            // kind === 'call' â€?fall through to normal call processing below
          }
      ...
      if (captureMap['heritage.class']) {
        if (captureMap['heritage.extends']) {
          ...
          // Named fields like `Breed string` also match â€?skip them.
          const extendsNode = captureMap['heritage.extends'];
          const fieldDecl = extendsNode.parent;
          const isNamedField =
      ...
      if (nodeLabel === 'Function' || nodeLabel === 'Method' || nodeLabel === 'Constructor') {
        // Try MethodExtractor first â€?it provides everything extractMethodSignature does, plus
      ...
      // Compute enclosing class for Method/Constructor/Property/Function â€?used for both ownerId and HAS_METHOD
      ...
      // â”€â”€ HAS_METHOD / HAS_PROPERTY: link member to enclosing class â”€â”€
      if (enclosingClassId) {
...
// ============================================================================
// Worker message handler â€?supports sub-batch streaming
// ============================================================================
