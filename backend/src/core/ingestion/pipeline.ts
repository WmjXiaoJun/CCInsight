...
import { type PipelineProgress, getLanguageFromFilename } from 'ccinsight-shared';
  ...
  try {
    for (let chunkIdx = 0; chunkIdx < numChunks; chunkIdx++) {
      ...
      if (chunkWorkerData) {
        ...
        // (populated later in runCrossFileBindingPropagation). This block is latent �?        // it activates only if incremental export collection is added per-chunk.
...
async function runGraphAnalysisPhases(
  ...
  if (isDev) {
    console.log(
      `🏘�?Community detection: ${communityResult.stats.totalCommunities} communities found (modularity: ${communityResult.stats.modularity.toFixed(3)})`,
    );
  }
...
export const runPipelineFromRepo = async (
  ...
  try {
    ...
    if (routeRegistry.size > 0) {
      ...
      if (isDev) {
        console.log(
          `🗺�?Route registry: ${routeRegistry.size} routes${duplicateRoutes > 0 ? ` (${duplicateRoutes} duplicate URLs skipped)` : ''}`,
        );
      }
    }
      ...
      if (middlewareCandidates.length > 0) {
        ...
        for (const [mwPath, mwContent] of mwContents) {
          ...
          if (isDev && linkedCount > 0) {
            console.log(
              `🛡�?Linked ${mwPath} middleware [${mwLabel.join(', ')}] to ${linkedCount} routes`,
            );
          }
        }
      }
