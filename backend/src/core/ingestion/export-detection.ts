 ...
 * in its language. This is a pure function â€?safe for use in worker threads.
 *
...
/** Java: check for 'public' modifier â€?modifiers are siblings of the name node, not parents. */
...
const CSHARP_DECL_TYPES = new Set([
  ...
  // record_declaration â€?these two node types are listed defensively but
 ...
 * in the same module/target. Since ccinsight indexes at the target level,
...
export const swiftExportChecker: ExportChecker = (node, _name) => {
  ...
  while (current) {
    if (current.type === 'modifiers' || current.type === 'visibility_modifier') {
      ...
      // Exclude private(set)/fileprivate(set) â€?only the setter is restricted,
