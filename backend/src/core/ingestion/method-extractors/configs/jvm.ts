// ccinsight/src/core/ingestion/method-extractors/configs/jvm.ts
...
function extractJavaParameters(node: SyntaxNode): ParameterInfo[] {
  ...
  // Compact constructors have no parameter list â€?inherit from parent record_declaration
  if (!paramList && node.type === 'compact_constructor_declaration') {
    const recordNode = node.parent?.parent; // compact_ctor â†?class_body â†?record_declaration
...
export const kotlinMethodConfig: MethodExtractionConfig = {
  ...
  isStatic(_node) {
    // Kotlin has no static â€?companion object members are separate
    return false;
  },
  ...
  isFinal(node) {
    // Kotlin functions are closed (final) by default â€?only open/abstract/override makes them overridable
  ...
  extractReceiverType(node) {
    ...
    // e.g., fun String.format(template: String) â†?receiver is "String"
    for (let i = 0; i < node.namedChildCount; i++) {
      const child = node.namedChild(i);
      if (!child) continue;
      if (child.type === 'simple_identifier') break; // past the name â€?no receiver
