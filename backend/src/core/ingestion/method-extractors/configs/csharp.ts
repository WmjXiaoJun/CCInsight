// ccinsight/src/core/ingestion/method-extractors/configs/csharp.ts
  ...
  while (i < paramList.childCount) {
    ...
    if (child.isNamed && child.type === 'parameter') {
      ...
      if (nameNode && nameNode.text.trim()) {
        ...
        // Detect ref / out modifiers inside the parameter node â€?prefix the type string
        ...
        // Detect optional (default value) â€?an '=' token among direct children
 ...
 * [param: Required]) â€?those target a different declaration element.
...
export const csharpMethodConfig: MethodExtractionConfig = {
  ...
  extractName(node) {
    ...
    // operator_declaration: no 'name' field â€?use 'operator' field (e.g., +, ==)
    if (node.type === 'operator_declaration') {
      const op = node.childForFieldName('operator');
      return op ? `operator ${op.text.trim()}` : undefined;
    }
    // conversion_operator_declaration: no 'name' field â€?implicit/explicit + target type
  ...
  extractPrimaryConstructor(
    ...
    // but has NO field name â€?it must be found by iterating named children.
