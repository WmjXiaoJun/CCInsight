...
import { SupportedLanguages } from 'ccinsight-shared';
...
import type { NodeLabel } from 'ccinsight-shared';

/**
 * Per-file scoped type environment: maps (scope, variableName) â†?typeName.
 ...
 * - Tier 2: single-pass assignment chain propagation in source order â€?resolves
...
export interface TypeEnvironment {
  ...
  /** All scoped bindings as a read-only nested map (scope â†?varName â†?type).
  ...
  /** Maps `scope\0varName` â†?constructor type for virtual dispatch override.
   *  Populated when a variable has BOTH a declared base type AND a more specific
   *  constructor type (e.g., `Animal a = new Dog()` â†?key maps to 'Dog'). */
...
/** scope â†?varName â†?overrides (checked in order, first range match wins) */
...
/** Implementation of the lookup logic â€?shared between TypeEnvironment and the legacy export. */
const lookupInEnv = (
  env: TypeEnv,
  varName: string,
  callNode: SyntaxNode,
 ...
 * - Java/Ruby: `superclass` field â†?type_identifier/constant
 * - Python: `superclasses` field â†?argument_list â†?first identifier
 * - TypeScript/JS: unnamed `class_heritage` child â†?`extends_clause` â†?identifier
 * - C#: unnamed `base_list` child â†?first identifier
 * - PHP: unnamed `base_clause` child â†?name
 * - Kotlin: unnamed `delegation_specifier` child â†?constructor_invocation â†?user_type â†?type_identifier
 * - C++: unnamed `base_class_clause` child â†?type_identifier
 * - Swift: unnamed `inheritance_specifier` child â†?user_type â†?type_identifier
 */
...
const findEnclosingScopeKey = (
  ...
  while (current) {
    ...
    // Language-specific hook (e.g., Dart function_body â†?sibling function_signature)
 ...
 *
 * Only `.has()` is exposed â€?the SymbolTable doesn't support iteration.
 ...
 * bindings that couldn't be resolved locally â€?all in one AST walk.
 *
 ...
 * Conservative leaf-only set â€?verified safe across all 12 supported language grammars.
 * IMPORTANT: Do NOT add expression containers (arguments, binary_expression, etc.) â€? * they can contain arrow functions with typed parameters.
 */
const SKIP_SUBTREE_TYPES = new Set([
  // Plain string literals (NOT template_string â€?it contains interpolated expressions
 ...
 *  inference) which is NOT captured â€?the type is inferred, not explicit.
 *  Kotlin constructors use `call_expression` (no `new` keyword) â€?not detected. */
 ...
 *   const user = getUser();      // callResult â†?User
 *   const addr = user.address;   // fieldAccess â†?Address (depends on user)
 *   const city = addr.getCity(); // methodCallResult â†?City (depends on addr)
 *   const alias = city;          // copy â†?City (depends on city)
 * Data flow: SymbolTable (immutable) + scopeEnv â†?resolve â†?scopeEnv.
...
const resolveFixpointBindings = (
  ...
  for (let iter = 0; iter < MAX_FIXPOINT_ITERATIONS; iter++) {
    ...
    if (!changed) break;
    if (iter === MAX_FIXPOINT_ITERATIONS - 1 && process.env.ccinsight_DEBUG) {
...
export interface BuildTypeEnvOptions {
   ...
   *  Same hook as LanguageProvider.enclosingFunctionFinder â€?handles languages
...
export const buildTypeEnv = (
  ...
  // Phase P: maps `scope\0varName` â†?constructor type when a declaration has BOTH
  // a base type annotation AND a more specific constructor initializer.
  // e.g., `Animal a = new Dog()` â†?constructorTypeMap.set('func@42\0a', 'Dog')
  ...
  // Ambiguous (2+) â†?undefined, no cross-file fallback (conservative, local-first principle).
  const returnTypeLookup: ReturnTypeLookup = {
    lookupReturnType(callee: string): string | undefined {
      ...
      if (symbolTable) {
        ...
        // Ambiguous (2+) â†?return undefined (conservative, no cross-file fallback)
        if (callables.length > 1) return undefined;
      }
      // No match (0 results or no symbolTable) â†?fall back to cross-file
  ...
  // Tier 2: unified fixpoint propagation â€?collects copy, callResult, fieldAccess, and
  // methodCallResult items during walk(), then iterates until no new bindings are produced.
  // Handles arbitrary-depth mixed chains: callResult â†?fieldAccess â†?methodCallResult â†?copy.
  ...
  // Maps `scope\0varName` â†?the type annotation AST node from the original declaration.
  ...
  // NOTE: This is a SUPERSET of scopeEnv â€?entries exist even when extractSimpleTypeName
  ...
  /**
   * Try to extract a (variableName â†?typeName) binding from a single AST node.
   *
  ...
  const extractTypeBinding = (
    ...
    if (TYPED_PARAMETER_TYPES.has(node.type)) {
      ...
      if (typeNode) {
        ...
        // Fallback: positional children (Kotlin `parameter` â†?simple_identifier + user_type)
    ...
    // Checked before declarationNodeTypes â€?loop variables are not declarations.
    ...
    if (config.declarationNodeTypes.has(node.type)) {
      ...
      // This decouples type node capture from scopeEnv success â€?container types
      ...
      // Map preserves insertion order, so new keys are always at the end â€?      // skip the first sizeBefore entries to find only newly-added variables.
      ...
      // Always called when available â€?each language's extractInitializer
      ...
      // e.g., `Animal a = new Dog()` â†?constructorTypeMap.set('scope\0a', 'Dog')
      if (sizeBefore >= 0 && scopeEnv.size > sizeBefore) {
        ...
        for (const varName of scopeEnv.keys()) {
          ...
          // Unwrap wrapper types (e.g., C++ shared_ptr<Animal> â†?Animal) for an
  ...
  const walk = (node: SyntaxNode, currentScope: string): void => {
    ...
    if (
      ...
      if (patternBinding) {
        if (patternBinding.narrowingRange) {
          ...
          // Also store in flat scopeEnv as fallback (last arm wins â€?same as before
    ...
    // Delegates to per-language extractPendingAssignment â€?AST shapes differ widely
  ...
  // (local declarations from walk() take precedence â€?first-writer-wins)
  ...
  //   - walk-time: users untyped â†?u unresolved
  //   - fixpoint: users â†?User[]
  //   - replay: users now typed â†?u â†?User
