...
import type { GraphNode, NodeLabel } from 'ccinsight-shared';
...
export interface ProcessNode {
  ...
  label: string; // "HandleLogin â†?CreateSession"
...
export const processProcesses = async (
  ...
  onProgress?.(
    `Deduped ${uniqueTraces.length} â†?${endpointDeduped.length} unique endpoint pairs`,
    70,
  );
  ...
  limitedTraces.forEach((trace, idx) => {
    ...
    const heuristicLabel = `${capitalize(entryName)} â†?${capitalize(terminalName)}`;
...
const findEntryPoints = (
  ...
  if (sorted.length > 0 && isDev) {
    ...
    sorted.slice(0, 10).forEach((c, i) => {
      ...
      console.log(`     score: ${c.score.toFixed(2)} = [${c.reasons.join(' x ')}]`);
    });
  }
...
/**
 * Keep only the longest trace per unique entryâ†’terminal pair.
