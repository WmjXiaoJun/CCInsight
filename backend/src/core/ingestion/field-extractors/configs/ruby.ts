// ccinsight/src/core/ingestion/field-extractors/configs/ruby.ts
...
function extractAttrNames(node: SyntaxNode): string[] {
  ...
  for (let i = 0; i < args.namedChildCount; i++) {
    ...
    if (!arg) continue;
    // simple_symbol text is :name â€?strip the leading colon
    const text = arg.text;
 ...
 *   (their arguments are symbol names â†?field names)
 *
