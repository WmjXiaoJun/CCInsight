 ...
 *   - C# / Java: apply the `I[A-Z]` naming convention (e.g. IDisposable â†?IMPLEMENTS)
 ...
 * Uses the symbol table first (authoritative â€?Tier 1); falls back to provider-defined
...
export const resolveExtendsType = (
  ...
  // Unresolved symbol â€?fall back to provider-defined heuristics
 ...
 * Uses ctx.resolve() â†?pick first candidate's nodeId â†?generate synthetic ID.
 */
...
export const processHeritage = async (
  ...
  if (skippedByLang && skippedByLang.size > 0) {
    for (const [lang, count] of skippedByLang.entries()) {
      console.warn(
        `[ingestion] Skipped ${count} ${lang} file(s) in heritage processing â€?${lang} parser not available.`,
      );
    }
  }
};
 ...
 * No AST parsing â€?workers already extracted className + parentName + kind.
 */
