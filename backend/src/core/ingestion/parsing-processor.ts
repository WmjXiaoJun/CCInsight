import type { GraphNode, GraphRelationship, NodeLabel } from 'ccinsight-shared';
...
const processParsingWithWorkers = async (
  ...
  // Dispatch to worker pool â€?pool handles splitting into chunks and sub-batching
...
// Keyed by tree-sitter node reference â€?cleared at the start of each file.
...
// FieldExtractor cache for sequential path â€?same pattern as parse-worker.ts
 ...
 *  SymbolTable, but it's incomplete at this stage â€?use the stub for safety). */
...
const processParsingSequential = async (
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      continue; // parser unavailable â€?safety net
    }
    ...
    // Build per-file type environment for FieldExtractor context (lightweight â€?skipped if no fieldExtractor)
    ...
    matches.forEach((match) => {
      ...
      if (nodeLabel === 'Property' && definitionNode) {
        ...
        // All 14 languages register a FieldExtractor â€?no fallback needed.
      }
