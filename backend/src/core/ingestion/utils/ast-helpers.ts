 ...
 *  For Go method_declaration nodes, extracts receiver type (e.g. `func (u *User) Save()` â†?User struct). */
export const findEnclosingClassId = (node: SyntaxNode, filePath: string): string | null => {
  ...
  while (current) {
    ...
    if (current.type === 'method_declaration') {
      ...
      if (receiver) {
        ...
        if (paramDecl) {
          ...
          if (typeNode) {
            // Unwrap pointer_type (*User â†?User)
    ...
    // field_declaration â†?field_declaration_list â†?struct_type â†?type_spec â†?type_declaration
    ...
    if (CLASS_CONTAINER_TYPES.has(current.type)) {
      ...
      if (current.type === 'impl_item') {
        ...
        // Fall through: plain `impl Struct {}` â€?use first type_identifier below
      }
      const nameNode =
...
export interface MethodSignature {
   ...
   *  Only set when fewer than parameterCount â€?enables range-based arity filtering.
   ...
   *  undefined (not []) when no types are extractable â€?avoids empty array allocations. */
...
export const extractMethodSignature = (node: SyntaxNode | null | undefined): MethodSignature => {
  ...
  const hasDefaultValue = (paramNode: SyntaxNode): boolean => {
    ...
    // The AST is: parameter, =, <literal>  â€?all at function_value_parameters level.
  ...
  if (parameterList && paramListTypes.has(parameterList.type)) {
    for (const param of parameterList.namedChildren) {
      ...
      // TypeScript/JavaScript: rest parameter â€?required_parameter containing rest_pattern
      ...
      // Kotlin uses positional children instead of 'type' field â€?fall back to
      ...
      if (paramTypeNode) {
        ...
        // Kotlin: parameter â†?[simple_identifier, user_type|nullable_type]
        let found = false;
    ...
    // C/C++: bare `...` token in parameter list (not a named child â€?check all children)
    if (!isVariadic) {
  ...
  // Return type extraction â€?language-specific field names
  ...
  if (goResult) {
    if (goResult.type === 'parameter_list') {
      // Multi-return: extract first parameter's type only (e.g. (*User, error) â†?*User)
      ...
      if (firstParam?.type === 'parameter_declaration') {
        ...
        // Unnamed return types: (string, error) â€?first child is a bare type node
  ...
  // Rust: 'return_type' field â€?the value IS the type node (e.g. primitive_type, type_identifier).
  ...
  // Kotlin: fun getUser(): User â€?return type is a bare user_type child of
  ...
  // Only set requiredParameterCount when it differs from total â€?saves memory on the common case.
