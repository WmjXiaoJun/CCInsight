...
export interface SymbolDefinition {
  ...
  returnType?: string;
  /** Declared type for non-callable symbols â€?fields/properties (e.g. 'Address', 'List<User>') */
  declaredType?: string;
...
export interface SymbolTable {
   ...
   * Returns first matching definition â€?use lookupExactAll for overloaded methods.
   */
   ...
   * Faster than `lookupFuzzy` + filter â€?backed by a lazy callable-only index.
   * Used by ReturnTypeLookup to resolve callee â†?return type.
   */
...
export const createSymbolTable = (): SymbolTable => {
  // 1. File-Specific Index â€?stores full SymbolDefinition(s) for O(1) lookup.
  ...
  // 3. Lazy Callable Index â€?populated on first lookupFuzzyCallable call.
  ...
  // 4. Eagerly-populated Field/Property Index â€?keyed by "ownerNodeId\0fieldName".
  ...
  const add = (
    ...
    // A. Add to File Index (shared reference â€?zero additional memory)
    ...
    // B. Properties go to fieldByOwner index only â€?skip globalIndex to prevent
