...
export function parseReplacingClause(text: string): CopyReplacing[] {
  ...
  while (i < tokens.length) {
    ...
    if (tokens[i].value.toUpperCase() !== 'BY') {
      // Malformed â€?skip this token and try to resync
      continue;
    }
    i++; // skip BY
...
export function expandCopies(
  ...
  function expandRecursive(
    ...
    // No COPY statements â€?return as-is
    ...
    for (let ci = copyStatements.length - 1; ci >= 0; ci--) {
      ...
      // Cannot resolve â€?keep original lines
      ...
      // Max depth exceeded â€?keep unexpanded
      if (depth >= maxDepth) {
        console.warn(
