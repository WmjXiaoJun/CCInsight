 ...
 *
 * DESIGN DECISION â€?Why regex instead of a full parser (ANTLR4, tree-sitter):
 *
 ...
 *    is a Java project â€?using it from Node.js requires Java subprocesses or
 ...
 *    same output â€?no grammar ambiguity or parser state issues.
 *
 * This module provides:
 * 1. preprocessCobolSource() â€?cleans patch markers (kept for potential future use)
 * 2. extractCobolSymbolsWithRegex() â€?single-pass state machine COBOL extraction
 */
 ...
 * irrelevant to parsing â€?compilers and tools always ignore it.  This
...
export function preprocessCobolSource(content: string): string {
  // Skip preprocessing for free-format COBOL â€?cols 1-6 are program text, not sequence area
...
// GO TO â€?control flow transfer (same graph semantics as PERFORM)
// GO TO â€?captures first target; GO TO p1 p2 p3 DEPENDING ON x handled below
const RE_GOTO =
...
// SEARCH â€?table access
const RE_SEARCH = /\bSEARCH\s+(?:ALL\s+)?([A-Z][A-Z0-9-]+)/i;

// CANCEL â€?program lifecycle
...
// INITIALIZE statement â€?data reset (captures targets before REPLACING/WITH clause)
...
// MOVE statement â€?captures everything after TO for multi-target extraction
...
// Note: CALL is intentionally excluded â€?it's handled by the callAccum state machine.
...
function parseDataItemClauses(rest: string): {
  ...
  // USAGE [IS] <usage-type> â€?including non-standard COMP-6, COMP-X etc.
...
function parseConditionValues(valuesStr: string): string[] {
  ...
  // No quotes â€?split on whitespace, filtering out THRU/THROUGH keywords
...
function parseExecSqlBlock(
  ...
  // EXEC SQL INCLUDE â€?extract member name for IMPORTS edge
...
function parseExecCicsBlock(
  ...
  // Command: first keyword(s) â€?handle two-word commands like SEND MAP, RECEIVE MAP
...
export function extractCobolSymbolsWithRegex(
  ...
  for (let i = 0; i < rawLines.length; i++) {
    ...
    // Normal line â€?flush any pending continuation first
    if (pendingLine !== null) {
      processLogicalLine(pendingLine, pendingLineNumber);
      pendingLine = null;
    }
  ...
  function processLogicalLine(line: string, lineNum: number): void {
      ...
      switch (divName) {
        ...
        case 'PROCEDURE': {
          ...
          if (procUsingMatch) {
            ...
            // USING may be on the next line â€?flag for extractProcedure to pick up
    ...
    // Continuation lines (not the start line) are consumed entirely â€?return after flush
    ...
    if (callAccum !== null) {
      ...
      // Only use RE_PROC_PARAGRAPH as flush trigger when in Area A (â‰? leading spaces, fixed-format).
      ...
      if (/\.\s*$/.test(line) || /\bEND-CALL\b/i.test(line)) {
        // Single-line CALL â€?extract immediately via flushCallAccum
        callAccum = line;
        callAccumLine = lineNum;
        flushCallAccum();
      } else {
        // Multi-line CALL â€?start accumulating
        callAccum = line;
        callAccumLine = lineNum;
  ...
  function flushSort(): void {
    ...
    if (smatch) {
      ...
      // INPUT PROCEDURE IS / OUTPUT PROCEDURE IS â†?control-flow targets (like PERFORM)
  ...
  function extractData(line: string, lineNum: number): void {
    ...
    if (anonRedefMatch) {
      ...
      // followed by more clauses â€?it's the REDEFINES target directly after level
      ...
      if (!dataMatch || dataMatch[2].toUpperCase() === 'REDEFINES') {
        // Truly anonymous â€?skip, no node
        return;
      }
    }
  ...
  function extractProcedure(line: string, lineNum: number): void {
    ...
    if (secMatch) {
      ...
      if (
        ...
        // Don't set currentParagraph to section name â€?sections are Namespaces,
    ...
    if (paraMatch) {
      ...
      const leadingSpaces = line.match(/^(\s*)/)?.[1].length ?? 0;
      if (!isFreeFormat && leadingSpaces > 7) return; // Area B â€?not a paragraph
      if (
        !EXCLUDED_PARA_NAMES.has(name.toUpperCase()) &&
    ...
    // PERFORM (global â€?captures multiple PERFORMs on the same logical line)
    for (const perfMatch of line.matchAll(RE_PERFORM)) {
      ...
      if (!PERFORM_KEYWORD_SKIP.has(target.toUpperCase())) {
        // Also check for "PERFORM identifier TIMES" â€?the identifier is a
    ...
    // GO TO â€?control flow transfer (handles GO TO p1 p2 p3 DEPENDING ON x)
    ...
    if (sortAccum !== null) {
      ...
      sortAccum += ' ' + line;
      if (!/\.\s*$/.test(sortAccum)) return; // still accumulating â€?skip other extractors
      // Period found â€?flush, then re-check line for a new SORT/MERGE after the period
      flushSort();
    ...
    if (sortMatch && sortAccum === null) {
      ...
      if (!/\.\s*$/.test(sortAccum)) return; // multi-line â€?wait for period
      flushSort();
    }

    // INSPECT â€?multi-line accumulator (like SORT)
    ...
    // CANCEL â€?program lifecycle (global matchAll captures multiple CANCELs on same line)
    ...
    // Dynamic CANCEL â€?RE_CANCEL_DYNAMIC cannot match quoted targets, no dedup guard needed
    ...
    // INITIALIZE â€?data reset (multi-target: INITIALIZE WS-A WS-B WS-C.)
