...
function collectRustBindings(node: SyntaxNode, bindings: NamedBinding[]): void {
  ...
  // e.g. use crate::models::{User, Repo} â€?the path node "crate::models" is not an importable symbol
  if (node.type === 'scoped_identifier' && node.parent?.type === 'scoped_use_list') {
    return; // path prefix â€?the use_list sibling handles the actual symbols
  }
