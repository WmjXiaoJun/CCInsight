...
export function extractTsNamedBindings(importNode: SyntaxNode): NamedBinding[] | undefined {
  ...
  if (importClause) {
    ...
    for (let i = 0; i < namedImports.namedChildCount; i++) {
      ...
      if (identifiers.length === 1) {
        ...
        // import { Foo as Bar } â†?exported='Foo', local='Bar'
  ...
  // Re-export: export { X } from './y' â†?export_statement > export_clause > export_specifier
  ...
  if (exportClause) {
    ...
    for (let i = 0; i < exportClause.namedChildCount; i++) {
      ...
      if (identifiers.length === 1) {
        // export { User } from './base' â†?re-exports User as User
        bindings.push({ local: identifiers[0], exported: identifiers[0] });
      } else if (identifiers.length === 2) {
        // export { Repo as Repository } from './models' â†?name=Repo, alias=Repository
