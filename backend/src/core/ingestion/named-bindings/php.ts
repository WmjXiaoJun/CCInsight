...
export function extractPhpNamedBindings(importNode: SyntaxNode): NamedBinding[] | undefined {
  ...
  // Skip 'use function' and 'use const' declarations â€?these import callables/constants,
  ...
  // Collect all clauses â€?from direct children AND from namespace_use_group
  ...
  for (const clause of clauses) {
    ...
    if (qualifiedName && names.length > 0) {
      ...
      // Grouped aliased import: {Repo as R} â€?first name = exported, second = alias
