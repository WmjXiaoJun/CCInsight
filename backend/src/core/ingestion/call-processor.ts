...
import { getLanguageFromFilename } from 'ccinsight-shared';
...
/** Build a map of imported callee names â†?return types for cross-file call-result binding.
 ...
 *  Uses graph node isExported flag â€?does NOT require isExported on SymbolDefinition. */
...
/** Build ExportedTypeMap from graph nodes â€?used for worker path where TypeEnv
...
export function buildExportedTypeMapFromGraph(
  ...
  graph.forEachNode((node) => {
    ...
    // Extract simple type name (strip Promise<>, etc.) â€?reuse shared utility
 ...
 *  using ExportedTypeMap + namedImportMap â€?zero disk I/O, zero AST re-parsing.
...
// strips nullable wrappers (Option<User> â†?User), these chain steps are no-ops
// for type resolution â€?the current type passes through unchanged.
...
/** Maps interface/abstract-class name â†?set of file paths of direct implementors. */
 ...
 * `extends` is ignored â€?dispatch keyed on abstract class bases is not modeled here.
 */
/**
 * Maps interface name â†?file paths of classes that implement it (direct only).
...
export const processCalls = async (
  ...
  /** Phase 14: pre-resolved cross-file bindings to seed into buildTypeEnv. Keyed by filePath â†?Map<localName, typeName>. */
  importedBindingsMap?: ReadonlyMap<string, ReadonlyMap<string, string>>,
  /** Phase 14 E3: cross-file return types for imported callables. Keyed by filePath â†?Map<calleeName, returnType>.
  ...
  /** Phase 14 E3: cross-file RAW return types for for-loop element extraction. Keyed by filePath â†?Map<calleeName, rawReturnType>. */
  ...
  // Used as a secondary check when per-file parentMap lacks the relationship â€?helps
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    matches.forEach((match) => {
      const captureMap: Record<string, any> = {};
      match.captures.forEach((c) => (captureMap[c.name] = c.node));
      // â”€â”€ Write access: emit ACCESSES {reason: 'write'} for assignments to member fields â”€â”€
      if (
        ...
        // Resolve receiver type: simple identifier â†?TypeEnv lookup or class resolution
        ...
        // forEach iteration â€?this acts as a `continue` in the match loop.
      ...
      // Phase P: virtual dispatch override â€?when the declared type is a base class but
      ...
      if (receiverTypeName && receiverName && typeEnv && typeEnv.constructorTypeMap.size > 0) {
        ...
        if (ctorType && ctorType !== receiverTypeName) {
          ...
          // confirming both are class-like types is sufficient â€?the original code would
      ...
      // (field chain, call chain, or interleaved â€?e.g. user.address.city.save() or
      ...
      // Only used when multiple candidates survive arity filtering â€?~1-3% of calls.
  ...
  // â”€â”€ Resolve deferred write-access edges â”€â”€
  ...
  if (skippedByLang && skippedByLang.size > 0) {
    for (const [lang, count] of skippedByLang.entries()) {
      console.warn(
        `[ingestion] Skipped ${count} ${lang} file(s) in call processing â€?${lang} parser not available.`,
      );
    }
  }
...
/**
 * Kotlin often declares parameters with boxed names (`Int`, `Boolean`, â€? while
...
const matchCandidatesByArgTypes = (
  ...
  const matched = candidates.filter((c) => {
    // Keep candidates without type info â€?conservative: partially-annotated codebases
    // (e.g. C++ with some missing declarations) may have mixed typed/untyped overloads.
    // If one typed and one untyped both survive, matched.length > 1 â†?returns null (no edge).
    if (!c.parameterTypes) return true;
    return c.parameterTypes.every((pType, i) => {
      if (i >= argTypes.length || !argTypes[i]) return true;
      // Normalise Kotlin boxed type names (Intâ†’int, Booleanâ†’boolean, etc.) so
  ...
  // implementation body all collide via generateId). Deduplicate by nodeId â€?if all
...
const resolveCallTarget = (
  ...
  // The attribute access gives callForm='member', but the callee may be a Class â€?a valid
  ...
  // Module-alias disambiguation: Python `import auth; auth.User()` â€?receiverName='auth'
  // selects auth.py via moduleAliasMap. Runs for ALL member calls with a known module alias,
  // not just ambiguous ones â€?same-file tier may shadow the correct cross-module target when
  ...
  //
  // Applied regardless of candidate count â€?the sole same-file candidate may
...
// â”€â”€ Scope key helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/** Extract the function name from a scope key ("funcName@startIndex" â†?"funcName"). */
const extractFuncNameFromScope = (scope: string): string => scope.slice(0, scope.indexOf('@'));

/** Extract the trailing function name from a sourceId ("Function:filepath:funcName" â†?"funcName"). */
 ...
 * Built once per file from the verified receiver map, keyed by funcName â†?varName.
 */
...
const lookupReceiverType = (
  ...
  if (funcBucket) {
    ...
    if (entry?.kind === 'ambiguous') {
      // Ambiguous in this function scope â€?try file-level fallback
...
const resolveFieldAccessType = (
  ...
  // Use stripNullable (not extractReturnTypeName) â€?field types like List<User>
 ...
 * Does NOT require declaredType â€?used by write-access tracking where only the
...
const walkMixedChain = (
  ...
  for (const step of chain) {
    ...
    if (step.kind === 'field') {
      ...
      // Try field resolution first â€?if the name is a known property with declaredType,
 ...
 * No AST parsing â€?workers already extracted calledName + sourceId.
 */
export const processCallsFromExtracted = async (
  ...
  // Scope-aware receiver types: keyed by filePath â†?"funcName\0varName" â†?typeName.
  ...
  for (const [filePath, calls] of byFile) {
    ...
    for (const call of calls) {
      ...
      // Step 1c: mixed chain resolution (field, call, or interleaved â€?e.g. svc.getUser().address.save()).
...
const RESPONSE_ACCESS_BLOCKLIST = new Set([
  ...
  'values',
  'entries',
  // DOM methods â€?file-download patterns often reuse `data`/`response` variable names
  'appendChild',
  'removeChild',
  'insertBefore',
  'replaceChild',
...
export const extractConsumerAccessedKeys = (content: string): string[] => {
  ...
  // Pattern 1: Destructuring from .json() â€?const { key1, key2 } = await res.json()
...
export const processNextjsFetchRoutes = (
  ...
  routeRegistry: Map<string, string>, // routeURL â†?handlerFilePath
  consumerContents?: Map<string, string>, // filePath â†?file content
) => {
