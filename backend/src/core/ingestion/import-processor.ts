...
// aliased imports (`import { User as U }`) can resolve U â†?User in the source file.
...
// ImportResolutionContext is defined in ./import-resolvers/types.ts â€?re-exported here for consumers.
...
function applyImportResult(
  ...
  if (result.kind === 'package' && packageMap) {
    ...
    // 'files' kind, or 'package' without PackageMap â€?use ImportMap directly
    ...
    // These are module-level aliases, not symbol bindings â€?they don't belong in namedImportMap.
...
export const processImports = async (
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    matches.forEach((match) => {
      ...
      if (captureMap['import']) {
        ...
        if (!sourceNode) {
          if (isDev) {
            console.log(`âš ï¸ Import captured but no source node in ${file.path}`);
          }
          return;
        }
    ...
    // Tree is now owned by the LRU cache â€?no manual delete needed
  }
  ...
  if (skippedByLang && skippedByLang.size > 0) {
    for (const [lang, count] of skippedByLang.entries()) {
      console.warn(
        `[ingestion] Skipped ${count} ${lang} file(s) in import processing â€?${lang} parser not available.`,
      );
    }
  }
  ...
  if (isDev) {
    console.log(
      `ðŸ“Š Import processing complete: ${getResolvedCount()}/${totalImportsFound} imports resolved to graph edges`,
    );
  }
};
...
export const processImportsFromExtracted = async (
  ...
  if (isDev) {
    console.log(
      `ðŸ“Š Import processing (fast path): ${getResolvedCount()}/${totalImportsFound} imports resolved to graph edges`,
    );
  }
};
