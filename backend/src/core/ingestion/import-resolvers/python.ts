/**
 * Python import resolution â€?PEP 328 relative imports and proximity-based bare imports.
 ...
 *
 * 1. Relative (PEP 328): `.module`, `..module` â€?1 dot = current package, each extra dot goes up one level.
 * 2. Proximity bare import: static heuristic â€?checks the importer's own directory first.
 *    Approximates the common case where co-located files find each other without an installed package.
 *    Single-segment only â€?multi-segment (e.g. `os.path`) falls through to suffixResolve.
 *    Checks package (__init__.py) before module (.py), matching CPython's finder order (PEP 451 Â§4).
...
export function resolvePythonImportInternal(
  ...
  // Relative import â€?PEP 328 (https://peps.python.org/pep-0328/)
  if (importPath.startsWith('.')) {
    ...
    // PEP 328: more dots than directory levels â†?beyond top-level package â†?invalid
  ...
  // Proximity bare import â€?single-segment only; package before module (PEP 451 Â§4)
  ...
  // Ancestor directory walk â€?Python resolves bare imports against sys.path entries,
