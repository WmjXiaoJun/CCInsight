 ...
 * a known limitation â€?PHP function imports cannot be resolved to a specific file
...
export function resolvePhpImportInternal(
  ...
  // Reject path traversal attempts (defense-in-depth â€?walker whitelist also prevents this)
  ...
  if (composerConfig) {
    ...
    for (const [nsPrefix, dirPrefix] of sorted) {
      ...
      if (normalized.startsWith(nsPrefixSlash + '/') || normalized === nsPrefixSlash) {
        ...
        // 1. Try class-style PSR-4: full path â†?file (e.g. App\Models\User â†?app/Models/User.php)
        ...
        //    e.g. App\Models\getUser â†?directory app/Models/, find first .php file in that dir.
