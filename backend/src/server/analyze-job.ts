...
export class JobManager {
  ...
  /** Subscribe to graph node batch events for a job. Returns unsubscribe function. */
  onNodes(jobId: string, listener: (batch: GraphNodeBatch) => void): () => void {
    const event = `nodes:${jobId}`;
    this.emitter.on(event, listener);
    return () => this.emitter.off(event, listener);
  }

  /** Emit a batch of graph nodes to all subscribers. */
  emitNodes(jobId: string, batch: GraphNodeBatch) {
    this.emitter.emit(`nodes:${jobId}`, batch);
  }
