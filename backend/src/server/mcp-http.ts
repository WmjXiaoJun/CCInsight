 ...
 * MCP over HTTP
 *
 * Mounts the ccinsight MCP server on Express using StreamableHTTP transport.
 * Each connecting client gets its own stateful session; the LocalBackend
 * is shared across all sessions (thread-safe â€?lazy LadybugDB per repo).
 *
...
export function mountMCPEndpoints(app: Express, backend: LocalBackend): () => Promise<void> {
  ...
  const handleMcpRequest = async (req: Request, res: Response) => {
    ...
    if (sessionId && sessions.has(sessionId)) {
      ...
      // Unknown/expired session ID â€?tell client to re-initialize (per MCP spec)
      res.status(404).json({
        jsonrpc: '2.0',
      ...
      // No session ID â€?new client initializing
