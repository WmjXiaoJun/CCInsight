 ...
 *   Child -> Parent: { type: 'nodes', nodes: GraphNode[], relationships: GraphRelationship[] }
...
type WorkerMessage = ProgressMessage | NodesMessage | CompleteMessage | ErrorMessage;
...
process.on('message', async (msg: StartMessage) => {
  ...
  try {
    const result = await runFullAnalysis(msg.repoPath, msg.options, {
      ...
      onNodes: (nodes, relationships) => {
        // Stream nodes to parent for real-time graph display
        send({ type: 'nodes', nodes, relationships });
      },
    });
