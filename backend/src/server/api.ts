 ...
 * HTTP API Server
 *
 * REST API for browser-based clients to query the local .ccinsight/ index.
...
import { NODE_TABLES, type GraphNode, type GraphRelationship } from 'ccinsight-shared';
 ...
 * - https://ccinsight.vercel.app ??the deployed ccinsight web UI
 *
...
export const isAllowedOrigin = (origin: string | undefined): boolean => {
  ...
  if (
    ...
    origin === 'http://[::1]' ||
    origin === 'https://ccinsight.vercel.app'
  ) {
    return true;
  }
...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  app.get('/api/repo', async (req, res) => {
    try {
      ...
      if (!entry) {
        res.status(404).json({ error: 'Repository not found. Run: ccinsight analyze' });
        return;
      }
  ...
  app.delete('/api/repo', async (req, res) => {
    try {
      ...
      try {
        ...
        // 1. Delete the .ccinsight index/storage directory
        const storagePath = getStoragePath(entry.path);
        await fs.rm(storagePath, { recursive: true, force: true }).catch(() => {});

        // 2. Delete the cloned repo dir if it lives under ~/.ccinsight/repos/
          ...
          const forkWorker = () => {
            ...
            child.on('message', (msg: any) => {
              if (msg.type === 'progress') {
                ...
                // Forward node batch to SSE subscribers for real-time graph display
                jobManager.emitNodes(job.id, {
                  nodes: msg.nodes ?? [],
  ...
  // GET /api/analyze/:jobId/nodes - SSE stream of graph node batches during analysis
  app.get('/api/analyze/:jobId/nodes', (req, res) => {
    const job = jobManager.getJob(req.params.jobId);
    if (!job) {
      res.status(404).json({ error: 'Job not found' });
      return;
    ...
    // If already terminal, just send end event
    if (job.status === 'complete' || job.status === 'failed') {
      eventId++;
    ...
    // Subscribe to node batches
    const unsubscribe = jobManager.onNodes(req.params.jobId, (batch) => {
      try {
        eventId++;
  ...
  // POST /api/llm/chat ? proxy LLM requests to avoid CORS issues
  app.post('/api/llm/chat', async (req, res) => {
    try {
      const { baseUrl, apiKey, body } = req.body;

      if (!baseUrl || !body) {
        res.status(400).json({ error: 'Missing baseUrl or body' });
        return;
      }
      ...
      if (response.headers.get('content-type')?.includes('text/event-stream')) {
        ...
        if (response.body) {
          ...
          const readChunk = () => {
            reader.read().then(({ done, value }) => {
              ...
              console.error('LLM proxy stream read error:', err);
              res.end();
            });
          };
        ...
        if (!response.ok) {
          ...
          res.status(response.status).json({ error: `LLM provider error: ${response.status}`, details: errorText });
          return;
        }
      ...
      res.status(500).json({ error: err.message || 'LLM proxy failed' });
    }
  });

  // GET /api/llm/models ? fetch available models from a provider
  app.get('/api/llm/models', async (req, res) => {
    try {
      ...
      if (!response.ok) {
        res.status(response.status).json({ error: `Failed to fetch models: ${response.status}` });
        return;
      }
  ...
  await new Promise<void>((resolve, reject) => {
    const server = app.listen(port, host, () => {
      console.log(`ccinsight server running on http://${host}:${port}`);
      resolve();
    });
