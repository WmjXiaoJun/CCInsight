...
// NOTE: Negation patterns in .ccinsightignore (e.g. `!vendor/`) cannot override
// entries in DEFAULT_IGNORE_LIST â€?this is intentional. The hardcoded list protects
...
/**
 * Load .gitignore and .ccinsightignore rules from the repo root.
 * Returns an `ignore` instance with all patterns, or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .ccinsightignore. Defaults to ccinsight_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
...
export const loadIgnoreRules = async (
  ...
  const skipGitignore = options?.noGitignore ?? !!process.env.ccinsight_NO_GITIGNORE;
  const filenames = skipGitignore ? ['.ccinsightignore'] : ['.gitignore', '.ccinsightignore'];
 ...
 * - .gitignore / .ccinsightignore patterns (via `ignore` package)
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // Check .gitignore / .ccinsightignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      // defense-in-depth â€?do not remove `dot: false` assuming this covers it.
      if (DEFAULT_IGNORE_LIST.has(p.name)) return true;
      // Check against .gitignore / .ccinsightignore patterns.
