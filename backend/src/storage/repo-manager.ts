 ...
 * Repository Manager
 *
 * Manages ccinsight index storage in .ccinsight/ at repo root.
 * Also maintains a global registry at ~/.ccinsight/registry.json
...
/**
 * Shape of an entry in the global registry (~/.ccinsight/registry.json)
 */
...
const ccinsight_DIR = '.ccinsight';

// ─── Local Storage Helpers ─────────────────────────────────────────────

/**
 * Get the .ccinsight storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), ccinsight_DIR);
};
 ...
 *
 * Returns:
 *   found        �?true if .ccinsight/kuzu existed and was deleted
...
/**
 * Check if a path has a ccinsight index
 */
...
/**
 * Find .ccinsight by walking up from a starting path
 */
...
/**
 * Add .ccinsight to .gitignore if not already present
 */
export const addToGitignore = async (repoPath: string): Promise<void> => {
  ...
  try {
    ...
    if (content.includes(ccinsight_DIR)) return;

    const newContent = content.endsWith('\n')
      ? `${content}${ccinsight_DIR}\n`
      : `${content}\n${ccinsight_DIR}\n`;
    await fs.writeFile(gitignorePath, newContent, 'utf-8');
  } catch {
    // .gitignore doesn't exist, create it
    await fs.writeFile(gitignorePath, `${ccinsight_DIR}\n`, 'utf-8');
  }
};

// ─── Global Registry (~/.ccinsight/registry.json) ───────────────────────

/**
 * Get the path to the global ccinsight directory
 */
export const getGlobalDir = (): string => {
  return path.join(os.homedir(), '.ccinsight');
};
 ...
 * Called after `ccinsight analyze` completes.
 */
 ...
 * Optionally validates that each entry's .ccinsight/ still exists.
 */
export const listRegisteredRepos = async (opts?: {
  ...
  // Validate each entry still has a .ccinsight/ directory
...
// ─── Global CLI Config (~/.ccinsight/config.json) ─────────────────────────
...
/**
 * Load CLI config from ~/.ccinsight/config.json
 */
...
/**
 * Save CLI config to ~/.ccinsight/config.json
 */
