 ...
 * Each test file clears all data, reseeds, and initializes adapters â€? * avoiding per-file schema creation overhead.
 *
 ...
 * Seed data is NOT included â€?each test provides its own via options.seed.
 */
...
export interface IndexedDBHandle {
  ...
  dbPath: string;
  /** Unique repoId for MCP pool adapter â€?prevents cross-file collisions */
  repoId: string;
 ...
 *
 * Lifecycle: initLbug â†?loadFTS â†?dropFTS â†?clearData â†?seed
 *            â†?createFTS â†?[closeCoreLbug + poolInitLbug] â†?afterSetup
 */
 ...
 *
 * All data operations go through the core adapter's writable connection â€? * no raw lbug.Database() connections are opened.  This avoids file-lock
 ...
 * hooks â€?safe to call multiple times in the same file.
 */
export function withTestLbugDB(
  ...
  const setup = async () => {
    ...
    // 1. Init core adapter (writable) â€?reuses existing connection if
    //    already open for this dbPath (no new native objects created).
    await adapter.initLbug(dbPath);

    // 2. Load FTS extension (idempotent â€?skips if already loaded)
    ...
    //    LadybugDB enforces file locks â€?writable + read-only can't coexist
    ...
    // tmpHandle.dbPath â†?parent temp dir (not the lbug file) so tests
  ...
  const lazyHandle = new Proxy({} as IndexedDBHandle, {
    get(_target, prop) {
      if (!ref.handle)
        throw new Error('withTestLbugDB: handle not initialized â€?beforeAll has not run yet');
      return (ref.handle as any)[prop];
    },
  });

  // Wrap in describe to scope beforeAll/afterAll â€?prevents lifecycle
