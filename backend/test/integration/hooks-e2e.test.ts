 ...
 *
 * Tests the hook scripts with real git repos and .ccinsight directories.
...
const CJS_HOOK = path.resolve(__dirname, '..', '..', 'hooks', 'claude', 'ccinsight-hook.cjs');
const PLUGIN_HOOK = path.resolve(
  __dirname,
  '..',
  '..',
  '..',
  'ccinsight-claude-plugin',
  'hooks',
  'ccinsight-hook.js',
);
...
// ─── Temp git repo with .ccinsight ───────────────────────────────────

let tmpDir: string;
let ccinsightDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hooks-e2e-'));
  ccinsightDir = path.join(tmpDir, '.ccinsight');
  fs.mkdirSync(ccinsightDir, { recursive: true });
...
describe.each(HOOKS)('hooks e2e ($name)', ({ name, path: hookPath }) => {
  describe('PostToolUse staleness detection', () => {
    it('detects stale index when meta.json lastCommit differs from HEAD', () => {
      ...
      fs.writeFileSync(
        path.join(ccinsightDir, 'meta.json'),
      ...
      expect(output!.additionalContext).toContain('npx ccinsight analyze');
    });
    ...
    it('treats missing meta.json as stale', () => {
      // Remove meta.json
      const metaPath = path.join(ccinsightDir, 'meta.json');
  ...
  describe('PreToolUse �?silent without ccinsight CLI', () => {
    // PreToolUse tries to spawn `ccinsight augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `ccinsight augment` hangs and the 10s timeout kills the process.
  ...
  describe('unhappy paths', () => {
    it('handles corrupted meta.json (invalid JSON) without crashing', () => {
      fs.writeFileSync(path.join(ccinsightDir, 'meta.json'), 'THIS IS NOT JSON {{{');
    ...
    it('handles meta.json with missing lastCommit field', () => {
      fs.writeFileSync(path.join(ccinsightDir, 'meta.json'), JSON.stringify({ stats: {} }));
  ...
  describe('directory without .ccinsight', () => {
    // The hook walks up 5 parent directories looking for .ccinsight.
    // To guarantee none is found, create a deeply nested temp dir at the
    // filesystem root where no .ccinsight could exist in any ancestor.
    let noccinsightDir: string;

    beforeAll(() => {
      // Use a root-level temp path so parent traversal can't find .ccinsight
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, `no-ccinsight-${Date.now()}`);
      // Nest 6 levels deep (hook walks up 5) to ensure isolation
      noccinsightDir = path.join(base, 'a', 'b', 'c', 'd', 'e', 'f');
      fs.mkdirSync(noccinsightDir, { recursive: true });
      spawnSync('git', ['init'], { cwd: noccinsightDir, stdio: 'pipe' });
    });
    ...
    afterAll(() => {
      ...
      const base = path.join(
        root,
        path.basename(path.resolve(noccinsightDir, '..', '..', '..', '..', '..', '..')),
      );
      fs.rmSync(base, { recursive: true, force: true });
    });

    it('ignores PostToolUse when no .ccinsight directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PostToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'git commit -m "x"' },
    ...
    it('ignores PreToolUse when no .ccinsight directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PreToolUse',
        tool_name: 'Grep',
