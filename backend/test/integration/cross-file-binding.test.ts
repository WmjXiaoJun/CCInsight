 ...
 * file B imports `user`, Phase 14 seeds `user â†?User` into file B's type
...
// ---------------------------------------------------------------------------
// Simple cross-file: models â†?service â†?app
// models.ts exports getUser(): User
// service.ts exports const user = getUser()   (user â†?User via call-result)
// app.ts imports user from service â†?seeds user â†?User â†?resolves user.save()
// ---------------------------------------------------------------------------
...
describe('Cross-File Binding Propagation: TypeScript simple cross-file', () => {
  ...
  it('emits IMPORTS edges across all three files', () => {
    ...
    // service.ts â†?models.ts and app.ts â†?service.ts
    expect(imports.length).toBeGreaterThanOrEqual(2);
    const paths = imports.map((e) => `${e.sourceFilePath} â†?${e.targetFilePath}`);
...
// ---------------------------------------------------------------------------
// Re-export chain: core â†?index (barrel) â†?app
...
// â†?config.validate() resolves to Config#validate via local call-result binding
// ---------------------------------------------------------------------------
...
// â†?c is typed Config via importedReturnTypes (E3), enabling Config#validate edge
// ---------------------------------------------------------------------------
