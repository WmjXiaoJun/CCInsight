...
describe('TypeScript heritage resolution', () => {
  ...
  it('emits exactly 3 IMPORTS edges', () => {
    ...
    expect(edgeSet(imports)).toEqual([
      'logger.ts â†?models.ts',
      'service.ts â†?logger.ts',
      'service.ts â†?models.ts',
    ]);
  });

  it('emits exactly 1 EXTENDS edge: UserService â†?BaseService', () => {
  ...
  it('emits exactly 2 IMPLEMENTS edges', () => {
    ...
    expect(edgeSet(implements_)).toEqual(['ConsoleLogger â†?ILogger', 'UserService â†?ILogger']);
  });
  ...
  it('emits HAS_METHOD edges linking methods to classes', () => {
    ...
    expect(edgeSet(hasMethod)).toEqual([
      'BaseService â†?getName',
      'ConsoleLogger â†?log',
      'UserService â†?getUsers',
      'UserService â†?log',
    ]);
  });
  ...
  it('emits HAS_PROPERTY edge for class fields', () => {
    ...
    expect(edgeSet(hasProperty)).toEqual(['BaseService â†?name']);
  });
...
describe('TypeScript call resolution with arity filtering', () => {
  ...
  it('resolves run â†?writeAudit to src/one.ts via arity narrowing', () => {
...
describe('TypeScript member-call resolution', () => {
  ...
  it('resolves processUser â†?save as a member call on User', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('TypeScript named import disambiguation', () => {
  ...
  it('resolves processInput â†?formatData to src/format-upper.ts via named import', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Alias import resolution: import { User as U } resolves U â†?User
// ---------------------------------------------------------------------------
...
describe('TypeScript local definition shadows import', () => {
  ...
  it('resolves run â†?save to same-file definition, not the imported one', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('TypeScript variadic call resolution', () => {
  ...
  it('resolves processInput â†?logEntry to src/logger.ts despite 3 args vs rest param', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('TypeScript parent resolution', () => {
  ...
  it('emits EXTENDS edge: User â†?BaseModel', () => {
    const extends_ = getRelationships(result, 'EXTENDS');
    expect(extends_.length).toBe(1);
  ...
  it('emits IMPLEMENTS edge: User â†?Serializable', () => {
    const implements_ = getRelationships(result, 'IMPLEMENTS');
    expect(implements_.length).toBe(1);
...
describe('TypeScript generic parent super resolution', () => {
  ...
  it('emits EXTENDS edge: User â†?BaseModel (not BaseModel<string>)', () => {
    const extends_ = getRelationships(result, 'EXTENDS');
    expect(extends_.length).toBe(1);
    expect(extends_[0].source).toBe('User');
    expect(extends_[0].target).toBe('BaseModel');
...
describe('TypeScript nullable receiver resolution (optional chaining)', () => {
  ...
  it('emits exactly 2 save() CALLS edges (one per receiver type)', () => {
    const calls = getRelationships(result, 'CALLS');
    const saveCalls = calls.filter((c) => c.target === 'save');
    // user?.save() â†?User.save + repo?.save() â†?Repo.save = 2 edges
...
// `const alias = u` where `u: User` â†?alias.save() resolves to User#save
// ---------------------------------------------------------------------------
...
describe('TypeScript assignment chain propagation (Tier 2)', () => {
  ...
  it('resolves alias.save() to User#save via assignment chain', () => {
    ...
    // rAlias.save() correctly goes to Repo â€?but we verify there is exactly one
    // per-receiver resolution (user alias â†?User, repo alias â†?Repo)
  ...
  it('resolves rAlias.save() to Repo#save via assignment chain', () => {
    ...
    expect(userSave).toBeDefined();
    // Both resolve separately â€?alias â†?User, rAlias â†?Repo
    expect(userSave!.targetFilePath).not.toBe(repoSave!.targetFilePath);
  });
});

// ---------------------------------------------------------------------------
// Multi-hop forward-declared chain (a â†?b â†?c) â€?validates that single-pass
// in source order resolves chains deeper than depth-1.
// ---------------------------------------------------------------------------

describe('TypeScript multi-hop assignment chain (a â†?b â†?c)', () => {
  let result: PipelineResult;
  ...
  it('resolves c.save() to User#save through a â†?b â†?c chain', () => {
    const calls = getRelationships(result, 'CALLS');
  ...
  it('resolves c.save() to Repo#save through a â†?b â†?c chain (Repo variant)', () => {
    const calls = getRelationships(result, 'CALLS');
    const repoSave = calls.find(
      (c) =>
...
// union (User | null â†?User) before the chain propagation can work.
...
// Resolution must walk the chain: getUser() returns User, so save() â†?User#save.
// ---------------------------------------------------------------------------
...
describe('TypeScript overloaded-receiver resolution (receiverKey collision fix)', () => {
  ...
  it('emits exactly 2 save() CALLS edges â€?one per class', () => {
    const calls = getRelationships(result, 'CALLS');
    const saveCalls = calls.filter((c) => c.target === 'save');
    expect(saveCalls.length).toBe(2);
...
// ---------------------------------------------------------------------------
// for (const [key, user] of entries) â€?destructured for-of resolution
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// if (x instanceof User) { x.save() } â€?instanceof narrowing resolution
// ---------------------------------------------------------------------------
...
describe('Field type resolution (TypeScript)', () => {
  ...
  it('emits HAS_PROPERTY edges linking properties to classes', () => {
    ...
    expect(edgeSet(propEdges)).toContain('User â†?address');
    expect(edgeSet(propEdges)).toContain('User â†?name');
    expect(edgeSet(propEdges)).toContain('Address â†?city');
    expect(edgeSet(propEdges)).toContain('Config â†?DEFAULT');
  });

  it('resolves user.address.save() â†?Address#save via field type', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Phase 8: Field type disambiguation â€?both User and Address have save()
// ---------------------------------------------------------------------------
...
describe('Field type disambiguation (TypeScript)', () => {
  ...
  it('resolves user.address.save() â†?Address#save (not User#save)', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('Deep field chain resolution (TypeScript)', () => {
  ...
  it('emits HAS_PROPERTY edges for nested type chain', () => {
    ...
    expect(edgeSet(propEdges)).toContain('User â†?address');
    expect(edgeSet(propEdges)).toContain('Address â†?city');
    expect(edgeSet(propEdges)).toContain('City â†?zipCode');
  });

  it('resolves 2-level chain: user.address.save() â†?Address#save', () => {
    const calls = getRelationships(result, 'CALLS');
    const saveCalls = calls.filter((e) => e.target === 'save' && e.source === 'processUser');
    expect(saveCalls.length).toBe(1);
    expect(saveCalls[0].targetFilePath).toContain('models');
  ...
  it('resolves 3-level chain: user.address.city.getName() â†?City#getName', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Mixed chain resolution (field â†?call interleaved)
// ---------------------------------------------------------------------------
...
describe('Mixed field+call chain resolution (TypeScript)', () => {
  ...
  it('resolves callâ†’field chain: svc.getUser().address.save() â†?Address#save', () => {
    const calls = getRelationships(result, 'CALLS');
  ...
  it('resolves fieldâ†’call chain: user.getAddress().city.getName() â†?City#getName', () => {
    const calls = getRelationships(result, 'CALLS');
...
// Activates Tier 2b pendingCallResults â€?binds return type at TypeEnv build time.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Method chain binding (Phase 9C): getUser() â†?.address â†?.getCity() â†?.save()
...
describe('TypeScript method chain binding via unified fixpoint (Phase 9C)', () => {
  ...
  it('resolves city.save() to City#save via 3-step chain (callResult â†?fieldAccess â†?methodCallResult)', () => {
    const calls = getRelationships(result, 'CALLS');
    const saveCall = calls.find(
      (c) =>
...
// ---------------------------------------------------------------------------
// Phase A: Object destructuring â€?const { field } = receiver â†?fieldAccess PendingAssignment
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Phase A: Post-fixpoint for-loop replay â€?iterable resolved via callResult fixpoint
...
// ---------------------------------------------------------------------------
// Phase B: Deep MRO â€?walkParentChain() at depth 2 (Câ†’Bâ†’A)
...
describe('TypeScript grandparent method resolution via MRO (Phase B)', () => {
  ...
  it('emits EXTENDS edges: Bâ†’A, Câ†’B', () => {
    const extends_ = getRelationships(result, 'EXTENDS');
    expect(edgeSet(extends_)).toContain('B â†?A');
    expect(edgeSet(extends_)).toContain('C â†?B');
  });
...
// ---------------------------------------------------------------------------
// Phase C: TS null-check narrowing â€?if (x !== null) { x.save() }
...
// â”€â”€ Phase P: Virtual Dispatch via Constructor Type â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
describe('TypeScript virtual dispatch via constructor type (same-file)', () => {
  ...
  it('resolves fetchBall() calls from run() â€?proves virtual dispatch override', () => {
    const calls = getRelationships(result, 'CALLS');
    ...
    // receiver from Animal â†?Dog. dog.fetchBall() resolves directly.
    // Both target same nodeId â†?1 CALLS edge after dedup.
    expect(fetchCalls.length).toBe(1);
  });
});

// â”€â”€ Phase P: Overload Disambiguation via inferLiteralType â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
describe('TypeScript overload disambiguation via inferLiteralType', () => {
  ...
  it('detects lookup function with parameterTypes on graph node', () => {
    ...
    // generateId collision â†?1 graph node, first overload's parameterTypes wins
  ...
  it('emits CALLS edges from process() â†?lookup() via overload disambiguation', () => {
    const calls = getRelationships(result, 'CALLS');
    ...
    // enables resolution where previously 2 same-arity candidates â†?null.
    // Both calls resolve to same nodeId (ID collision) â†?1 CALLS edge after dedup.
    expect(lookupCalls.length).toBe(1);
  });
});

// â”€â”€ Phase P: Optional / Default Parameter Arity Resolution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
