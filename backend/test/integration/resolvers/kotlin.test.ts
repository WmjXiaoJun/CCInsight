...
describe('Kotlin heritage resolution', () => {
  ...
  it('resolves exactly 4 IMPORTS edges (JVM-style package imports)', () => {
    ...
    expect(edgeSet(imports)).toEqual([
      'User.kt â†?Serializable.kt',
      'User.kt â†?Validatable.kt',
      'UserService.kt â†?Serializable.kt',
      'UserService.kt â†?User.kt',
    ]);
  });
  ...
  it('resolves ambiguous validate() call through non-aliased import with import-resolved reason', () => {
    ...
    // validate is defined in both Validatable (interface) and User (override) â†?needs import scoping
  ...
  it('resolves unique save() call through non-aliased import', () => {
    const calls = getRelationships(result, 'CALLS');
    // save is unique globally (only in BaseModel) â†?resolves as unique-global
...
describe('Kotlin call resolution with arity filtering', () => {
  ...
  it('resolves processUser â†?writeAudit to util/OneArg.kt via arity narrowing', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Alias import resolution: import com.example.User as U resolves U â†?User
// ---------------------------------------------------------------------------
...
// Tests that Kotlin's navigation_expression â†?navigation_suffix AST structure
...
describe('Kotlin data class primary constructor property capture', () => {
  ...
  it('emits HAS_PROPERTY edges for primary constructor properties', () => {
    ...
    expect(edgeSet(propEdges)).toContain('User â†?name');
    expect(edgeSet(propEdges)).toContain('User â†?address');
    expect(edgeSet(propEdges)).toContain('User â†?age');
  });

  it('resolves user.address.save() â†?Address#save via data class field type', () => {
    const calls = getRelationships(result, 'CALLS');
    const saveCalls = calls.filter((e) => e.target === 'save');
    const addressSave = saveCalls.find(
      (e) => e.source === 'processUser' && e.targetFilePath.includes('Models'),
...
// ---------------------------------------------------------------------------
// Phase C: Kotlin null-check narrowing â€?if (x != null) { x.save() }
...
// â”€â”€ Phase P: Virtual Dispatch via Constructor Type (cross-file) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Phase P: Default Parameter Arity Resolution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
