...
describe('Ruby require_relative, heritage & property resolution', () => {
  ...
  it('emits CALLS from singleton method: factory â†?run_validations', () => {
  ...
  it('resolves 5 require_relative imports to IMPORTS edges', () => {
    ...
    expect(importEdges).toContain('user.rb â†?base_model.rb');
    expect(importEdges).toContain('user.rb â†?serializable.rb');
    expect(importEdges).toContain('user.rb â†?loggable.rb');
    expect(importEdges).toContain('user.rb â†?cacheable.rb');
    expect(importEdges).toContain('service.rb â†?user.rb');
  });
  ...
  it('emits EXTENDS edge: User â†?BaseModel', () => {
    ...
    expect(edges).toContain('User â†?BaseModel');
  });
  ...
  it('emits method-level CALLS: create_user â†?persist (member call)', () => {
    const calls = getRelationships(result, 'CALLS').filter(
  ...
  it('emits method-level CALLS: create_user â†?greet_user (member call)', () => {
    const calls = getRelationships(result, 'CALLS').filter(
  ...
  it('emits method-level CALLS: greet_user â†?persist (bare call)', () => {
    const calls = getRelationships(result, 'CALLS').filter(
  ...
  it('emits method-level CALLS: greet_user â†?serialize_data (bare call)', () => {
    const calls = getRelationships(result, 'CALLS').filter(
  ...
  it('emits method-level CALLS: persist â†?run_validations (bare call)', () => {
    const calls = getRelationships(result, 'CALLS').filter(
...
describe('Ruby call resolution with arity filtering', () => {
  ...
  it('resolves run_task â†?write_audit to one_arg.rb via arity narrowing', () => {
...
describe('Ruby member-call resolution', () => {
  ...
  it('resolves process_user â†?persist_record as a member call on User', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('Ruby local definition shadows import', () => {
  ...
  it('resolves run_app â†?do_work to same-file definition, not the imported one', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Constructor-inferred type resolution: user = User.new; user.save â†?User.save
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// YARD annotation type resolution: @param repo [UserRepo] â†?repo.save resolves
// ---------------------------------------------------------------------------
...
// The fixture has BOTH User#save and Repo#save â€?fuzzy matching alone
...
describe('Ruby return type inference via function call', () => {
  ...
  it('detects save method on both User and Repo (disambiguation required)', () => {
    const methods = getNodesByLabel(result, 'Method');
    // Both classes have save â€?fuzzy match alone cannot resolve this
    expect(methods.filter((m) => m === 'save').length).toBe(2);
  });
  ...
  it('resolves user.save to User#save via YARD @return [User] on get_user()', () => {
    ...
    // requires return type inference: get_user() â†?@return [User] â†?user is User
    const calls = getRelationships(result, 'CALLS');
...
describe('Ruby YARD generic type annotations (Hash<Symbol, User>)', () => {
  ...
  it('does NOT resolve cache param to a class (Hash<Symbol, UserRepo> is a generic container)', () => {
    // The @param cache [Hash<Symbol, UserRepo>] should extract type "Hash" â€?not "UserRepo".
  ...
  it('resolves admin_repo.save in audit() to AdminRepo#save via alternate @param [AdminRepo] order', () => {
    const calls = getRelationships(result, 'CALLS');
    // audit() calls admin_repo.save â€?should resolve via the alternate YARD format
...
// for chain extraction â€?the tree-sitter-ruby grammar differs from other languages.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Ruby for-in loop: for user in users â€?YARD @param resolution
// ---------------------------------------------------------------------------
...
describe('Field type resolution (Ruby)', () => {
  ...
  it('resolves user.address.save â†?Address#save via YARD @return [Address]', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Method chain binding (Phase 9C): get_user() â†?.get_address() â†?.get_city() â†?.save
// ---------------------------------------------------------------------------
