...
describe('C++ diamond inheritance', () => {
  ...
  it('captures speak as Method nodes (declaration in headers + definition in .cpp)', () => {
    ...
    // and duck.cpp (out-of-line definition) â€?all captured as Method nodes
...
describe('C++ call resolution with arity filtering', () => {
  ...
  it('resolves run â†?write_audit to one.h via arity narrowing', () => {
...
// ---------------------------------------------------------------------------
// Constructor-inferred type resolution: auto user = User(); user.save() â†?User.save
...
describe('C++ parent resolution', () => {
  ...
  it('emits EXTENDS edge: User â†?BaseModel (base_class_clause)', () => {
    const extends_ = getRelationships(result, 'EXTENDS');
    expect(extends_.length).toBe(1);
...
// ---------------------------------------------------------------------------
// Brace-init constructor inference: auto x = User{}; x.save() â†?User.save
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// C++ range-based for: for (auto& user : users) â€?Tier 1c
// ---------------------------------------------------------------------------
...
describe('C++ pointer/reference member field capture', () => {
  ...
  it('emits HAS_PROPERTY edges for pointer/reference fields', () => {
    const propEdges = getRelationships(result, 'HAS_PROPERTY');
    expect(edgeSet(propEdges)).toContain('User â†?address');
    expect(edgeSet(propEdges)).toContain('User â†?ref_address');
    expect(edgeSet(propEdges)).toContain('User â†?name');
  });
});
...
describe('C++ smart pointer virtual dispatch via make_shared', () => {
  ...
  it('emits CALLS edge from process â†?speak', () => {
    const calls = getRelationships(result, 'CALLS');
