...
describe('Go package import & call resolution', () => {
  ...
  it('resolves exactly 7 IMPORTS edges across Go packages', () => {
    ...
    expect(edgeSet(imports)).toEqual([
      'main.go â†?admin.go',
      'main.go â†?repository.go',
      'main.go â†?service.go',
      'main.go â†?user.go',
      'service.go â†?admin.go',
      'service.go â†?repository.go',
      'service.go â†?user.go',
    ]);
  });

  it('emits exactly 1 EXTENDS edge for struct embedding: Admin â†?User', () => {
...
describe('Go call resolution with arity filtering', () => {
  ...
  it('resolves main â†?WriteAudit to internal/onearg/log.go via arity narrowing', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Multi-assignment: user, repo := User{}, Repo{} â€?both sides captured in TypeEnv
// ---------------------------------------------------------------------------
...
describe('Go multi-assignment short var declaration', () => {
  ...
  it('resolves both struct literals in multi-assignment: User{} and Repo{}', () => {
    ...
    expect(edgeSet(structCalls)).toEqual(['process â†?Repo', 'process â†?User']);
  });
...
describe('Go parent resolution (struct embedding)', () => {
  ...
  it('emits EXTENDS edge: User â†?BaseModel (struct embedding)', () => {
    const extends_ = getRelationships(result, 'EXTENDS');
    expect(extends_.length).toBe(1);
...
// Go pointer types (*User) â€?extractSimpleTypeName strips pointer prefix.
// ---------------------------------------------------------------------------
