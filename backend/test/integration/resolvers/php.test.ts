...
describe('PHP heritage & import resolution', () => {
  ...
  it('emits 4 IMPLEMENTS edges: classâ†’interface + classâ†’trait', () => {
    const implements_ = getRelationships(result, 'IMPLEMENTS');
    expect(edgeSet(implements_)).toEqual([
      'BaseModel â†?HasTimestamps',
      'BaseModel â†?Loggable',
      'User â†?SoftDeletes',
      'UserService â†?Repository',
    ]);
  });
  ...
  it('resolves 6 IMPORTS edges via PSR-4 composer.json', () => {
    ...
    expect(edgeSet(imports)).toEqual([
      'BaseModel.php â†?HasTimestamps.php',
      'BaseModel.php â†?Loggable.php',
      'User.php â†?SoftDeletes.php',
      'UserService.php â†?Repository.php',
      'UserService.php â†?User.php',
      'UserService.php â†?UserRole.php',
    ]);
  });
  ...
  it('emits CALLS edge: save â†?getId', () => {
...
describe('PHP call resolution with arity filtering', () => {
  ...
  it('resolves create_user â†?write_audit to app/Utils/OneArg/log.php via arity narrowing', () => {
...
// ---------------------------------------------------------------------------
// Alias import resolution: use App\Models\User as U resolves U â†?User
// ---------------------------------------------------------------------------
...
describe('PHP alias import resolution', () => {
  ...
  it('emits exactly 2 IMPORTS edges via alias resolution', () => {
    ...
    expect(edgeSet(imports)).toEqual(['Main.php â†?Repo.php', 'Main.php â†?User.php']);
  });
});
...
describe('PHP grouped import with alias', () => {
  ...
  it('resolves non-aliased User via NamedImportMap (not just the aliased Repo)', () => {
    // Both User (non-aliased) and Râ†’Repo (aliased) should resolve through grouped import
    const calls = getRelationships(result, 'CALLS');
...
describe('PHP variadic call resolution', () => {
  ...
  it('resolves run â†?Logger.record despite extra args (variadic)', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('PHP constructor property promotion resolution', () => {
  ...
  // implemented. The promoted parameter type IS extracted into the TypeEnv â€?it just
...
describe('PHP return type inference via member call', () => {
  ...
  it('detects save on both User and Repo, and getUser method', () => {
    ...
    // save exists on both User and Repo â€?disambiguation required
...
// ---------------------------------------------------------------------------
// PHP foreach ($users as $user) â€?Tier 1c
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// PHP foreach with PHPDoc generic Collection<User> â€?element type extraction
...
// ---------------------------------------------------------------------------
// PHP foreach ($this->users as $user) â€?member access key mismatch fix
...
describe('PHP constructor promotion property capture', () => {
  ...
  it('resolves $user->address->save() â†?Address#save via promoted field type', () => {
    const calls = getRelationships(result, 'CALLS');
    const saveCalls = calls.filter((e) => e.target === 'save');
    const addressSave = saveCalls.find(
      (e) => e.source === 'processUser' && e.targetFilePath.includes('Models'),
...
// ---------------------------------------------------------------------------
// Method chain binding (Phase 9C): getUser() â†?->getCity() â†?->save()
// ---------------------------------------------------------------------------
