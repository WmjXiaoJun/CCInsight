...
describe('C# heritage resolution', () => {
  ...
  it('emits exactly 1 EXTENDS edge: User â†?BaseEntity', () => {
  ...
  it('emits exactly 1 IMPLEMENTS edge: User â†?IRepository', () => {
  ...
  it('emits CALLS edges from CreateUser (constructor + member calls)', () => {
    ...
    // _repo.Save() â†?IRepository.Save (primary) plus interface-dispatch â†?User.Save (impl)
    expect(calls.length).toBe(5);
    const targets = edgeSet(calls);
    expect(targets).toContain('CreateUser â†?User'); // new User() constructor
    expect(targets).toContain('CreateUser â†?Validate'); // user.Validate() â€?receiver-typed
    expect(targets).toContain('CreateUser â†?Save'); // _repo.Save() â€?IRepository + User (dispatch)
    expect(targets).toContain('CreateUser â†?Log'); // _logger.Log() â€?receiver-typed
  });
...
describe('C# call resolution with arity filtering', () => {
  ...
  it('resolves CreateUser â†?WriteAudit to Utils/OneArg.cs via arity narrowing', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Alias import resolution: using U = Models.User resolves U â†?User
// ---------------------------------------------------------------------------
...
describe('C# alias import resolution', () => {
  ...
  it('emits exactly 2 IMPORTS edges via alias resolution', () => {
    ...
    expect(edgeSet(imports)).toEqual(['Main.cs â†?Repo.cs', 'Main.cs â†?User.cs']);
  });
});
...
describe('C# parent resolution', () => {
  ...
  it('emits EXTENDS edge: User â†?BaseModel (from base_list)', () => {
    const extends_ = getRelationships(result, 'EXTENDS');
    expect(extends_.length).toBe(1);
    expect(extends_[0].source).toBe('User');
  ...
  it('emits IMPLEMENTS edge: User â†?ISerializable', () => {
    const implements_ = getRelationships(result, 'IMPLEMENTS');
    expect(implements_.length).toBe(1);
    expect(implements_[0].source).toBe('User');
...
describe('C# return type inference via var + invocation', () => {
  ...
  it('resolves user.Save() to User#Save (not Repo#Save) via return type of GetUser(): User', () => {
    // scanConstructorBinding binds `var user = svc.GetUser()` â†?calleeName "GetUser".
    ...
    expect(saveCall).toBeDefined();
    // Must NOT resolve to Repo.Save â€?that would mean disambiguation failed
    const repoSave = calls.find(
...
// ---------------------------------------------------------------------------
// C# is-pattern disambiguation: `if (obj is User user)` should bind user â†?User
...
// ---------------------------------------------------------------------------
// C# Dictionary .Values foreach â€?member_access_expression resolution
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// C# recursive_pattern: obj is User { Name: "Alice" } u â€?Phase 6.1
// ---------------------------------------------------------------------------
...
describe('C# recursive_pattern type resolution', () => {
  ...
  it('resolves exactly one Save call per target class (no cross-resolution)', () => {
    ...
    // u.Save() â†?User#Save only, r.Save() â†?Repo#Save only
...
// ---------------------------------------------------------------------------
// Phase C: C# null-check narrowing â€?if (x != null) and if (x is not null)
...
describe('C# overload disambiguation by parameter types', () => {
  ...
  it('emits CALLS edge from Run() â†?Lookup() via overload disambiguation', () => {
    ...
    // Both Lookup(42) and Lookup("alice") resolve to same nodeId â†?1 CALLS edge
...
describe('C# import resolution without .csproj (suffix fallback)', () => {
  ...
  // C# 'using Models;' is a namespace import â€?suffix matching cannot resolve
