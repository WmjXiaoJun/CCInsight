...
describe('Java heritage resolution', () => {
  ...
  it('emits exactly 1 EXTENDS edge: User â†?BaseModel', () => {
  ...
  it('emits exactly 2 IMPLEMENTS edges: User â†?Serializable, User â†?Validatable', () => {
    const implements_ = getRelationships(result, 'IMPLEMENTS');
    expect(implements_.length).toBe(2);
    expect(edgeSet(implements_)).toEqual(['User â†?Serializable', 'User â†?Validatable']);
  });
  ...
  it('resolves exactly 4 IMPORTS edges', () => {
    ...
    expect(edgeSet(imports)).toEqual([
      'User.java â†?Serializable.java',
      'User.java â†?Validatable.java',
      'UserService.java â†?Serializable.java',
      'UserService.java â†?User.java',
    ]);
  });
  ...
  it('emits exactly 3 CALLS edges', () => {
    ...
    expect(edgeSet(calls)).toEqual([
      'processUser â†?save',
      'processUser â†?serialize',
      'processUser â†?validate',
    ]);
  });
...
describe('Java call resolution with arity filtering', () => {
  ...
  it('resolves processUser â†?writeAudit to util/OneArg.java via arity narrowing', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('Java constructor-call resolution', () => {
  ...
  it('resolves new User() as a CALLS edge to the User constructor', () => {
    ...
    // Java has explicit constructor_declaration â†?Constructor node
...
// ---------------------------------------------------------------------------
// Java Optional<User> receiver resolution â€?extractSimpleTypeName unwraps
...
// Phase 5.2: extractPatternBinding on instanceof_expression binds user â†?User.
...
// ---------------------------------------------------------------------------
// Java Map .values() for-loop â€?method-aware type arg resolution
// ---------------------------------------------------------------------------
...
// â”€â”€ Phase P: Overload Disambiguation via Parameter Types â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
describe('Java overload disambiguation by parameter types', () => {
  ...
  it('emits CALLS edge from run() â†?lookup() via overload disambiguation', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('Java virtual dispatch via constructor type (same-file)', () => {
  ...
  it('resolves fetchBall() calls from run() â€?proves virtual dispatch override', () => {
    ...
    // receiver from Animal â†?Dog (since only Dog has fetchBall).
...
// â†?user is typed User via cross-file return type propagation
// ---------------------------------------------------------------------------
