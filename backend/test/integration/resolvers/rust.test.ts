...
describe('Rust trait implementation resolution', () => {
  ...
  it('emits exactly 2 IMPLEMENTS edges with reason trait-impl', () => {
    ...
    expect(edgeSet(implements_)).toEqual(['Button â†?Clickable', 'Button â†?Drawable']);
  ...
  it('resolves exactly 1 IMPORTS edge: main.rs â†?button.rs', () => {
...
describe('Rust call resolution with arity filtering', () => {
  ...
  it('resolves main â†?write_audit to src/onearg/mod.rs via arity narrowing', () => {
...
describe('Rust member-call resolution', () => {
  ...
  it('resolves process_user â†?save as a member call on User', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Alias import resolution: use crate::models::User as U resolves U â†?User
// ---------------------------------------------------------------------------
...
describe('Rust alias import resolution', () => {
  ...
  it('emits exactly 1 IMPORTS edge: src/main.rs â†?src/models.rs', () => {
    const imports = getRelationships(result, 'IMPORTS');
    expect(imports.length).toBe(1);
...
describe('Rust grouped import resolution', () => {
  ...
  it('resolves main â†?format_name to src/helpers/mod.rs', () => {
    const calls = getRelationships(result, 'CALLS');
  ...
  it('resolves main â†?validate_email to src/helpers/mod.rs', () => {
    const calls = getRelationships(result, 'CALLS');
  ...
  it('emits exactly 1 IMPORTS edge: main.rs â†?helpers/mod.rs', () => {
    const imports = getRelationships(result, 'IMPORTS');
    expect(imports.length).toBe(1);
    expect(imports[0].source).toBe('main.rs');
...
describe('Rust parent resolution (trait impl)', () => {
  ...
  it('emits IMPLEMENTS edge: User â†?Serializable (trait impl)', () => {
    const implements_ = getRelationships(result, 'IMPLEMENTS');
...
describe('Rust return type inference', () => {
  ...
  it('resolves main â†?get_user as a CALLS edge to src/models.rs', () => {
    const calls = getRelationships(result, 'CALLS');
...
describe('Rust return-type inference via function return type', () => {
  ...
  it('user.save() does NOT resolve to Repo#save', () => {
    ...
    // Should resolve to exactly one target â€?if it resolves at all, check it's the right one
    if (wrongSave) {
...
// ---------------------------------------------------------------------------
// Rust ::default() constructor resolution â€?scanner exclusion
// ---------------------------------------------------------------------------
...
describe('Rust ::default() constructor resolution', () => {
  ...
  it('resolves user.save() in process_with_default() via User::default() constructor', () => {
    ...
    // NOT by the scanner â€?the scanner excludes ::default() to avoid
  ...
  it('does NOT cross-contaminate (user.save() does not resolve to Repo#save)', () => {
    ...
    // Either undefined (correctly disambiguated) or present (both resolved) â€?no single wrong one
    if (wrongCall) {
...
// Rust Option<User> â€?stripNullable unwraps Option wrapper to inner type.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Rust Option<User> receiver resolution â€?extractSimpleTypeName unwraps
...
describe('Rust Option<User> receiver resolution via wrapper unwrapping', () => {
  ...
  it('resolves alias.save() to User#save via Option<User> â†?assignment chain', () => {
    const calls = getRelationships(result, 'CALLS');
    const userSave = calls.find(
      (c) =>
        c.target === 'save' &&
...
// ---------------------------------------------------------------------------
// if let Some(user) = opt â€?Phase 5.2 pattern binding: unwrap Option<T>
// `opt: Option<User>` â†?Option<User> is stored as "User" in TypeEnv via
// NULLABLE_WRAPPER_TYPES. extractPatternBinding maps `user` â†?"User".
// Disambiguation: User.save vs Repo.save â€?only User.save should be called.
// ---------------------------------------------------------------------------
...
// Result<User, AppError> â†?Err(e) should type e as AppError (typeArgs[1]).
...
// ---------------------------------------------------------------------------
// for user in users.iter() â€?call_expression iterable resolution
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// for user in get_users() â€?direct call_expression iterable resolution
...
// ---------------------------------------------------------------------------
// Phase 8: Field/property type resolution â€?struct field capture
// ---------------------------------------------------------------------------
...
describe('Deep field chain resolution (Rust)', () => {
  ...
  it('emits HAS_PROPERTY edges for nested type chain', () => {
    ...
    expect(edgeSet(propEdges)).toContain('User â†?address');
    expect(edgeSet(propEdges)).toContain('Address â†?city');
    expect(edgeSet(propEdges)).toContain('Address â†?street');
    expect(edgeSet(propEdges)).toContain('City â†?zip_code');
  });
  ...
  it('resolves 3-level chain: user.address.city.get_name() â†?City#get_name', () => {
    const calls = getRelationships(result, 'CALLS');
...
// ---------------------------------------------------------------------------
// Method chain binding (Phase 9C): get_user() â†?.address â†?.get_city() â†?.save()
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Phase A: Rust struct_pattern destructuring â€?let Point { x, y } = p
// Each field emits a fieldAccess PendingAssignment; fixpoint resolves x/y â†?Vec2
// ---------------------------------------------------------------------------
...
describe('Rust struct_pattern destructuring resolution (Phase A)', () => {
  ...
  it('resolves both x.save() and y.save() â€?emits at least 1 CALLS to Vec2#save', () => {
    const calls = getRelationships(result, 'CALLS');
    const saveCalls = calls.filter((c) => c.target === 'save' && c.targetFilePath.includes('vec2'));
    // Both x and y are Vec2 â€?the same function, so calls may deduplicate to 1
...
// â†?u is typed User via cross-file return type propagation
// ---------------------------------------------------------------------------
