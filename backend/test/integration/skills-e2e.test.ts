 ...
 *
 * Tests `ccinsight analyze --skills` across 11 supported languages plus
...
// Absolute file:// URL to tsx loader â€?needed when spawning CLI with cwd
...
function assertSkillFiles(
  ...
  expect(fs.existsSync(path.join(tmpDir, '.ccinsight'))).toBe(true);
  ...
  if (!fs.existsSync(generatedDir)) {
    ...
    // Skip skill assertions gracefully â€?this is platform-dependent.
    return false;
  }
