 ...
 * - analyzeCommand: verify pipeline runs and creates .ccinsight/ output
 *
...
afterAll(() => {
  // Clean up .git/ and .ccinsight/ directories created during the test
  for (const dir of ['.git', '.ccinsight']) {
...
describe('CLI end-to-end', () => {
  ...
  it('analyze command runs pipeline on mini-repo', () => {
    ...
    // Successful analyze should create .ccinsight/ output directory
    const ccinsightDir = path.join(MINI_REPO, '.ccinsight');
    expect(fs.existsSync(ccinsightDir)).toBe(true);
    expect(fs.statSync(ccinsightDir).isDirectory()).toBe(true);
  });
  ...
  describe('unhappy path', () => {
    ...
    it('shows help with --help flag', () => {
      ...
      expect(result.stdout).toMatch(/ccinsight/i);
  ...
  describe('CLI error handling', () => {
    ...
    it('status on non-indexed repo reports not indexed', () => {
      ...
      // finds the parent project's .ccinsight. Use an isolated temp git
      // repo to guarantee no .ccinsight exists anywhere in the path.
  ...
  describe('wiki command flags', () => {
    ...
    it('wiki on non-indexed repo fails with "No ccinsight index"', () => {
      ...
      try {
        ...
        // Must spawn outside project tree so it doesn't find parent .ccinsight
        ...
        expect(result.status).toBe(1);
        expect(result.stdout).toMatch(/No ccinsight index found/);
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
  ...
  // Requires analyze to have run first (the analyze test above populates .ccinsight/).
  ...
  describe('tool output goes to stdout via fd 1 (#324)', () => {
    ...
    it('impact: JSON appears on stdout, not stderr', () => {
      ...
      expect(result.status).toBe(0);
      // impact may return an error object (symbol not found) or a real result �?      // either way it must be valid JSON on stdout
      expect(() => JSON.parse(result.stdout.trim())).not.toThrow();
    });
    ...
    it('READY signal appears on stdout, not stderr', () => {
      return new Promise<void>((resolve, reject) => {
        ...
        child.stdout.on('data', (chunk: Buffer) => {
          ...
          if (stdoutBuffer.includes('ccinsight_EVAL_SERVER_READY:')) {
            foundOnStdout = true;
            child.kill('SIGTERM');
          }
        });
        ...
        child.stderr.on('data', (chunk: Buffer) => {
          ...
          if (text.includes('ccinsight_EVAL_SERVER_READY:')) {
            foundOnStderr = true;
            child.kill('SIGTERM');
          }
        });
