 ...
 * processCommunities â€?the public export. Graphs are built so that Leiden's community
...
describe('calculateCohesion â€?internal edge ratio', () => {
  /**
  ...
  it('cohesion is 1.0 when community has no external edges', async () => {
    ...
    // Single isolated clique of 4 â€?no boundary edges at all
   ...
   * by building a graph where one node has no edges â€?it won't appear in a
   ...
   *   - Total: 7, internal: 6, ratio: 6/7 â‰?0.857
   */
