 ...
 *
 * Tests generateSkillFiles() â€?the only public export from cli/skill-gen.ts.
...
// ============================================================================
// TESTS â€?RETURN VALUES
// ============================================================================

describe('generateSkillFiles â€?return values', () => {
  let tmpDir: string;
   ...
   * without crashing â€?it simply has no execution flows.
   */
  ...
  it('does not double-count nodes shared across aggregated communities', async () => {
    ...
    expect(result.skills).toHaveLength(1);
    // fileCount should be 3 (shared.ts, a.ts, b.ts) â€?not 4
...
describe('generateSkillFiles â€?file output', () => {
  let tmpDir: string;
