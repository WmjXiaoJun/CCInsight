...
describe('callLLM â€?auth header', () => {
  ...
  it('auto-detects Azure from URL when no provider field set', async () => {
    ...
    await callLLM('test', {
      ...
      maxTokens: 100,
      temperature: 0,
      // no provider field â€?should auto-detect from URL
    });
...
describe('callLLM â€?reasoning model params', () => {
  afterEach(() => vi.unstubAllGlobals());
...
describe('callLLM â€?Azure content_filter error', () => {
  afterEach(() => vi.unstubAllGlobals());
...
describe('readSSEStream â€?content_filter handling', () => {
  afterEach(() => vi.unstubAllGlobals());
