 ...
 *   and ResolveCtx â€?that path is covered by test/integration/resolvers/rust.test.ts.
 * - PHP `use function` / `use const` filtering (via `extractPhpNamedBindings`) requires
 *   tree-sitter PHP nodes â€?covered by test/integration/resolvers/php.test.ts.
 */
...
// ---------------------------------------------------------------------------
// Minimal SyntaxNode stub â€?only the fields preprocessImportPath touches.
...
// ---------------------------------------------------------------------------
// preprocessImportPath â€?universal cleaning behaviour
// ---------------------------------------------------------------------------
...
describe('preprocessImportPath', () => {
  describe('quote and bracket stripping', () => {
    ...
    it('strips mixed quote and angle bracket characters', () => {
      const node = makeNode();
      // Pathological input â€?all stripped characters removed
      expect(
  ...
  describe('null returns for invalid inputs', () => {
    it('returns null for an empty string (after cleaning)', () => {
      const node = makeNode();
      // Only quote characters â€?cleaned result is empty string
      expect(
  ...
  describe('Kotlin wildcard pass-through', () => {
    it('delegates to appendKotlinWildcard when language is Kotlin â€?no wildcard child', () => {
    ...
    it('delegates to appendKotlinWildcard when language is Kotlin â€?wildcard_import child present', () => {
...
// ---------------------------------------------------------------------------
// The dispatch function requires a live ResolveCtx with file lists â€?unit
