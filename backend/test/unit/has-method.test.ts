...
`,
      ...
      // In TS, interface methods are method_signature nodes â€?find method name
      const nameNode = findNode(
        tree.rootNode,
...
`,
  ...
  describe('Go', () => {
    it('returns receiver struct ID for Go methods', () => {
      ...
      // findEnclosingClassId extracts the receiver type to link method â†?struct
      const tree = parseCode(
        Go,
        `
package main
...
`,
  ...
  describe('edge cases', () => {
    it('handles nested classes â€?returns innermost enclosing class', () => {
      const tree = parseCode(
        TypeScript.typescript,
        `
class Outer {
