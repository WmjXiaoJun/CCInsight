...
describe('getResourceDefinitions', () => {
  ...
  it('includes repos resource', () => {
    const defs = getResourceDefinitions();
    const repos = defs.find((d) => d.uri === 'ccinsight://repos');
  ...
  it('includes setup resource', () => {
    const defs = getResourceDefinitions();
    const setup = defs.find((d) => d.uri === 'ccinsight://setup');
...
describe('getResourceTemplates', () => {
  ...
  it('includes context, clusters, processes, schema, cluster detail, process detail', () => {
    ...
    expect(uris).toContain('ccinsight://repo/{name}/context');
    expect(uris).toContain('ccinsight://repo/{name}/clusters');
    expect(uris).toContain('ccinsight://repo/{name}/processes');
    expect(uris).toContain('ccinsight://repo/{name}/schema');
    expect(uris).toContain('ccinsight://repo/{name}/cluster/{clusterName}');
    expect(uris).toContain('ccinsight://repo/{name}/process/{processName}');
  });
...
describe('readResource', () => {
  it('routes ccinsight://repos to listRepos', async () => {
    ...
    const result = await readResource('ccinsight://repos', backend);
  ...
  it('routes ccinsight://setup to setup resource', async () => {
    const backend = createMockBackend({
      repos: [
        {
          name: 'proj',
    ...
    const result = await readResource('ccinsight://setup', backend);
    expect(result).toContain('ccinsight MCP');
  ...
  it('routes ccinsight://repo/{name}/context correctly', async () => {
    const backend = createMockBackend({
      context: {
    ...
    const result = await readResource('ccinsight://repo/test-project/context', backend);
  ...
  it('routes ccinsight://repo/{name}/schema to static schema', async () => {
    const backend = createMockBackend();
    const result = await readResource('ccinsight://repo/any/schema', backend);
    expect(result).toContain('ccinsight Graph Schema');
    expect(result).toContain('CALLS');
    expect(result).toContain('IMPORTS');
  });

  it('routes ccinsight://repo/{name}/clusters correctly', async () => {
    const backend = createMockBackend({
      clusters: {
        clusters: [{ heuristicLabel: 'Auth', symbolCount: 10, cohesion: 0.9 }],
      },
    });
    const result = await readResource('ccinsight://repo/test/clusters', backend);
  ...
  it('routes ccinsight://repo/{name}/processes correctly', async () => {
    const backend = createMockBackend({
      processes: {
        processes: [{ heuristicLabel: 'LoginFlow', processType: 'intra_community', stepCount: 3 }],
      },
    });
    const result = await readResource('ccinsight://repo/test/processes', backend);
  ...
  it('routes ccinsight://repo/{name}/cluster/{clusterName} correctly', async () => {
    const backend = createMockBackend({
      clusterDetail: {
    ...
    const result = await readResource('ccinsight://repo/test/cluster/Auth', backend);
  ...
  it('handles cluster detail error', async () => {
    ...
    const result = await readResource('ccinsight://repo/test/cluster/Missing', backend);
    expect(result).toContain('Cluster not found');
  });

  it('routes ccinsight://repo/{name}/process/{processName} correctly', async () => {
    const backend = createMockBackend({
      processDetail: {
    ...
    const result = await readResource('ccinsight://repo/test/process/LoginFlow', backend);
  ...
  it('handles process detail error', async () => {
    ...
    const result = await readResource('ccinsight://repo/test/process/Missing', backend);
    expect(result).toContain('Process not found');
  });

  it('throws for unknown resource URI', async () => {
    const backend = createMockBackend();
    await expect(readResource('ccinsight://unknown', backend)).rejects.toThrow(
  ...
  it('throws for unknown repo-scoped resource type', async () => {
    const backend = createMockBackend();
    await expect(readResource('ccinsight://repo/test/nonexistent', backend)).rejects.toThrow(
      'Unknown resource',
    );
  });
  ...
  it('decodes URI-encoded repo names', async () => {
    const backend = createMockBackend();
    await readResource('ccinsight://repo/my%20project/schema', backend);
  ...
  it('decodes URI-encoded cluster names', async () => {
    ...
    await readResource('ccinsight://repo/test/cluster/Auth%20Module', backend);
