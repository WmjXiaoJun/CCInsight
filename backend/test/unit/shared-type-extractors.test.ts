...
describe('extractElementTypeFromString', () => {
  // --- Array suffix ---
  it('strips array suffix: User[] â†?User', () => {
    expect(extractElementTypeFromString('User[]')).toBe('User');
  });

  it('strips array suffix for primitive: string[] â†?string', () => {
  ...
  // --- Go slice prefix ---
  it('strips Go slice prefix: []User â†?User', () => {
    expect(extractElementTypeFromString('[]User')).toBe('User');
  });

  it('strips Go slice prefix for primitive: []string â†?string', () => {
    expect(extractElementTypeFromString('[]string')).toBe('string');
  });

  // --- Swift array sugar ---
  it('unwraps Swift array sugar: [User] â†?User', () => {
    expect(extractElementTypeFromString('[User]')).toBe('User');
  });

  // --- Generic angle brackets ---
  it('unwraps single-arg generic angle: Array<User> â†?User', () => {
    expect(extractElementTypeFromString('Array<User>')).toBe('User');
  });

  it('unwraps single-arg generic angle: C++ vector<User> â†?User', () => {
    expect(extractElementTypeFromString('vector<User>')).toBe('User');
  });

  it('unwraps single-arg generic angle: Rust Vec<User> â†?User', () => {
    expect(extractElementTypeFromString('Vec<User>')).toBe('User');
  });

  // --- Generic square brackets ---
  it('unwraps Python subscript: List[User] â†?User', () => {
    expect(extractElementTypeFromString('List[User]')).toBe('User');
  });

  // --- Multi-argument generics ---
  it('returns last arg by default for Map<String, User>: â†?User', () => {
    expect(extractElementTypeFromString('Map<String, User>')).toBe('User');
  });

  it('returns first arg with pos=first for Map<String, User>: â†?String', () => {
    expect(extractElementTypeFromString('Map<String, User>', 'first')).toBe('String');
  });

  it('returns last arg explicitly for Map<String, User>: â†?User', () => {
  ...
  it('returns undefined at exactly the 2048-char limit', () => {
    // Length > 2048 is rejected; length === 2048 may or may not parse â€?test boundary
  ...
  it('returns undefined for malformed brackets: Map<String, User]', () => {
    // Mismatched bracket at depth 0 â†?malformed
  ...
  it('returns undefined when no brackets are present: User', () => {
    // No opening bracket â†?falls through to undefined
  ...
  it('does not treat [User] as Swift if it contains angle bracket (impossible case sanity-check)', () => {
    // [User<X>] â€?starts with [ ends with ] but contains <, so Swift path is skipped.
  ...
  it('handles whitespace in generic args: Array< User > â†?User', () => {
    expect(extractElementTypeFromString('Array< User >')).toBe('User');
  });
});
...
describe('stripNullable', () => {
  ...
  it('strips "| null": User | null â†?User', () => {
    expect(stripNullable('User | null')).toBe('User');
  });

  it('strips "| undefined": User | undefined â†?User', () => {
    expect(stripNullable('User | undefined')).toBe('User');
  });

  it('strips both "| null | undefined": User | null | undefined â†?User', () => {
    expect(stripNullable('User | null | undefined')).toBe('User');
  });

  // --- Nullable suffix ---
  it('strips nullable suffix: User? â†?User', () => {
  ...
  // --- Trimming ---
  it('trims surrounding whitespace: "  User  " â†?User', () => {
    expect(stripNullable('  User  ')).toBe('User');
  });

  // --- No-op for clean type names ---
  it('returns the type name unchanged when no nullable markers: User â†?User', () => {
    expect(stripNullable('User')).toBe('User');
  });
  ...
  it('returns "User?" when ? is embedded before pipe: User? | null', () => {
    ...
    // The pipe branch splits â†?parts: ['User?', 'null'] â†?filters 'null' â†?one part
...
describe('extractReturnTypeName', () => {
  ...
  it('strips Go pointer prefix: *User â†?User', () => {
    expect(extractReturnTypeName('*User')).toBe('User');
  });

  it('strips Rust reference prefix: &User â†?User', () => {
    expect(extractReturnTypeName('&User')).toBe('User');
  });

  it('strips Rust mutable reference prefix: &mut User â†?User', () => {
    expect(extractReturnTypeName('&mut User')).toBe('User');
  });

  // --- Non-wrapper generic returns base type ---
  it('returns base type for non-wrapper generic: List<User> â†?List', () => {
    expect(extractReturnTypeName('List<User>')).toBe('List');
  });

  // --- Qualified names ---
  it('extracts last segment of dot-qualified name: models.User â†?User', () => {
    expect(extractReturnTypeName('models.User')).toBe('User');
  });

  it('extracts last segment of PHP namespace: \\App\\Models\\User â†?User', () => {
    expect(extractReturnTypeName('\\App\\Models\\User')).toBe('User');
  });

  it('extracts last segment of Rust path: models::User â†?User', () => {
    expect(extractReturnTypeName('models::User')).toBe('User');
  });
  ...
  it('recursively unwraps nested wrappers: Future<Option<User>> â†?User', () => {
    expect(extractReturnTypeName('Future<Option<User>>')).toBe('User');
  });

  // --- Rust smart pointer ---
  it('unwraps Rc: Rc<User> â†?User', () => {
    expect(extractReturnTypeName('Rc<User>')).toBe('User');
  });

  it('unwraps Arc: Arc<User> â†?User', () => {
  ...
  it("skips Rust lifetime parameters in Result: Result<'_, User, Error> is unusual but lifetime in first pos is skipped", () => {
    // Lifetime as FIRST arg: Result<'a, User> â€?extractFirstTypeArg skips lifetimes
  ...
  it('accepts underscore-start identifier: _User', () => {
    // Pattern is /^[A-Z_]\w*$/ â€?underscore start is accepted
    expect(extractReturnTypeName('_User')).toBe('_User');
  });

  // --- Type with multiple pointer sigils ---
  it('strips multiple pointer sigils: **User â†?User', () => {
...
describe('methodToTypeArgPosition', () => {
  // --- Known single-element container â†?always 'last' ---
  ...
  it('returns first for keySet on HashMap (Java keyMethods via STD_KEY_METHODS â€?note: HashMap uses STD_KEY_METHODS, not JAVA_KEY_METHODS)', () => {
    // HashMap uses STD_KEY_METHODS which has 'keys', not 'keySet'
    // So keySet on HashMap is not a key method â†?falls to 'last'
  ...
  it('returns last when method is undefined and container is known map', () => {
    // containerTypeName is Map (arity 2), method is undefined â†?not in keyMethods â†?'last'
