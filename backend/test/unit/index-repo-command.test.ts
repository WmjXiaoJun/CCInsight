...
describe('indexCommand', () => {
  ...
  beforeEach(() => {
    ...
    mockGetStoragePaths.mockImplementation((repoPath: string) => ({
      storagePath: `${repoPath}/.ccinsight`,
      lbugPath: `${repoPath}/.ccinsight/lbug`,
      metaPath: `${repoPath}/.ccinsight/meta.json`,
    }));
  ...
  it('fails when .ccinsight folder does not exist', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    mockAccess.mockRejectedValueOnce(new Error('missing .ccinsight'));
    ...
    expect(logSpy).toHaveBeenCalledWith(
      `  No .ccinsight/ folder found at: ${resolvedRepo}/.ccinsight`,
    );
  });
  ...
  it('fails when lbug database does not exist', async () => {
    ...
    expect(process.exitCode).toBe(1);
    expect(logSpy).toHaveBeenCalledWith(
      '  .ccinsight/ folder exists but contains no LadybugDB index.',
    );
  });
