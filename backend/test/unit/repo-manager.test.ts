...
describe('getStoragePath', () => {
  it('appends .ccinsight to resolved repo path', () => {
    const result = getStoragePath('/home/user/project');
    expect(result).toContain('.ccinsight');
    expect(path.basename(result)).toBe('.ccinsight');
  });
...
describe('getStoragePaths', () => {
  it('returns storagePath, lbugPath, metaPath', () => {
    ...
    expect(paths.storagePath).toContain('.ccinsight');
...
describe('readRegistry', () => {
  it('returns empty array when registry does not exist', async () => {
    // readRegistry reads from ~/.ccinsight/registry.json
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('ccinsight-config-test-');
