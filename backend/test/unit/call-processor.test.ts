...
describe('processCallsFromExtracted', () => {
  ...
  it('refuses ambiguous global symbols â€?no CALLS edge created', async () => {
  ...
  it('return type inference: binds variable to return type of callee', async () => {
    ...
    // Binding: user = getUser() â€?getUser is not a class, so constructor path fails,
  ...
  it('return type inference: skips ambiguous callees (multiple definitions)', async () => {
    ...
    // Ambiguous callee â€?don't guess
    const rels = graph.relationships.filter((r) => r.type === 'CALLS');
    expect(rels).toHaveLength(0);
  });
  ...
  it('receiverKey collision: same scope funcName + same varName + same type resolves (non-ambiguous)', async () => {
    // Two save@* scopes both bind "db" to the same type â€?not ambiguous, should resolve.
    ctx.symbols.add('src/db/Database.ts', 'Database', 'Class:src/db/Database.ts:Database', 'Class');
    ctx.symbols.add('src/db/Database.ts', 'query', 'Method:src/db/Database.ts:query', 'Method', {
      ownerId: 'Class:src/db/Database.ts:Database',
    });
  ...
  it('receiverKey collision: same scope funcName + same varName + different types â†?ambiguous, no CALLS edge', async () => {
    // Two save@* scopes in the same file bind "db" to different types â€?truly ambiguous.
    ctx.symbols.add('src/db/Database.ts', 'Database', 'Class:src/db/Database.ts:Database', 'Class');
    ctx.symbols.add('src/db/Cache.ts', 'Cache', 'Class:src/db/Cache.ts:Cache', 'Class');
    ctx.symbols.add('src/db/Database.ts', 'query', 'Method:src/db/Database.ts:query', 'Method', {
      ownerId: 'Class:src/db/Database.ts:Database',
    ...
    // Ambiguous â€?different types for same funcName+varName, should not emit a CALLS edge
    const rels = graph.relationships.filter((r) => r.type === 'CALLS');
    expect(rels).toHaveLength(0);
  });
  ...
  it('scope-aware bindings: same varName in different functions resolves to correct type', async () => {
    ...
    // (the important thing is they don't collide â€?without scope awareness,
...
describe('extractReturnTypeName', () => {
  ...
  it('extracts qualified type: models.User â†?User', () => {
    expect(extractReturnTypeName('models.User')).toBe('User');
  });

  it('handles non-wrapper generics: Map<K, V> â†?Map', () => {
    expect(extractReturnTypeName('Map<string, User>')).toBe('Map');
  });

  it('handles nested wrapper: Promise<Option<User>>', () => {
    // Promise<Option<User>> â†?unwrap Promise â†?Option<User> â†?unwrap Option â†?User
  ...
  it('extracts Ruby :: qualified type: Models::User â†?User', () => {
    expect(extractReturnTypeName('Models::User')).toBe('User');
  });

  it('extracts C++ :: qualified type: ns::HttpClient â†?HttpClient', () => {
    expect(extractReturnTypeName('ns::HttpClient')).toBe('HttpClient');
  });

  it('extracts deep :: qualified type: crate::models::User â†?User', () => {
    expect(extractReturnTypeName('crate::models::User')).toBe('User');
  });

  it('extracts mixed qualifier: ns.module::User â†?User', () => {
  ...
  it('extracts deep dot-qualified: com.example.models.User â†?User', () => {
    expect(extractReturnTypeName('com.example.models.User')).toBe('User');
  });

  it('unwraps wrapper over non-wrapper generic: Promise<Map<string, User>> â†?Map', () => {
    // Promise is a wrapper â€?unwrap it to get Map<string, User>.
    // Map is not a wrapper, so return its base type: Map.
    expect(extractReturnTypeName('Promise<Map<string, User>>')).toBe('Map');
  });

  it('unwraps doubly-nested wrapper: Future<Result<User, Error>> â†?User', () => {
    // Future â†?unwrap â†?Result<User, Error>; Result â†?unwrap first arg â†?User
    expect(extractReturnTypeName('Future<Result<User, Error>>')).toBe('User');
  });

  it('unwraps CompletableFuture<Optional<User>> â†?User', () => {
    // CompletableFuture â†?unwrap â†?Optional<User>; Optional â†?unwrap â†?User
    expect(extractReturnTypeName('CompletableFuture<Optional<User>>')).toBe('User');
  });

  // Rust smart pointer unwrapping
  it('unwraps Rc<User> â†?User', () => {
    expect(extractReturnTypeName('Rc<User>')).toBe('User');
  });
  it('unwraps Arc<User> â†?User', () => {
    expect(extractReturnTypeName('Arc<User>')).toBe('User');
  });
  it('unwraps Weak<User> â†?User', () => {
    expect(extractReturnTypeName('Weak<User>')).toBe('User');
  });
  it('unwraps MutexGuard<User> â†?User', () => {
    expect(extractReturnTypeName('MutexGuard<User>')).toBe('User');
  });
  it('unwraps RwLockReadGuard<User> â†?User', () => {
    expect(extractReturnTypeName('RwLockReadGuard<User>')).toBe('User');
  });
  it('unwraps Cow<User> â†?User', () => {
    expect(extractReturnTypeName('Cow<User>')).toBe('User');
  });
  // Nested: Arc<Option<User>> â†?User (double unwrap)
  it('unwraps Arc<Option<User>> â†?User', () => {
  ...
  it("skips lifetime in Ref<'_, User> â†?User", () => {
    expect(extractReturnTypeName("Ref<'_, User>")).toBe('User');
  });
  it("skips lifetime in RefMut<'a, User> â†?User", () => {
    expect(extractReturnTypeName("RefMut<'a, User>")).toBe('User');
  });
  it("skips lifetime in MutexGuard<'_, User> â†?User", () => {
  ...
  it('extracts PHP backslash-namespaced type: \\App\\Models\\User â†?User', () => {
    expect(extractReturnTypeName('\\App\\Models\\User')).toBe('User');
  });

  it('extracts PHP single-segment namespace: \\User â†?User', () => {
    expect(extractReturnTypeName('\\User')).toBe('User');
  });

  it('extracts PHP deep namespace: \\Vendor\\Package\\Sub\\Client â†?Client', () => {
  ...
  it('pre-cap: accepts raw input at exactly 2048 characters (boundary)', () => {
    ...
    // result is undefined â€?but the pre-cap itself does NOT reject it.
  ...
  it('pre-cap: accepts inputs shorter than 2048 characters without rejection', () => {
    // 'User' is well under 2048 â€?should resolve normally.
    expect(extractReturnTypeName('User')).toBe('User');
  });
...
describe('processCallsFromExtracted â€?interface dispatch', () => {
  let graph: ReturnType<typeof createKnowledgeGraph>;
  let ctx: ResolutionContext;
