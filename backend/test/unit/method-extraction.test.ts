...
describe('Java MethodExtractor', () => {
  ...
  describe('no methods', () => {
    it('returns null for class with no methods', () => {
      ...
      // No method_declaration nodes â†?empty methods array
      expect(result).not.toBeNull();
...
describe('C# MethodExtractor', () => {
  ...
  describe('record struct', () => {
    // tree-sitter-c-sharp ^0.23.1 emits record_declaration for 'record struct' â€?    // there is no separate record_struct_declaration node type.
  ...
  describe('partial methods', () => {
    it('detects partial method declaration (no body)', () => {
      ...
      // partial declaration-only is not abstract â€?it's a compile-time slot
      expect(m!.isAbstract).toBe(false);
    });
    ...
    // public T GetValue<T>() â†?name: 'GetValue' (no <T>).
    // This is intentional â€?the call graph uses names, not signatures.
