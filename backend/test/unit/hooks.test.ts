 ...
 *
 * Tests the hook scripts (ccinsight-hook.cjs and ccinsight-hook.js) that run
 * as PreToolUse and PostToolUse hooks in Claude Code.
 *
 * Covers:
 * - extractPattern: pattern extraction from Grep/Glob/Bash tool inputs
 * - findccinsightDir: .ccinsight directory discovery
...
// ─── Test fixtures: temporary .ccinsight directory ───────────────────

let tmpDir: string;
let ccinsightDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ccinsight-hook-test-'));
...
describe('Windows .cmd extension handling', () => {
  ...
  it('Plugin hook uses .cmd extension for Windows ccinsight binary', () => {
    const source = fs.readFileSync(PLUGIN_HOOK, 'utf-8');
    expect(source).toContain('ccinsight.cmd');
  });
});
