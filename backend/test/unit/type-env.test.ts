...
/** Flatten a scoped TypeEnvironment into a simple nameâ†’type map (for simple test assertions). */
...
`,
      ...
      it('does NOT extract binding when the pattern is not a class_pattern', () => {
        // `case 42 as n:` â€?integer pattern, not a class_pattern
        const tree = parse(
          `
def process(x):
    match x:
...
`,
        ...
        const typeEnv = buildTypeEnv(tree, 'python');
        // No class_pattern child â€?should return undefined
...
`,
    ...
    it('handles qualified YARD types (Models::User â†?User)', () => {
      const tree = parse(
        `
...
`,
    ...
    it('handles nullable YARD types (String, nil â†?String)', () => {
      const tree = parse(
        `
...
`,
    ...
    it('skips ambiguous union YARD types (String, Integer â†?undefined)', () => {
      const tree = parse(
        `
...
`,
    ...
    it('handles generic YARD types (Array<User> â†?Array)', () => {
      const tree = parse(
        `
...
class Standalone {
  save(): boolean {
    return true;
      ...
      // No calls in this code â€?test the resolution function directly
...
function handleRepo(user: Repo) {
  user.save();
      ...
      // First call is inside handleUser â†?user should be User
      expect(typeEnv.lookup('user', calls[0])).toBe('User');
      // Second call is inside handleRepo â†?user should be Repo
...
class RepoService {
  process(repo: Repo) {
    repo.save();
      ...
      expect(calls.length).toBe(2);
      // First call inside UserService.process â†?user should be User
      expect(typeEnv.lookup('user', calls[0])).toBe('User');
      // Second call inside RepoService.process â†?repo should be Repo
  ...
  describe('destructuring patterns (known limitations)', () => {
    it('captures the typed source variable but not destructured bindings', () => {
      ...
      // â€?not extractable from annotations alone
    ...
    it('does not extract from object-type-annotated destructuring', () => {
      ...
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // Complex type annotation (object type) â€?extractSimpleTypeName returns undefined
      expect(flatSize(typeEnv)).toBe(0);
    });
  });
  ...
  describe('constructor inference (Tier 1 fallback)', () => {
    describe('TypeScript', () => {
      ...
      it('infers type from new expression with as cast', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'typescript');
        // Unwraps as_expression to find the inner new_expression â†?User
      ...
      it('infers type from new expression with non-null assertion', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'typescript');
        // Unwraps non_null_expression to find the inner new_expression â†?User
        expect(flatGet(typeEnv, 'x')).toBe('User');
      });
      ...
      it('infers type from double-cast (new X() as unknown as T)', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'typescript');
        // Unwraps nested as_expression to find inner new_expression â†?User
        expect(flatGet(typeEnv, 'x')).toBe('User');
      });
    ...
    describe('Rust if-let / while-let pattern bindings', () => {
      ...
      it('extracts binding from if let Some(x) = opt via Phase 5.2 pattern binding', () => {
        ...
        // extractPatternBinding maps `user` â†?"User" from the scopeEnv lookup for `opt`.
        expect(flatGet(typeEnv, 'user')).toBe('User');
      });
      ...
      it('does NOT extract field bindings from struct pattern destructuring', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'rust');
        // 'name' is a field of User â€?we don't know its type without field-type resolution
        expect(flatGet(typeEnv, 'name')).toBeUndefined();
      ...
      it('extracts type from scoped struct pattern (Message::Data)', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'rust');
        // scoped_type_identifier: Message::Data â€?extractSimpleTypeName returns "Data"
      ...
      it('Phase 5.2: extracts binding from if let Some(x) = opt where opt: Option<User>', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'rust');
        // opt: Option<User> â†?scopeEnv stores "User" (NULLABLE_WRAPPER_TYPES unwrapping)
        // extractPatternBinding maps user â†?opt's type â†?"User"
        expect(flatGet(typeEnv, 'user')).toBe('User');
      });
      ...
      it('Phase 5.2: does NOT extract binding when source variable is unknown', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'rust');
        // unknown_var is not in scopeEnv â€?conservative, produces no binding
        expect(flatGet(typeEnv, 'x')).toBeUndefined();
      });
      ...
      it('Phase 5.2: does NOT extract binding for non-Option/Result wrappers', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'rust');
        // SomeOtherVariant is not a known unwrap wrapper â€?no binding
        expect(flatGet(typeEnv, 'x')).toBeUndefined();
      });
    });
    ...
    describe('Java instanceof pattern variable (Phase 5.2)', () => {
      ...
      it('extracts boolean type from plain instanceof (no pattern variable)', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'java');
        // No pattern variable â€?b gets its declared type 'boolean', not 'User'
    ...
    describe('C++', () => {
      ...
      it('does not infer from auto with function call (not a known class)', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'cpp');
        // getUser is an identifier but NOT a known class â€?no inference
        expect(flatGet(typeEnv, 'x')).toBeUndefined();
      });
    ...
    describe('Kotlin constructor inference', () => {
      ...
      it('does NOT infer type from plain function call', () => {
        ...
        const typeEnv = buildTypeEnv(tree, 'kotlin');
        // getUser is not a class name â€?should NOT produce a binding
        expect(flatGet(typeEnv, 'user')).toBeUndefined();
      });
...
class User : BaseModel<string> {
  ...
  describe('constructorBindings merged into buildTypeEnv', () => {
    it('returns constructor bindings for Kotlin val x = UnknownClass()', () => {
      ...
      const typeEnv = buildTypeEnv(tree, 'kotlin');
      // UnknownClass is not defined locally â€?should appear as unverified binding
      expect(flatGet(typeEnv, 'user')).toBeUndefined();
    ...
    it('does NOT emit constructor binding when TypeEnv already resolved', () => {
      ...
      const typeEnv = buildTypeEnv(tree, 'kotlin');
      // Explicit annotation resolves it â€?no unverified binding needed
      expect(flatGet(typeEnv, 'user')).toBe('User');
      expect(typeEnv.constructorBindings.find((b) => b.varName === 'user')).toBeUndefined();
    });
...
`,
  ...
  describe('assignment chain propagation (Tier 2, depth-1)', () => {
    it('propagates explicit annotation: const a: User = ...; const b = a â†?b is User', () => {
      const tree = parse(
        `
    ...
    it('propagates constructor inference: const a = new User(); const b = a â†?b is User', () => {
      const tree = parse(
        `
        const a = new User();
        const b = a;
    ...
    it('depth-2 in declaration order resolves because single pass iterates sequentially', () => {
      // b = a â†?resolved (a has User), c = b â†?also resolved because the same
    ...
    it('does not propagate when RHS is a call expression (not a plain identifier)', () => {
      ...
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // getUser() is a call_expression â€?should not create a binding
      expect(flatGet(typeEnv, 'x')).toBeUndefined();
    });
  });

  describe('stripNullable', () => {
    it('strips User | null â†?User', () => {
      expect(stripNullable('User | null')).toBe('User');
    });

    it('strips User | undefined â†?User', () => {
      expect(stripNullable('User | undefined')).toBe('User');
    });

    it('strips User | null | undefined â†?User', () => {
      expect(stripNullable('User | null | undefined')).toBe('User');
    });

    it('strips User? â†?User', () => {
    ...
    it('refuses genuine union User | Repo â†?undefined', () => {
    ...
    it('strips User | void â†?User', () => {
      expect(stripNullable('User | void')).toBe('User');
    });

    it('strips User | None â†?User (Python)', () => {
      expect(stripNullable('User | None')).toBe('User');
    });

    it('strips User | nil â†?User (Ruby)', () => {
      expect(stripNullable('User | nil')).toBe('User');
    });

    it('strips User | void | nil â†?User (multiple nullable keywords)', () => {
    ...
    it('strips nullable suffix with spaces: User ? â†?User', () => {
      expect(stripNullable(' User? ')).toBe('User');
    });
  ...
  // â”€â”€ Assignment chain: reverse-order depth limitation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  describe('assignment chain â€?reverse-order limitation', () => {
    it('resolves reverse-declared Tier 2â†’Tier 0 (Tier 0 set during walk, before post-walk)', () => {
    ...
    it('resolves reverse-ordered Tier 2 chains via fixpoint (b = a, a = c, c: User)', () => {
      ...
      //   Iter 1: a = c (c is Tier 0 â†?a = User)
      //   Iter 2: b = a (a now resolved â†?b = User)
      const tree = parse(
        `
        function process() {
          const b = a;
  ...
  // â”€â”€ Assignment chain: per-language coverage for refactored code â”€â”€â”€â”€â”€â”€â”€â”€

  describe('assignment chain â€?Go var_spec form', () => {
  ...
  describe('assignment chain â€?C# equals_value_clause', () => {
  ...
  describe('assignment chain â€?Kotlin property_declaration', () => {
  ...
  describe('assignment chain â€?Java variable_declarator', () => {
  ...
  describe('assignment chain â€?Python identifier', () => {
...
def process():
  ...
  describe('assignment chain â€?Rust let_declaration', () => {
  ...
  describe('assignment chain â€?PHP variable_name', () => {
  ...
  describe('assignment chain â€?Ruby assignment', () => {
    it('captures assignment of simple identifier for pending propagation', () => {
      ...
      // In unit tests (no SymbolTable), constructor bindings are pending â€?so we test
...
`,
      ...
      const typeEnv = buildTypeEnv(tree, 'ruby');
      // get_user() is a call â€?user has no resolved type, so alias_user should not resolve either
      expect(flatGet(typeEnv, 'alias_user')).toBeUndefined();
    });
  });

  // â”€â”€ lookupInEnv with nullable stripping â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def process():
  ...
  // â”€â”€ extractSimpleTypeName: nullable wrapper unwrapping â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  describe('extractSimpleTypeName â€?nullable wrapper unwrapping', () => {
    it('unwraps Java Optional<User> â†?User', () => {
      const tree = parse(
        `
        class App {
          void process() {
    ...
    it('unwraps Rust Option<User> â†?User', () => {
      const tree = parse(
        `
        fn process() {
    ...
    it('does NOT unwrap List<User> â€?containers stay as List', () => {
      const tree = parse(
        `
        class App {
          void process() {
    ...
    it('does NOT unwrap Map<String, User> â€?containers stay as Map', () => {
      const tree = parse(
        `
        class App {
          void process() {
    ...
    it('does NOT unwrap CompletableFuture<User> â€?async wrappers stay', () => {
      const tree = parse(
        `
        class App {
          void process() {
  ...
  // â”€â”€ C++ assignment chain propagation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  describe('assignment chain â€?C++ auto alias', () => {
  ...
  // â”€â”€ Tier 1c: for-loop element type inference â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  describe('for-loop element type inference (Tier 1c) â€?TypeScript', () => {
    ...
    it('does NOT bind loop variable for for...in (produces string keys, not elements)', () => {
      ...
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // for...in yields string keys â€?extractor must NOT bind 'key' to User
  ...
  describe('for-loop element type inference (Tier 1c) â€?Python', () => {
...
def process():
    ...
    for user in users:
      ...
      const typeEnv = buildTypeEnv(tree, 'python');
      // List[User] â†?extractSimpleTypeName returns 'List' (base name), stored as 'List'
      // extractElementTypeFromString('List') â†?undefined (no brackets in the string)
  ...
  describe('for-loop element type inference (Tier 1c) â€?Go', () => {
...
func process() {
    for i := 0; i < 10; i++ {
      ...
      const typeEnv = buildTypeEnv(tree, 'go');
      // C-style for loop has no range_clause â€?extractor must return early
...
func process() {
    ...
    for _, user := range users {
      ...
      const typeEnv = buildTypeEnv(tree, 'go');
      // users has no type annotation â€?only a constructor binding candidate
      // Without a resolved type for users, user cannot be inferred
      expect(flatGet(typeEnv, 'user')).toBeUndefined();
    });
  });

  describe('for-loop element type inference (Tier 1c) â€?Rust', () => {
...
fn process() {
  ...
  describe('for-loop element type inference (Tier 1c) â€?C#', () => {
...
class Foo {
  ...
  describe('for-loop element type inference (Tier 1c) â€?Kotlin', () => {
...
fun process() {
  ...
  describe('for-loop element type inference (Tier 1c) â€?Java', () => {
...
class Foo {
  ...
  describe('previously-skipped limitations (now resolved)', () => {
    it('TS destructured for-of: for (const [k, v] of entries) â€?last-child heuristic', () => {
...
function process(entries: Map<string, User>) {
  for (const [key, user] of entries) {
    ...
    it('Python tuple unpacking: for key, value in dict.items() â€?call iterable + pattern_list', () => {
      // call iterable: data.items() â†?extract receiver 'data' for type lookup.
...
def process(data: dict[str, User]):
    ...
    it('Python enumerate(dict.items()): for i, k, v â€?skips int index, binds value var to User', () => {
      // enumerate() wraps the iterable: right node is call with fn='enumerate', not fn.attribute.
      // Without enumerate() support, iterableName is never set â†?v stays unbound.
      // With the fix: unwrap enumerate â†?inner call â†?data.items() â†?v binds to User.
      const tree = parse(
        `
def process(data: dict[str, User]):
    for i, k, v in enumerate(data.items()):
      ...
      const typeEnv = buildTypeEnv(tree, 'python');
      expect(flatGet(typeEnv, 'v')).toBe('User');
      // i is the int index from enumerate â€?must NOT be bound to User
      expect(flatGet(typeEnv, 'i')).toBeUndefined();
    });

    it('Python enumerate(dict.items()) with nested tuple: for i, (k, v) â€?binds v to User', () => {
...
def process(data: dict[str, User]):
    ...
    it('Python enumerate with parenthesized tuple: for (k, v) in enumerate(users) â€?binds v to User', () => {
...
def process(users: List[User]):
    for (k, v) in enumerate(users):
      ...
      const typeEnv = buildTypeEnv(tree, 'python');
      // enumerate yields (index, element) â€?k is int (unbound), v is User
      expect(flatGet(typeEnv, 'v')).toBe('User');
      expect(flatGet(typeEnv, 'k')).toBeUndefined();
    });

    it('TS instanceof narrowing: if (x instanceof User) â€?first-writer-wins, not block-scoped', () => {
...
function process(x) {
  if (x instanceof User) {
    ...
    it('Rust for with .iter(): for user in users.iter() â€?call_expression iterable', () => {
...
fn process(data: HashMap<String, User>) {
    ...
    it('Vec.iter() resolves to element type (User) â€?arity 1 always returns last', () => {
      const tree = parse(
        `
fn process(users: Vec<User>) {
    for user in users.iter() {
...
function process(cache: MyCache<string, User>) {
  ...
  describe('for-loop Phase 2 enhancements', () => {
    it('TS object destructuring skip: for (const { id, name } of users) â€?no binding produced', () => {
      const tree = parse(
        `
function process(users: User[]) {
  for (const { id, name } of users) {
    console.log(id, name);
      ...
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // Object destructuring should NOT produce bindings â€?field types are unknown
      expect(flatGet(typeEnv, 'id')).toBeUndefined();
      expect(flatGet(typeEnv, 'name')).toBeUndefined();
    });

    it('TS member access: for (const user of this.users) with users: User[] param â€?resolves', () => {
      const tree = parse(
        `
function process(users: User[]) {
  for (const user of this.users) {
    ...
    it('Python member access: for user in self.users with users: List[User] param â€?resolves', () => {
      const tree = parse(
        `
def process(users: List[User]):
    ...
    it('C++ structured bindings: for (auto& [key, value] : map) with map<string, User> param â€?binds value', () => {
      const tree = parse(
        `
void process(std::map<std::string, User>& map) {
  for (auto& [key, value] : map) {
    ...
    it('C++ structured bindings: exact App.cpp fixture â€?binds user and repo', () => {
      const tree = parse(
        `
#include "User.h"
#include "Repo.h"
...
void processRepoMap(std::map<std::string, Repo> repoMap) {
  ...
  describe('known limitations (documented skip tests)', () => {
    it.skip('Ruby block parameter: users.each { |user| } â€?closure param inference, different feature', () => {
      // Not a for-loop; .each { |user| } is a method call with a block.
      // Requires closure parameter inference â€?a different feature category
...
fun process(x: Any) {
    ...
    it('when (x) { is User -> ...; is Admin -> ... } â€?last arm overwrites (allowPatternBindingOverwrite)', () => {
      const tree = parse(
        `
fun process(x: Any) {
    when (x) {
      ...
      const typeEnv = buildTypeEnv(tree, 'kotlin');
      // allowPatternBindingOverwrite means each arm overwrites â€?last one wins
      expect(flatGet(typeEnv, 'x')).toBe('Admin');
    });

    it('when (x) { else -> } â€?no type check, no pattern binding produced', () => {
      const tree = parse(
        `
fun process() {
    val x: String = ""
    when (x) {
      ...
      const typeEnv = buildTypeEnv(tree, 'kotlin');
      // x has type String from its declaration â€?no pattern binding should narrow it
...
class App {
    ...
    it('switch (obj) { case User u -> ...; case Admin a -> ... } â€?both bind', () => {
      const tree = parse(
        `
class App {
    void process(Object obj) {
    ...
    it('switch (x) { case 42 -> ... } â€?no pattern variable, no binding', () => {
      const tree = parse(
        `
class App {
    ...
    it('obj instanceof User user â€?regression: still works after type_pattern addition', () => {
      const tree = parse(
        `
class App {
    void process(Object obj) {
...
class App {
  ...
  describe('C# recursive_pattern binding (Phase 6.1)', () => {
    it('obj is User { Name: "Alice" } u â€?binds u to User', () => {
      const tree = parse(
        `
public class App {
    ...
    it('switch expression with recursive_pattern â€?binds r to Repo', () => {
      const tree = parse(
        `
public class App {
    public void Process(object obj) {
    ...
    it('recursive_pattern without designation â€?no pattern binding produced', () => {
      const tree = parse(
        `
public class App {
    public void Process(object obj) {
      ...
      const typeEnv = buildTypeEnv(tree, 'csharp');
      // obj â†?object from the parameter, but no pattern binding
  ...
  describe('C# await foreach (Phase 6.1)', () => {
    it('await foreach (var user in users) â€?same node type as foreach, resolves element type', () => {
      const tree = parse(
        `
using System.Collections.Generic;
public class App {
    ...
    it('foreach (var user in this.data.Values) â€?nested member access with container property', () => {
      const tree = parse(
        `
using System.Collections.Generic;
public class App {
  ...
  describe('TypeScript class field declaration (Phase 6.1)', () => {
    it('class field with array type â€?for-loop resolves element type via declarationTypeNodes', () => {
      const tree = parse(
        `
class UserService {
    private users: User[] = [];
    processUsers() {
      ...
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // User[] is an array_type â€?extractSimpleTypeName returns undefined (no simple base name).
    ...
    it('class field with generic type annotation â€?binds field name to base type', () => {
      const tree = parse(
        `
class RepoService {
  ...
  describe('PHP foreach $this->property (Phase 7.4 â€?Strategy C)', () => {
...
class B {
  ...
  describe('match arm scoping â€?first-writer-wins regression', () => {
...
fn process(opt: Option<User>) {
  ...
  describe('performance optimizations â€?coverage for new code paths', () => {
    it('fastStripNullable: passes through simple identifier without stripping', () => {
      ...
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // lookup exercises fastStripNullable â€?"User" has no | or ? markers
...
function process(x: User | null) {
  if (x !== null) {
    ...
    x.fallback();
      ...
      // the flat scopeEnv which has "User | null" â€?fastStripNullable strips it to User.
...
function process(x: User | null) {
  ...
  x.other();
      ...
      const tree = parse(code, TypeScript.typescript);
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // After the if-block, x should use the flat scopeEnv (User | null â†?User via fastStripNullable)
...
function process(x: User) {
  if (x !== null) {
    x.save();
      ...
      const tree = parse(code, TypeScript.typescript);
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // x is already non-nullable â€?no narrowing override is emitted, but lookup still works
      const saveCall = tree.rootNode.descendantForIndex(tree.rootNode.text.indexOf('x.save'));
      expect(typeEnv.lookup('x', saveCall)).toBe('User');
    });
...
class App {
    ...
    it('Java: untyped declaration before typed does not get false type association', () => {
      // `x` has no type annotation â†?must NOT be associated with the User type
...
class App {
    ...
    it('TypeScript: const a: Animal = new Dog() â€?constructorTypeMap not populated (type on variable_declarator, not lexical_declaration)', () => {
      // TS virtual dispatch for this pattern works through call-processor,
      // not constructorTypeMap â€?the type annotation is on the child
...
class App {
  ...
  describe('Phase 14: importedBindings seeding', () => {
    ...
    it('local declarations take precedence over imported bindings', () => {
      // Source declares config: AppConfig â€?imported binding for 'config' must not overwrite it
...
function process() {
  config.validate();
      ...
      // config.validate() â€?lookup 'config' from inside the process function scope
  ...
  describe('importedReturnTypes (Phase 14 E3)', () => {
    ...
    it('SymbolTable has unambiguous match â†?uses it, ignores cross-file', () => {
      // SymbolTable knows getConfig() returns Config (SymbolType)
      // importedReturnTypes says getConfig â†?WrongType â€?SymbolTable must win
    ...
    it('SymbolTable has no match (0 results) â†?falls back to cross-file', () => {
    ...
    it('SymbolTable has 2+ matches (ambiguous) â†?returns undefined, NO cross-file fallback', () => {
      // Two overloads of process() â€?ambiguous â†?must NOT fall back to cross-file
      const symbolTable = makeSymbolTable([
      ...
      // Ambiguous â†?conservative â†?no binding produced
      expect(flatGet(typeEnv, 'r')).toBeUndefined();
    });

    it('no SymbolTable â†?uses cross-file return types directly', () => {
      const tree = parse('const c = getConfig();', TypeScript.typescript);
      const typeEnv = buildTypeEnv(tree, 'typescript', {
        importedReturnTypes: new Map([['getConfig', 'Config']]),
      });
    ...
    it('cross-file has entry but SymbolTable covers it â†?SymbolTable wins', () => {
    ...
    it('does nothing when importedReturnTypes is absent', () => {
      const tree = parse('const c = getConfig();', TypeScript.typescript);
      const typeEnv = buildTypeEnv(tree, 'typescript');
      // No annotation, no SymbolTable, no cross-file â†?no binding
    ...
    it('resolved cross-file callee enables downstream call edges via lookup', () => {
      ...
      // importedReturnTypes maps getConfig â†?Config
