 ...
 *
 * Tests: ccinsight_TOOLS from tools.ts
...
import { ccinsight_TOOLS } from '../../src/mcp/tools.js';

describe('ccinsight_TOOLS', () => {
  it('exports all tools (7 base + 3 route/tool/shape + 1 api_impact)', () => {
    expect(ccinsight_TOOLS).toHaveLength(11);
  });

  it('contains all expected tool names', () => {
    const names = ccinsight_TOOLS.map((t) => t.name);
  ...
  it('each tool has name, description, and inputSchema', () => {
    for (const tool of ccinsight_TOOLS) {
  ...
  it('query tool requires "query" parameter', () => {
    const queryTool = ccinsight_TOOLS.find((t) => t.name === 'query')!;
  ...
  it('api_impact tool has no required parameters', () => {
    const apiImpactTool = ccinsight_TOOLS.find((t) => t.name === 'api_impact')!;
  ...
  it('route_map description defers to api_impact for pre-change analysis', () => {
    const routeMapTool = ccinsight_TOOLS.find((t) => t.name === 'route_map')!;
  ...
  it('shape_check description defers to api_impact for pre-change analysis', () => {
    const shapeCheckTool = ccinsight_TOOLS.find((t) => t.name === 'shape_check')!;
