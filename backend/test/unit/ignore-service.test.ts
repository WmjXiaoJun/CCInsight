...
describe('loadIgnoreRules', () => {
  ...
  it('parses .ccinsightignore file', async () => {
    await fs.writeFile(path.join(tmpDir, '.ccinsightignore'), 'vendor/\n*.test.ts\n');
    const ig = await loadIgnoreRules(tmpDir);
    expect(ig).not.toBeNull();
    ...
    await fs.unlink(path.join(tmpDir, '.ccinsightignore'));
  });
...
describe('loadIgnoreRules â€?error handling', () => {
  let tmpDir: string;
...
describe('loadIgnoreRules â€?ccinsight_NO_GITIGNORE env var', () => {
  let tmpDir: string;
  ...
  it('skips .gitignore when ccinsight_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.gitignore'), 'data/\n');

    const original = process.env.ccinsight_NO_GITIGNORE;
    process.env.ccinsight_NO_GITIGNORE = '1';
    try {
      const ig = await loadIgnoreRules(tmpDir);
      // .gitignore should be skipped â€?no rules loaded
      expect(ig).toBeNull();
    } finally {
      if (original === undefined) {
        delete process.env.ccinsight_NO_GITIGNORE;
      } else {
        process.env.ccinsight_NO_GITIGNORE = original;
      }
      await fs.unlink(path.join(tmpDir, '.gitignore'));
    }
  });

  it('still reads .ccinsightignore when ccinsight_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.ccinsightignore'), 'vendor/\n');
