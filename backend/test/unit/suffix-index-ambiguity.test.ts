...
describe('resolvePythonImport â€?proximity-based resolution for Python', () => {
  it('resolves bare import to same-directory file when multiple files share the name', () => {
    const ctx = makeCtx([
      'app/models/user.py', // indexed first â€?would win without proximity
  ...
  it('handles importer at repo root (no directory) without crashing', () => {
    ...
    // importerDir is '' â€?proximity skipped, suffix fallback used
  ...
  it('resolves same-directory package (user/__init__.py) via proximity', () => {
    const ctx = makeCtx([
      'app/models/user/__init__.py', // indexed first â€?would win without proximity
  ...
  it('returns null when relative import dots exceed directory depth (PEP 328 over-traversal)', () => {
    ...
    // '...user' has 3 dots â†?2 upward hops required, but only 1 directory level exists.
...
// ---------------------------------------------------------------------------
// Namespace packages (PEP 420) â€?directory with no __init__.py
// ---------------------------------------------------------------------------

describe('resolvePythonImport â€?namespace packages (no __init__.py)', () => {
  ...
  it('bare import of namespace package returns null (no file to resolve to)', () => {
    // `import user` â€?proximity finds neither user.py nor user/__init__.py.
  ...
  it('submodule form resolves correctly via suffixResolve fallback', () => {
    // `import user.model` â€?multi-segment, proximity skipped, suffixResolve finds user/model.py.
    const ctx = makeCtx(files);
...
describe('resolveImportPath â€?Ruby bare require does not use proximity', () => {
  it('returns first-indexed file for bare require (Ruby $LOAD_PATH excludes current directory)', () => {
    ...
    // Ruby bare `require 'helpers'` searches $LOAD_PATH â€?current directory not included.
  ...
  it('resolves require_relative (dot-prefixed) to same-directory file via generic relative resolver', () => {
    ...
    // require_relative arrives as "./<path>" â€?caught by generic relative resolver, not proximity
    const result = resolve('lib/utils/formatter.rb', './helpers', SupportedLanguages.Ruby, ctx);
...
describe('resolveImportPath â€?no proximity for Java or TypeScript', () => {
  it('Java: fully-qualified import resolves to the correct file via unique suffix', () => {
    ...
    // "com.b.User" â†?"com/b/User" â†?unique suffix; no ambiguity
  ...
  it('TypeScript: relative import resolves via generic relative resolver', () => {
    ...
    // "./user" is explicit relative â€?resolved before proximity is checked
