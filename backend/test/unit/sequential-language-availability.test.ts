...
describe('sequential native parser availability', () => {
  ...
  it('warns when processImports skips files in verbose mode', async () => {
    ...
    const previous = process.env.ccinsight_VERBOSE;
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[ingestion] Skipped 1 swift file(s) in import processing â€?swift parser not available.',
    );
    ...
    if (previous === undefined) {
      ...
      process.env.ccinsight_VERBOSE = previous;
    }
  });
  ...
  it('warns when processCalls skips files in verbose mode', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[ingestion] Skipped 1 swift file(s) in call processing â€?swift parser not available.',
    );
  ...
  it('warns when processHeritage skips files in verbose mode', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[ingestion] Skipped 1 swift file(s) in heritage processing â€?swift parser not available.',
    );
