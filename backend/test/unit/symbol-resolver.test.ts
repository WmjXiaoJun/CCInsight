...
describe('ResolutionContext.resolve â€?resolveSymbol compatibility', () => {
  ...
  describe('Tier 2: Import-scoped resolution', () => {
    ...
    it('handles file with no imports â€?unique global falls through', () => {
  ...
  describe('Tier 3: Global resolution', () => {
    ...
    it('refuses ambiguous global â€?returns null when multiple candidates exist', () => {
...
describe('ResolutionContext.resolve â€?tier metadata', () => {
  let ctx: ResolutionContext;
  ...
  it('returns null for ambiguous global â€?refuses to guess', () => {
    ctx.symbols.add('src/a.ts', 'Config', 'Class:src/a.ts:Config', 'Class');
    ctx.symbols.add('src/b.ts', 'Config', 'Class:src/b.ts:Config', 'Class');
  ...
  it('Tier 1 wins over Tier 2 â€?same-file takes priority', () => {
...
describe('negative tests â€?ambiguous refusal per language family', () => {
  let ctx: ResolutionContext;
  ...
  it('TS/JS: two Logger definitions with no import â†?returns null', () => {
    ctx.symbols.add(
      'src/services/logger.ts',
      'Logger',
      'Class:src/services/logger.ts:Logger',
  ...
  it('Java: same-named class in different packages, no import â†?returns null', () => {
    ctx.symbols.add(
  ...
  it('C/C++: type defined in transitively-included header â†?returns null (not reachable via direct import)', () => {
  ...
  it('C#: two IService interfaces in different namespaces, no import â†?returns null', () => {
    ctx.symbols.add(
...
describe('heritage false-positive guard', () => {
  ...
  it('null from resolve prevents false edge â€?generateId fallback produces synthetic ID, not wrong match', () => {
    ctx.symbols.add(
      'src/api/base.ts',
      'BaseController',
...
describe('Tier 2b: PackageMap resolution (Go)', () => {
  ...
  it('does not resolve symbol from wrong package', () => {
    ...
    // Not in imported package, single global def â†?global tier
    expect(result).not.toBeNull();
    expect(result!.tier).toBe('global');
  });
  ...
  it('returns global without packageMap when ambiguous', () => {
    ...
    // No import or package match, 2 candidates â†?ambiguous â†?null
    expect(result).toBeNull();
  });
});
...
describe('per-file cache', () => {
  ...
  it('cache does not leak across files', () => {
    ...
    // Resolve from different file â€?should NOT use cache
