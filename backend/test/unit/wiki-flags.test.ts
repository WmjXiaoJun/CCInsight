...
describe('resolveLLMConfig', () => {
  ...
  beforeEach(async () => {
    ...
    const configDir = path.join(tmpDir, '.ccinsight');
...
describe('--verbose flag', () => {
  const originalEnv = process.env.ccinsight_VERBOSE;

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.ccinsight_VERBOSE;
    } else {
      process.env.ccinsight_VERBOSE = originalEnv;
    }
  });

  it('verboseLog writes to console when ccinsight_VERBOSE=1', async () => {
    ...
    expect(process.env.ccinsight_VERBOSE).toBe('1');

    consoleSpy.mockRestore();
  });

  it('verbose is off when ccinsight_VERBOSE is not set', () => {
    delete process.env.ccinsight_VERBOSE;
    expect(process.env.ccinsight_VERBOSE).toBeUndefined();
  });
});
