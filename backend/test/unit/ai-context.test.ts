...
describe('generateAIContextFiles', () => {
  ...
  beforeAll(async () => {
    ...
    storagePath = path.join(tmpDir, '.ccinsight');
  ...
  it('creates or updates CLAUDE.md with ccinsight section', async () => {
    const stats = { nodes: 50, edges: 100, processes: 5 };
    await generateAIContextFiles(tmpDir, storagePath, 'TestProject', stats);

    const claudeMdPath = path.join(tmpDir, 'CLAUDE.md');
    const content = await fs.readFile(claudeMdPath, 'utf-8');
    expect(content).toContain('ccinsight:start');
    expect(content).toContain('ccinsight:end');
  ...
  it('updates existing CLAUDE.md without duplicating', async () => {
    ...
    // Should only have one ccinsight section
    const starts = (content.match(/ccinsight:start/g) || []).length;
  ...
  it('installs skills files', async () => {
    ...
    const skillsDir = path.join(tmpDir, '.claude', 'skills', 'ccinsight');
    try {
