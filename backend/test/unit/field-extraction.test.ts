...
// ---------------------------------------------------------------------------
// Generic factory tests â€?TypeScript config
// ---------------------------------------------------------------------------

describe('GenericFieldExtractor â€?TypeScript config', () => {
  const parser = new Parser();
  ...
  it('uses body-discovery fallback when body type does not match config', () => {
    parser.setLanguage(TypeScript.typescript);
    // interface_declaration has an interface_body â€?which IS in the config bodyNodeTypes
...
describe('GenericFieldExtractor â€?Python', () => {
  const parser = new Parser();
...
`);
    ...
    expect(result).not.toBeNull();
    expect(result!.fields).toHaveLength(1);
    // __slots__ ends with __ â†?not private, but still starts with _ â†?protected
...
describe('GenericFieldExtractor â€?Go', () => {
  const parser = new Parser();
...
describe('GenericFieldExtractor â€?C++', () => {
  const parser = new Parser();
...
describe('GenericFieldExtractor â€?Ruby', () => {
  const parser = new Parser();
...
// ---------------------------------------------------------------------------
// C# config â€?primary constructor fields
// ---------------------------------------------------------------------------

describe('GenericFieldExtractor â€?C# primary constructor fields', () => {
  const parser = new Parser();
