/**
 * Vitest globalSetup â€?runs once in the MAIN process before any forks.
 *
...
export default async function setup({ provide }: GlobalSetupContext) {
  const tmpHandle = await createTempDir('ccinsight-shared-');
  ...
  try {
    ...
    // FTS may already be installed system-wide â€?not fatal
  }
