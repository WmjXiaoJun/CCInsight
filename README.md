...
**专为深度理解 Claude Code 架构而生 · 双语注解 · 交互式知识图谱**
...
- **交互式知识图谱** — Sigma.js 可视化图谱，点击节点直接跳转到源码与注解
- **Tier 分层导航** — 按重要性分层（核心层 → 通信层 → 命令层 → 工具层），优先学习最关键代码
- **智能代码搜索** — 图数据库查询 + 全文搜索，支持中文注解内容搜索
- **AI 辅助解读** — 接入 OpenAI / Gemini / Ollama 等 LLM，选中代码块 AI 自动生成中文解释
...
1. 克隆 `claude-code-source-code` 仓库到本地
2. 点击前端界面的「分析新仓库」按钮，选择该仓库路径
3. 等待索引完成（后端会解析代码结构、构建知识图谱）
4. 开始探索！
...
| Tier | 名称   | 文件数  | 描述                       |
|------|--------|---------|---------------------------|
| **T1** | 核心层 | ~20     | Agent 循环、工具接口、任务状态机 |
| **T2** | 通信层 | ~10     | API 客户端、服务器、上下文管理   |
| **T3** | 命令层 | ~10     | 斜杠命令系统、REPL 交互        |
| **T4** | 工具层 | ~30     | 文件系统、搜索、网络、子代理等    |

**学习路径建议**：从 T1 核心层开始，20 个文件即可掌握 Claude Code 80% 的设计理念。
